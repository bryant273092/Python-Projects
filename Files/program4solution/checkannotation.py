    ...
    def check(self,param,annot,value,check_history=''):
        ...
        def check_predicate():
            ...
            try:
                ...
                error_message = repr(param)+' annotation predicate('+str(annot)+') raised exception'+\
                              '\n  exception = '+type_as_str(message)+': '+str(message)+'\n'+check_history                 
            else:
                assert worked, repr(param)+' failed annotation check: value = '+repr(value)+\
                           '\n  predicate = '+str(annot)+'\n'+check_history 
            finally:
                if except_to_raise != None:
                    raise except_to_raise(error_message)
        ...
        def check_str():
            try:
                ...
                except_to_raise = AssertionError
                error_message = repr(param)+' annotation check(str predicate: '+repr(annot)+') raised exception'+\
                                  '\n  exception = '+type_as_str(message)+': '+str(message)+'\n'+check_history
            else:
        ...
        if annot == None:
            ...
            try:
                ...
                except_to_raise = AssertionError
                error_message = repr(param)+' annotation undecipherable: '+str(annot)+'\n'+check_history                 
            except Exception as message:
                if message.__class__ is AssertionError:
                    raise
                except_to_raise = AssertionError
                error_message = repr(param)+' annotation protocol('+str(annot)+') raised exception'+\
                                   '\n  exception = '+type_as_str(message)+': '+str(message)+'\n'+check_history                 
            finally:
                if except_to_raise != None:
                    raise except_to_raise(error_message)
    ...
    def __call__(self, *args, **kargs):
        ...
        try:
            ...
            for p in self._args.keys():
                if p in annot:
#                     print("calling check with:",p, annot[p],self._args[p])
#                     print(repr(annot[p])) #KLUDGE 
#                     print(annot[p]) #KLUDGE 
#                     print(repr(annot[p])) #KLUDGE 
...
#     
#     @Check_Annotation
#     def f (x : {str:int,int:str}) -> str: return x
#     f(dict())
#     class c: pass
#     def f(x:[lambda x : x>0]): pass
#     f = Check_Annotation(f)
