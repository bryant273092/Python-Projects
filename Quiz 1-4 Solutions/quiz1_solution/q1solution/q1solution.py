...
def integrate(f : callable, n : int) -> callable:
    assert type(n) == int and n>0, 'q1solutionF18.integrate n is not >0'
    def definite(a, b):
        assert a <= b, f'q1solutionF18.integrate: a({a}) is not <= b({b})'
        width = (b-a)/n
        return sum(width * f(a+i*width) for i in range(n))
    return definite
...
#    return sorted(ps.items(), key = lambda item)
...
def movie_rank(db : {str:{(str,int)}}) -> [(str,float)]:
    return sorted([(m,sum(s for _,s in sl)/len(sl)) for m,sl in db.items()], key = (lambda x : (-x[1],x[0])))
