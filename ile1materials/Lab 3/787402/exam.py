...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        result[line[0]][line[1]]=([int(i) for i in line[2:]])
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for i in db:
        for j in db[i]:
            if 'receiver' not in data[i]:
                data[i]['receiver'] = len(db[i][j])
            else:
                data[i]['receiver'] += len(db[i][j])
            if 'caller' not in data[j]:
                data[j]['caller'] = len(db[i][j])
            else:
                data[j]['caller'] += len(db[i][j])
    return sorted(data, key = lambda x: (-data[x]['receiver'],data[x]['caller'],x))
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for i in all_member:
        for j in all_member:
            if i in db and j in db[i]:
                if j in data[i]:
                    data[i][j] += sum(i for i in db[i][j])
                else:
                    data[i][j] = sum(i for i in db[i][j])
                if i in data[j]:
                    data[j][i] += sum(i for i in db[i][j])
                else:
                    data[j][i] = sum(i for i in db[i][j])
            else:
                if j in data[i]:
    ...
    for i in range(len(data)):
        result[i].extend(k[1] for j in data[i][1:] for k in j)
    return result
