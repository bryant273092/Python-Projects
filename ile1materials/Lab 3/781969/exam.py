...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if call.get(line[0]) != None and call.get(line[0]).get(line[1]) != None:
            call[line[0]][line[1]] = call[line[0]][line[1]] + [int(line[t]) for t in range(2, len(line))]
        else:
            call[line[0]][line[1]] = [int(line[t]) for t in range(2, len(line))]
    return call
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db:
        for r in db[c]:
            call[c] += len(db[c][r])
            call[r] += len(db[c][r])
    return [(key, call[key]) for key in sorted(call, key = lambda x : (-call[x], x))]
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db:
        for r in db[c]:
            mutual[c].append(r)
    for c in sorted(mutual):
        for r in mutual[c]:
            if c in mutual.get(r):
                hey = sorted([c,r])
                yo.add((hey[0], hey[1]))
    return yo
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db:
        for r in db[c]:
            call[c] += len(db[c][r])
            receive[r] += len(db[c][r])
    return[key for key in sorted(call, key = lambda x: (-call[x], receive[x]))]            



def matrix (db : {str:{str:[int]}}) -> [[int]]:
    listz = []
    for caller in sorted(db):
        listd = []
        for call in sorted(db):
            listd.append(db.get(caller).get(call, 0))
        listz.append(listd)
    print(listz)
    return listz
