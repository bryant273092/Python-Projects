...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for c,r in db.items():
        parties.add(c)
        for re,ts in db[c].items():
            parties.add(re)
    return parties

def read_db(file : open) -> {str:{str:[int]}}:
    db = defaultdict(dict)
    #recievers = defaultdict(list)

    for line in file:
        each_line = line.rstrip('\n').split(':')
        caller = each_line[0]
        reciever = each_line[1]
        time = each_line[2:]
        ...
        db[caller].update({reciever:int_time})
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    talker_freq = []
    for c, r in db.items():
        for re, ts in r.items():
            frequency[c] += len(ts)
            frequency[re] += len(ts)
    for caller, x in frequency.items():
        talker_freq.append((caller, x))
    return sorted(sorted(talker_freq), key = lambda x: x[1], reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c, r in db.items():
        for re in db[c].keys():
            if c in db[re] and re in db[c]:
                both_called.append(sorted([c,re]))
                
    for couple in both_called:
        mutual.add(tuple(couple))
    return mutual

def initiators (db : {str:{str:[int]}}) -> [str]:
    #decrease by number times they are caller
    #increase by number of times they are reciever
    #caller_x = defaultdict(int)
    caller_x = defaultdict(list)
    initiators = []
    reciever = 0
    caller = 0
    for c, r in db.items():
        ...
        caller_x[c].append(caller)
        caller_x[c].append(reciever)
        reciever = 0
        caller = 0
                        
    for caller, x in sorted(sorted(caller_x.items(), key = lambda x: x[1][1]), key = lambda x: x[1][0], reverse = True):
        initiators.append(caller)
