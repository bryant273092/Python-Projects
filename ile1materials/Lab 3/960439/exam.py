...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        try:
            dict1[line[0]][line[1]].extend([int(i) for i in line[2:]])
        except KeyError:
            dict1[line[0]][line[1]] = [int(i) for i in line[2:]]
    return dict(dict1)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for receiver in db[caller]:
            dict2[caller] += len(db[caller][receiver])
            dict2[receiver] += len(db[caller][receiver])
    return sorted(sorted(dict(dict2).items(), key = lambda x: x[0]),key = lambda x: x[1], reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db:
            ...
            for receiver in db[caller]:
                if len(db[caller][receiver])>=1:
                    try:
                        if (caller,receiver) not in set2 and (receiver, caller) not in set2:
                            if len(db[receiver][caller]) >= 1:
                                set2.add(tuple(sorted([caller, receiver])))
                    except KeyError:
                        continue
    return set2
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    callerdict = defaultdict(int)
    receiverdict = defaultdict(int)
    for caller in db:
        for receiver in db[caller]:
            callerdict[caller] += len(db[caller][receiver])
            receiverdict[receiver] += len(db[caller][receiver])
    return sorted(sorted(sorted(db), key = lambda x: receiverdict[x]), key = lambda x: callerdict[x], reverse = True)
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for caller in db:
        for receiver in db[caller]:
            try:
                matrixdict[caller][receiver]+=sum(db[caller][receiver])
                matrixdict[receiver][caller]+=sum(db[caller][receiver])
            except KeyError:
                matrixdict[caller][receiver] = sum(db[caller][receiver])
                matrixdict[receiver][caller]=sum(db[caller][receiver])
    print('MATRIXDICT', matrixdict)
    
    for name in sorted(matrixdict):
        for subname in sorted(matrixdict[name]):
            finaldict[name].append(matrixdict[name][subname])
    print('FINALDICT', finaldict)
    for i in sorted(dict(finaldict).items()):
        finallist.append(i[1])
    
    print('FINALLIST', finallist)
    x = sum([0,0])
    print('BOOOOOO', type(x), x)
    return finallist
