...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k,v in db.items():
        for i,j in v.items():
            d[i] += len(j)
            d[k] += len(j)
    return sorted([(k,v) for k,v in d.items()],key =(lambda a: (-a[1],a[0])))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    p = list(all_parties(db))
    return sorted(p, key = (lambda n: (-dc[n],d[n],n)  ))
