...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    r = set()
    for caller,sub in db.items():
        r.add(caller)
        for rece in sub.keys():
            r.add(rece)
    return r
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in content:
        ...
        for n in range(2,len(line)):
            line[n] = int(line[n])
        if r[line[0]].get(line[1],None) != None:
            r[line[0]][line[1]].extend(line[2:])
        else:
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller,sub in db.items():
        frec = 0
        for rece,time in sub.items():
            check[rece] = check[rece]+ len(time)
            frec += len(time)
        check[caller] = check[caller] + frec
    for p,time in check.items():
        r.append((p,time))
    return sorted(sorted(r,key = lambda x:x[0]),key = lambda x:x[1],reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller,sub in db.items():
        for rece,time in sub.items():
            if db.get(rece) != None:
                if caller in db.get(rece).keys():
                    s = sorted([caller,rece])
                    r.add(tuple(s))
    return r
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller,sub in db.items():
        f = 0
        for rece,time in sub.items():
            rec[rece] = rec[rece] + len(time)
            f += len(time)
        call[caller] = f
    for p,count in call.items():
        r.append([p,count,rec[p]])
    r = sorted(sorted(sorted(r,key = lambda x:x[0]),key= lambda x:x[2]),key=lambda x:x[1],reverse = True)
    for ele in r:
        result.append(ele[0])
    return result
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for caller,sub in db.items():
        ...
        for rece,time in sub.items():
            people.add(rece)
            check[caller][rece] = check[caller].get(rece,0)+sum(time)
            check[rece][caller] = check[rece].get(caller,0)+sum(time)
    people = sorted(list(people))
    for call in people:
        a = []
        for rece in people:
            a.append(check[call].get(rece,0))
        r.append(a)
    return r
