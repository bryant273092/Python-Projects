...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if items[1] not in result[items[0]].keys():
            result[items[0]].update({items[1]:list(int(item)for item in items[2:])})
        else:
            result[items[0]][items[1]].append(int(items[2]))
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k,v in result.items():
        result_lst.append((k,v))
    return sorted(sorted(result_lst), key = (lambda x : x[1]), reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db.keys():
        ...
        receiver_count = 0   
        for receiver in db[caller].keys():
            caller_count += len(db[caller][receiver])
            if caller in db[receiver].keys():
                receiver_count += len(db[receiver][caller])
            result[caller] = (caller_count, receiver_count)
            
    print(result)
    return sorted(result, key = lambda x: (-result[x][0], result[x][1], x))
