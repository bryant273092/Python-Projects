...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for i in info[2:]:
            if info[0] in db:
                if info[1] in db[info[0]]:
                    db[info[0]][info[1]].append(int(i))
                else:
                    db[info[0]].update({info[1]: [int(i)]})
            else:
                db.update({info[0]: {info[1]: [int(i)]}})
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db:
        for r in db[c]:
            if c in c_t:
                c_t[c] = c_t[c] + len(db[c][r])
            else:
                c_t.update({c: len(db[c][r])})
    for c in db:
        for r in db[c]:
            if r in r_t:
                r_t[r] = r_t[r] + len(db[c][r])
            else:
                r_t.update({r: len(db[c][r])})
    for c in c_t:
        if c in r_t:
            caller_num.update({c: c_t[c] + r_t[c]})
    return sorted([(c, caller_num[c]) for c in caller_num], key = lambda x : (-x[1], x[0]))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for i in c_t:
        if i in r_t:
            info.update({i: (c_t[i], r_t[i])})
    names = sorted([(n, info[n][0], info[n][1]) for n in info], key = lambda x : (-x[1], x[2], x[0]))
    for n in names:
        result.append(n[0])
    return result
