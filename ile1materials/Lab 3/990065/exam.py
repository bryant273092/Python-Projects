...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        caller, receiver = line[:2]
        calltimes = [int(time) for time in line[2:]]        
        if receiver in database[caller].keys():
            database[caller][receiver].extend(calltimes)
        else:
            database[caller].update({receiver: calltimes})
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller, data in db.items():
        for receiver, calltimes in data.items():
            calls[caller] += len(calltimes)            
            calls[receiver] += len(calltimes)
    return sorted(calls.items(), key = lambda f: (-f[1], f[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller, data in db.items():
        [outgoing[caller].add(receiver) for receiver in data.keys()]
            
    for caller, receivers in outgoing.items():
        for receiver in receivers:
            if caller in outgoing[receiver]:
                mutuals.append(tuple(sorted([caller, receiver])))
    return set(mutuals)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller, data in db.items():
        people.append(caller)
        for receiver, calltimes in data.items():
            if caller not in call_info:
                call_info[caller] = [len(calltimes), 0]
            else:
                call_info[caller][0] += len(calltimes)
            
            if receiver not in call_info:
                call_info[receiver] = [0, len(calltimes)]              
            else:
                call_info[receiver][1] += len(calltimes)
    return sorted(people, key = lambda f: (-call_info[f][0], call_info[f][1])) 
