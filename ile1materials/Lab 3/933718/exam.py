...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for item in file:
        ...
        if lst[0] not in dic:
            dic[lst[0]] = {lst[1]:[int(i) for i in lst[2:]]}
        elif lst[0] in dic:
            dic[lst[0]].update({lst[1]:[int(i) for i in lst[2:]]})
    return dic
        ...
        for k1,v1 in v.items():
            if k not in d:
                ...
                if k1 in d:
                    ...
                    d[k1] = len(v1)
    return [(k,v) for k,v in sorted(d.items(), key = lambda t: (-t[1], t[0]))]
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k,v in db.items():
        for k1,v1 in v.items():
            if len(v1) >= 1 and k in db[k1] and len(db[k1][k]) >= 1: #and k in d and len(d[k1][k]) >= 1 :
                if ((k1,k)) not in a:
                    add_this = sorted([k,k1])
                    a.add((add_this[0],add_this[1]))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    return [k for k,v in sorted(sorted(d.items(), key = lambda t: t[0], reverse = True), key = lambda t: (-t[1]))]
