...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        parts = line.split(':')
        c, r, ts = parts[0], parts[1], parts[2:]
        if r not in db[c]:
            db[c][r] = []
        db[c][r].extend([int(x) for x in ts])
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    tc = defaultdict(int)
    for c in db:
        tc[c] += sum([len(db[c][r]) for r in db[c]])
        for r in db[c]:
            tc[r] += len(db[c][r])
            
    return [(person,tc[person]) for person in sorted(tc, key = lambda person : (-tc[person], person))]
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db:
        for r in db[c]:
            if c in db[r] and r in db[c]:
                callers.append(tuple(sorted([r,c])))
    print(callers)
    return set(callers)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db:
        # Times person was a caller
        counts.append((c, sum([len(db[c][r]) for r in db[c]]), 0))
        for r in db[c]:
            # Times person was a receiver
            counts.append((r, len(db[c][r]), 1))
          
    tc = {} 
    for each in all_parties(db):
        tc[each] = [0, 0]
    
    for name,num,t in counts:
        tc[name][t] += num

    return [triple[0] for triple in sorted([(p,tc[p][0],tc[p][1]) for p in tc], key = lambda x : (-x[1],x[2],x[0]))]
