...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        newDict2[line_list[1]] = line_list[2:]
        newDict[line_list[0]] = newDict2
    return dict(newDict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for a,b in db.items():
        for c,d in b.items():
            newDict[a]+=len(d)
            newDict[c]+=len(d)
    return sorted(sorted(newDict.items()), key = (lambda t: t[1]), reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    newDict = defaultdict(int)
    newDict2 = defaultdict(int)
    for a,b in db.items():
        for c,d in b.items():
            newDict[a]+=len(d)
            newDict2[c]+=len(d)
    return sorted(sorted(newDict,key = (lambda t:newDict2[t])), key = (lambda t:newDict[t]), reverse=True)
