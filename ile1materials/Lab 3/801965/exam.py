...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key1 , value1 in db.items():
        for key2, value2 in value1.items():
            result[(key1, len(value2))]
    return sorted(list(result), key = lambda x:x[1] ,reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key1 , value1 in db.items():
        for key2, value2 in value1.items():
            if key1 in value1.items() and key2 in key1:
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for key1, value1 in db.items():
        result.append(key1)
    return sorted(list(result), key=lambda x:x[2][0], reverse = True)
