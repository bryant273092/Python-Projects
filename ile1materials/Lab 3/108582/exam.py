...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    parties_list = []
    for caller, recipient in db.items():
        parties_list.append(caller)
        for rec, num in recipient.items():
            parties_list.append(rec)
    return set(parties_list)


def read_db(file : open) -> {str:{str:[int]}}:
    lines = [lines.rstrip('\n').split(':') for lines in file]
    keys = list(set(item[0] for item in lines))
    db_dict = {key:{item[1]:item[2:] for item in lines if item[0] == key}for key in keys}
    return db_dict
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    return sorted(k for k, v in db.items, key = lambda x: len(x[1]))
