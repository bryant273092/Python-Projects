...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    names = set()
    for caller,dest in db.items():
        names.add(caller)
        for reciver in dest.keys():
            names.add(reciver)
    return names
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller,dest in db.items():
        for reciver in dest.keys():
            if (reciver,caller) not in result:
                result.add((caller,reciver))
    return result
