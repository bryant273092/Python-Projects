...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for key in db:
        all_set.add(key)
        for inner_key in db[key]:
            all_set.add(inner_key)
    return all_set
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        stuff_list = stripped_line.split(':')
        if stuff_list[1] in all_dict[stuff_list[0]]:
            for intg in stuff_list[2:]:
                all_dict[stuff_list[0]][stuff_list[1]].append(int(intg))
        else:
            all_dict[stuff_list[0]][stuff_list[1]] = [int(intg) for intg in stuff_list[2:]]
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key in db:
        ...
        for key2 in db:
            if key2 == key:
                for in_key in db[key2]:
                    for inte in db[key2][in_key]:
                        call_times += 1
            for inner_key in db[key2]:
                if inner_key == key:
                    for intg in db[key2][inner_key]:
                        call_times += 1
        tuple_list.append((key, call_times))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key in db:
        for inner_key in db[key]:
            if key in db[inner_key]:
                add_flag = True
                for pair in mutual_set:
                    if key in pair and inner_key in pair:
                        add_flag = False
                if add_flag:
                    mutual_set.add((key, inner_key))
    for tups in mutual_set:
        new_tup = sorted(tups)
        mutual_set.remove(tups)
        mutual_set.add(tuple(new_tup))
    return mutual_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    temp_dict = {}
    # list of names, sort by # of times caller -> # of times receiver -> alphabetical order
    # eg {'Alan': [caller, receiver]}
    
    for key in db:
        call_times = 0
        receive_times = 0
        for inner_key in db[key]:
            for inte in db[key][inner_key]:
                call_times += 1
        for key2 in db:
            for inner_key2 in db[key2]:
                if inner_key2 == key:
                    for inte2 in db[key2][inner_key2]:
                        receive_times += 1
        temp_dict[key] = [call_times, receive_times]
    
    name_list = []
    for key in sorted(temp_dict, key = (lambda x: (-temp_dict[x][0], temp_dict[x][1], x))):
