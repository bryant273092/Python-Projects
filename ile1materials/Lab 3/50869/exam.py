...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        for i in range(2,len(list)):
            ts.append(int(list[i]))
            if list[0] not in dictionary:
                dictionary[list[0]]={list[1]:ts}
            else:
                dictionary[list[0]].update({list[1]:ts})
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db:
        for r in db[c]:
            t=sum(db[c][r])
            result.append((r,t))
    return [s[0] for s in sorted(result, key = lambda x:x[1], reverse = True)]
