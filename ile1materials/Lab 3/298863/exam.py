...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for entry in [line.strip().split(":") for line in file]:
        result[entry[0]][entry[1]] = result[entry[0]].get(entry[1], []) + [int(t) for t in entry[2:]]
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db:
        for r, ts in db[c].items():
            temp_data[c]["c"] = len(ts) + temp_data[c].get("c", 0)
            temp_data[r]["r"] = len(ts) + temp_data[r].get("r", 0)
    return [k[0] for k in sorted(temp_data.items(), key=(lambda x : (-x[1]["c"], x[1]["r"], x[0])))]
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    temp_data = defaultdict(dict)
    for c, history in db.items():
        for r, ts in history.items():
            temp_data[c][r] = sum(ts) + temp_data[c].get(r, 0)
            temp_data[r][c] = sum(ts) + temp_data[r].get(c, 0)
    return [[temp_data[c].get(r, 0) for r in sorted(temp_data.keys())] for c in sorted(temp_data.keys())]
