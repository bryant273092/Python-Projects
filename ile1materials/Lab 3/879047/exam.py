...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        caller, reciever, *rest = line.strip().split(':')
        print(caller)
        if caller not in new_dict:
            new_dict[caller][reciever] = []
        new_dict[caller][reciever] = [int(x) for x in rest] 
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for reciever, minutes in db[caller].items():
            for _ in minutes:
                new_dict[caller]+=1
                new_dict[reciever]+=1
    dict(new_dict)
    return [(name, amount) for name, amount in sorted(sorted(new_dict.items(), key = lambda x : x[0]), key = lambda x : x[1], reverse = True)]
    ...
    for caller in db:
        for reciever in db[caller]:
            if caller in db[reciever]:
                #print(caller)
                #print(reciever)
                temp_list = [caller, reciever]
                new_list = sorted(temp_list)
                new_set.add((new_list[0], new_list[1]))
    return new_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    new_dictr = defaultdict(int)
    new_dict = defaultdict(list)
    for caller in db:
        for reciever, minutes in db[caller].items():
            for _ in minutes:
                new_dictc[caller]+=1
                new_dictr[reciever]+=1
    for caller in new_dictc:
        for reciever in new_dictr:
            if reciever == caller:
                new_dict[caller].append((new_dictc[caller], new_dictr[caller]))
    dict(new_dict)
    print(new_dict)            
    #y = [(x,y) for x,y in sorted(new_dictr.items(), key = lambda x : x[1])]
    x = [x for x,y in sorted(sorted(new_dict.items(), key = lambda x : x[1][0][1]), key = lambda x : x[1][0][0], reverse = True)]
    return x       
