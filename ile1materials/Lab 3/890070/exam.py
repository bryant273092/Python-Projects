...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if line_list[1] not in db[line_list[0]]:
            db[line_list[0]][line_list[1]] = list(int(time) for time in line_list[2:])
        else:
            db[line_list[0]][line_list[1]] += list(int(time) for time in line_list[2:])
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for receiver in db[caller]:
            ...
            freq_dict[caller] += count
            freq_dict[receiver] += count
    return sorted(sorted(dict(freq_dict).items()), key = (lambda item: item[1]), reverse = True)
    ...
    for caller in db:
        for receiver in db[caller]:
            if caller in db[receiver]:
                mutual_set.add((caller,receiver))
    return mutual_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db:
        data_dict[caller] += [0,0]
        for receiver in db[caller]:
            data_dict[receiver] += [0,0]
            count = len(db[caller][receiver])
            data_dict[caller][0] += count
            data_dict[receiver][1] += count
    return sorted(sorted(sorted(dict(data_dict).keys()), key = (lambda key: data_dict[key][1])), key = (lambda key: data_dict[key][0]), reverse = True)
