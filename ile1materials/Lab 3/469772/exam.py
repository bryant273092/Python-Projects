...
#         print(caller)
        if caller[0] not in names:
            names.add(caller[0])
        for receiver in caller[1]:
#             print(receiver)
            if receiver[0] not in names:
                names.add(receiver)
    return(names)
...
#         print(line)
        cl = line.strip('\n').split(':')
#         print(cl[1])
        if cl[0] not in db.keys():
            call_log[cl[1]] = [int(time) for time in cl[2:]]
            print(call_log)
            db[cl[0]] = call_log
            print(db)
#         else:
#             call_log[cl[1]] = [int(time) for time in cl[2:]]
#             print(call_log)
#             db.update(cl[0] = call_log)
#             print(db)
#         if cl[1] not in call_log.keys():
#             log = []
#             for time in range(2,len(cl)):
# #                 print(cl[time])
#                 log.append(int(cl[time]))
#             call_log[cl[1]] = log
#         else:
#             log = []
#             for time in range(2,len(cl)):
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.items():
        print(caller)
        print('caller', caller[0])
        person = []
        n = 0
        if caller[0] not in talkers:
            for receiver in caller[1]:
                if receiver not in person:
                    person.append(receiver)
#             for p in person:
#                 if p 
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    call_buddies = set()
    for calls in db.items():
        if calls[0] and calls[1][0] not in call_buddies:
            call_buddies.add((calls[0],calls[1]))
    return call_buddies
