...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for item in line_list[2:]:
            calldict[line_list[1]].append(int(item))
            db_dict[dbkey].update(dict(calldict))
    return dict(db_dict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result=[]# list of two tuples
    d=defaultdict(int)
    for caller in db.keys():
        for receiver in db[caller]:
            if caller in db[receiver]:
                d[receiver]+=(len(db[receiver][caller])+len(db[caller][receiver]))
        #print(d)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    result=set()
    d=defaultdict(str) # set of 2 tuples
    for caller in db.keys():
        for receiver in db[caller]:
