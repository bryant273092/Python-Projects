...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    name_set = set()
    for callers, receivers in db.items():
        name_set.add(callers)
        for r in receivers:
            name_set.add(r)
    return name_set
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if caller in line:
            ...
            inner_dict[receiver] = call_times
        
            call_dict[caller] = dict(inner_dict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    list_of_tuples = []
    for receivers, callers in db.items():
        call_numbers = 0
        for c in callers.values():
            call_numbers += len(c)
            
        if receivers in callers.keys():
            call_numbers += len(callers[receivers])
            
        list_of_tuples.append((receivers, call_numbers))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for receivers, callers in db.items():
        for c in callers:
            if c in db.keys() and receivers in db[c]:
                mutual.add(tuple(sorted((receivers, c))))
    return mutual
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for names in db:
        ...
        #name_list.sort(key = lambda x: len(db[x][y] for y in x)) 
