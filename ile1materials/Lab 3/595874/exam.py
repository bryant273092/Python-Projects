...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    all_calls = set()
    for caller in db:
        all_calls.add(caller)
        for receiver in db[caller]:
            all_calls.add(receiver)
    return all_calls


def read_db(file : open) -> {str:{str:[int]}}:
    call_db = defaultdict(dict)
    for line in file:
        call_list = []
        line_list = line.strip().split(':')
        caller = line_list[0] 
        reciever = line_list[1]
        for time in line_list[2:]:
            call_list.append(int(time))
        call_db[caller][reciever] = call_list
    return dict(call_db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for receiver in db[caller]:
            number_of_calls[caller] += len(db[caller][receiver])
            number_of_calls[receiver] += len(db[caller][receiver])
    return sorted(sorted([(key, number_of_calls[key]) for key in number_of_calls]), reverse = True, key = lambda item: item[1])
    ...
    for caller in db:
        for receiver in db[caller]:
            if caller in db[receiver]:
                match = tuple(sorted(tuple((caller, receiver))))
                mutual_set.add(match)
    return mutual_set
            


def initiators (db : {str:{str:[int]}}) -> [str]:
    is_receiver = defaultdict(int)
    is_caller = defaultdict(int)
    for caller in db:
        is_receiver[caller] = len(db[caller])
    for caller in db:
        for receiver in db[caller]:
            is_caller[caller] += len(db[caller][receiver])
    return sorted([call for call in sorted(sorted(is_receiver), key = lambda item: is_receiver[item])], reverse = True, key = lambda item:is_caller[item])
