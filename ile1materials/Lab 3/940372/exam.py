...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        caller = line_list[0]
        receiver = line_list[1]
        d[caller][receiver] = ([int(time) for time in line_list[2:]])
    return dict(d)
            
#INCORRECT
#      was       = {'Alan': {'Barb': [10, 20], 'Carl': [10, 10]}, 'Barb': {'Alan': [10, 10], 'Deja': [5, 5, 5]}, 'Carl': {'Alan': [5], 'Deja': [15], 'Elan': [5, 5]}, 'Deja': {'Elan': [5]}, 'Elan': {'Carl': [10]}}
#      should be = {'Alan': {'Barb': [10, 20], 'Carl': [10, 10]}, 'Barb': {'Alan': [10, 10], 'Deja': [5, 5, 5]}, 'Carl': {'Alan': [10, 5], 'Deja': [15], 'Elan': [5, 5]}, 'Deja': {'Elan': [5]}, 'Elan': {'Carl': [10]}}
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        summ = 0
        for receiver in db[caller].keys():
            summ += len(db[caller][receiver])
            p2 = [receiver]*len(db[caller][receiver])
            people.extend(p2)
        p1 = [caller]*summ
        people.extend(p1)
    for person in set(people):
        number = people.count(person)
        result.append((person, number))
    return sorted(sorted(result, key = lambda x: x[0]), key = lambda y: y[1], reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    d = defaultdict(list)
    for name in all_parties(db):
        d[name] = [caller_counter(db,name), receiver_counter(db, name)]
        
    result = sorted(d.keys())
    return sorted(sorted(result, key = lambda x: d[x][1]), key = lambda x: d[x][0], reverse = True)


def caller_counter(db : {str:{str:[int]}}, caller: str) -> int:
    caller_sum = 0
    for receiver in db[caller].keys():
        caller_sum += len(db[caller][receiver])
    return caller_sum

def receiver_counter(db : {str:{str:[int]}}, receiver: str) -> int:
    receiver_sum = 0
    for caller in db.keys():
        if receiver in db[caller]:
            receiver_sum += len(db[caller][receiver])
    return receiver_sum
