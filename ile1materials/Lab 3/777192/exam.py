...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    dic = defaultdict(list)
    for c in db.keys():
        for t_list in db[c].values():
            dic[c] += t_list
        # here I try to count the receiver
        # for r in dic[c]:
            # dic[r] += dic[c][r]
        result.append(tuple([c,len(dic[c])]))
    return result
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db:
        for r in db[c]:
            if len(db[c][r]) >= 1 and len(db[r].get(c,[])) >= 1:
                pair = [c,r]
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db.keys():
        for t_list in db[c].values():
            dic[c] += t_list
        dic[c].append(len(dic[c]))
        # here I try to count the receiver 
        # for r in dic[c]:
           # dic[r] += dic[c][r]
           # dic[r].append(len(dic[r]))'''
