...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller in db.keys():
        result_set.add(caller)
        for receiver in db[caller].keys():
            result_set.add(receiver)
    return result_set
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        caller = info_list[0]
        receiver = info_list[1]
        time_list = info_list[2:]
        if receiver not in result_dict[caller].keys():
            inner_dict = defaultdict(list)
        else:
            inner_dict = result_dict[caller]
        for time in time_list:
            inner_dict[receiver].append(int(time))
        result_dict[caller].update(inner_dict)
    return result_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        for receiver in db[caller].keys():
            result_dict[caller] += len(db[caller][receiver])
            result_dict[receiver] += len(db[caller][receiver])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        for receiver in db[caller].keys():
            if (len(db[caller][receiver])) >= 1 and (caller in db[receiver].keys()) and (len(db[receiver][caller])>=1) and ((caller,receiver) not in result_set) and ((receiver,caller) not in result_set):
                result_set.add(tuple(sorted([caller,receiver])))
    return result_set



def initiators (db : {str:{str:[int]}}) -> [str]:
    caller_dict = defaultdict(int)
    receiver_dict = defaultdict(int)
    name_set = set()
    for caller in db.keys():
        name_set.add(caller)
        for receiver in db[caller].keys():
            name_set.add(receiver)
            caller_dict[caller] += len(db[caller][receiver])
            receiver_dict[receiver] += len(db[caller][receiver])
    name_list = list(name_set)
    name_list.sort(key = lambda x: (-caller_dict[x],receiver_dict[x],x))
    return name_list
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for caller in db.keys():
        for receiver in db[caller].keys():
            for time in db[caller][receiver]:
                if ((caller,receiver) in total_time_dict.keys()):
                    total_time_dict[(caller,receiver)] += time
                elif ((receiver,caller) in total_time_dict.keys()):
                    total_time_dict[(receiver,caller)] += time
                else:
                    total_time_dict[(caller,receiver)] += time
    index_dict = {}
    for index, name in enumerate(name_list,0):
    ...
    for i in range(len(name_list)):
        ...
        for j in range(len(name_list)):
            if (index_dict[i] == index_dict[j]):
                inner_list.append(0)
            elif (index_dict[i],index_dict[j]) in total_time_dict.keys():
                inner_list.append(total_time_dict.get((index_dict[i],index_dict[j]),0))
            elif (index_dict[j],index_dict[i]) in total_time_dict.keys():
                inner_list.append(total_time_dict.get((index_dict[j],index_dict[i]),0))
            else:
                inner_list.append(0)
