...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        receiver = line[1]
        num = [int(x) for x in line[2:]]
        if line[0] not in result.keys():
            result[line[0]] = defaultdict(list)
        for x in num:
            result[line[0]][receiver].append(x)
    file.close()
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        for receiver in db[caller]:
            ...
            temp[receiver] +=  len(db[caller][receiver])
            temp[caller] += len(db[caller][receiver])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        for receiver in db[caller]:
            if caller in db[receiver].keys()and caller < receiver:
                temp[caller].append(receiver)
    for key in temp.keys():
        for x in temp[key]:
            result.add((key, x))
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db.keys():
        ...
        y = int()
        for receiver in db[caller]:
            x += len(db[caller][receiver])
        for call in db.keys():
            if call == caller:
                pass
            else:
                if caller in db[call].keys():
                    y += len(db[call][caller])
        temp[caller] = (x, y)
    for item in temp.items():
        temp2.append((item[0], item[1][0], item[1][1]))
    temp2 = sorted(temp2, key = lambda x: (-x[1], x[2], x[0]))
    for x in temp2:
        result.append(x[0])
    return result
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    x = sorted(list(all_parties(db)))
    for name in x:
        for y in x:
            temp[name][y] = dict()
    for caller in db.keys():
        temp[caller][caller] = 0
        for receiver in db[caller]:
            temp[caller][receiver] = 0
            for num in db[caller][receiver]:
                temp[caller][receiver] += num
    for caller in temp.keys():
        t = []
        for receiver in temp[caller]:
            x = 0
            if type(temp[caller][receiver]) is int: 
                x += temp[caller][receiver]
            if type(temp[receiver][caller]) is int:
                x += temp[receiver][caller]
            t.append(x)
        result.append(t)
    return result
