...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        result[text[0]].setdefault(text[1],[])
        result[text[0]][text[1]] += [int(i) for i in text[2:]]
    file.close()
    return dict(result)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for i in db.keys():
        for k in db[i].keys():
            ...
            result[i] += count
            result[k] += count
    output = list(zip(result.keys(),result.values()))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for i in db.keys():
        for k in db[i].keys():
            if k in db.keys() and i in db[k].keys() and tuple(sorted([i,k])) not in result:
                result.add(tuple(sorted([i,k])))
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for i in db.keys():
        for k in db[i].keys():
            ...
            temp[i][0] += count
            temp[k][1] += count
    result = sorted(temp.items(),key=lambda x:(-x[1][0],x[1][1],x[0]))
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for i in db.keys():
        ...
        for k in db[i].keys():
            temp[k][k] = 0
            time = sum(db[i][k])
            temp[i].setdefault(k,0)
            temp[k].setdefault(i,0)
            temp[i][k] += time
            temp[k][i] += time
    names = sorted(list(all_parties(db)))
    result = []
    for n in names:
        for x in names:
            if n in temp.keys() and x in temp[n].keys():
                result.append(temp[n][x])
            else:
                result.append(0)
    return [list(i) for i in zip(*([iter(result)]*len(names)))]
