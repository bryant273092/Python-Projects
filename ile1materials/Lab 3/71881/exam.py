...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    empty_lst = []
    for c, r in db.items() :
        for receiver, t in r.items() :
            empty_set.add(c)
            empty_set.add(receiver)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for lines in file :
        ...
        for i in range(len(elements)) :
            if elements[i] in empty_lst :
                db[elements[0]][elements[1]] = [int(elements[2])] #talk_times  #= [int(elements[2:-1])] #[int(talk_times)] #= [(int(talk_times))]
            else :
                x = {}
                x[elements[1]] = [int(elements[2])] #talk_times  #[int(elements[2:-1])] #+= [int(talk_times)] #= [(int(talk_times))] 
                db[elements[i]] = x
            empty_lst.append(elements[i])
    return db
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db.values() :
        for r in c : # if caller in d & receiver in d -> count ++ 
            if r in d and r == matching :
                count += 1
                d[r] = count
                #d[receiver] += caller[receiver]
            else :
                count = 0 
                d[r] = c[r]
                
    return sorted(sorted([(item, d[item]) for item in d])) #, key = (lambda t : (-t[1], t[0])))


def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    empty_set = defaultdict(set)
    for c in db :
        for r in db[c] :
            if c in r and c == r :
                empty_set.add((c, r))
        #if caller in db[caller].keys() :
...
#4
    for caller, receiver in db.items() :
        ...
        for receiver in db[caller] :
            ...
            if caller == receiver and receiver == caller:
                print('true')
                empty_set.add((caller, receiver))
            else :
                print('false')
