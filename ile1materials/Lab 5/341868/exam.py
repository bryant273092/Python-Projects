...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        print(line)
        call_history = line.split(':')
        for val in call_history:
            val = val.strip('\n')
        while len(call_history) < 2:
            numbers = []
            x = int(call_history.pop())
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for person in db.keys():
        x = db[person].items()
        for person,times in x:
            value = len(times)
            if person in final.keys():
                temp = final[person]
                final[person] = temp+value
            else:
                final[person] = value
    result = []
    for x in final.keys():
        result.append((x,final[x]))
