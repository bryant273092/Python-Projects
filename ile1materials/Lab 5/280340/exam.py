...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    myset = set()
    for caller, call_history in db.items():
        myset.add(caller)
        for receiver,time_of_call in call_history.items():
            myset.add(receiver)
    return myset
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        inner_dict = []
        caller = line.rstrip().split(':')[0] 
        receiver = line.rstrip().split(':')[1]
        call_list = line.rstrip().split(':')[2:]
        int_call_list = []
        for i in call_list:
            int_call_list.append(int(i))
        #inner_dict[receiver].extend(int_call_list)
        #print(inner_dict)
        mydict[caller][receiver] = int_call_list
    return mydict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller, call_history in db.items():
        ...
        looking = caller
        for receiver,time_of_call in call_history.items():
            mydict[caller]+=len(time_of_call)
            for caller, call_history in db.items():
                if looking in call_history:
                     mydict[looking] += len(db[caller][looking])
            if receiver == looking:
                mydict[receiver]+=len(time_of_call)
    return sorted(list(mydict.items()), key = (lambda x:(-x[1], x[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller, call_history in db.items():
       for receiver,time_of_call in call_history.items():
           if len(db[caller][receiver]) > 0:
               if caller in db[receiver]:
                   if len(db[receiver][caller]) > 0:
                       myset.add(tuple(sorted(list((caller, receiver)))))
    return myset
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    str_result = []
    result = []
    #callers_dict = defaultdict(int)
    for caller, call_history in db.items():
        caller_calls = 0
        receiver_calls = 0
        for receiver,time_of_call in call_history.items():
            caller_calls +=len(time_of_call)
            for x in db:
                if caller in db[x]:
                    receiver_calls+= len(db[x][caller])
        result.append((caller, caller_calls, receiver_calls))
    new_result = sorted(result, key = (lambda x: (-x[1], x[2], x[0])))
    
    print(new_result)
    
    for name, caller, receiver in new_result:
        str_result.append(name)
    return str_result
