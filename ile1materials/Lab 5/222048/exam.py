...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in line_list:
        ...
        int_list = []
        for each_num in word_list[2:]:
            int_list.append(int(each_num))
        if word_list[1] not in r_dict[word_list[0]]:
            r_dict[word_list[0]].update({word_list[1]: int_list})
        else:
            r_dict[word_list[0]][word_list[1]].extend(int_list)
    return dict(r_dict)
        ...
        for k in db:
            if k == n:
                ...
                for k2 in db[k]:
                    if k2 == n:
                        count += len(db[k][k2])
        r_list.append((n, count))
    return sorted(r_list, key=lambda x:(-x[1],x[0]))
    ...
    for k in db:
        for k2 in db[k]:
            if k2 in db:
                if k in db[k2]:
                    r_list.append({k, k2})
    for each_set in r_list:
        if each_set not in r_list2:
            r_list2.append(each_set)
    for each_set in r_list2:
        r_set.add(tuple(sorted(tuple(each_set))))
    return r_set
    ...
    for n in name_list:
        ...
        for k in db:
            ...
            for k2 in db[k]:
                if k2 == n:
                    rec += len(db[k][k2])
        r_list2.append( (n,(call,rec)) )
    r_list3 = sorted(r_list2, key=lambda x:(-x[1][0], x[1][1], x[0]))
    for each_tuple in r_list3:
        r_list.append(each_tuple[0])
    return r_list
