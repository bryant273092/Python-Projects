...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        name = line[0]
        line = line[1:]
        d[name][line[0]] = [int(i) for i in line[1:]]
    return dict(d)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for i, j in db.items():
        if k in db[i]:
            d[k] += len(db[i][k])
    return sorted(sorted(d.items()), key = lambda x: x[1], reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k, v in db.items():
        for item in v:
            if item in db[k] and k in db[item]:
                if k[0] < item[0]:
                    s.append((k, item))
                else:
                    s.append((item, k))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    return [k for k, v in sorted(sorted(db.items()), key = lambda x: (-len(x[1])))]
