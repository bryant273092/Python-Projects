...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in lines:
        times = [int(x) for x in line[2:]]
        try:
            db[line[0]][line[1]].extend(times)
        except KeyError:
            db[line[0]][line[1]] = times
    return dict(db)


#ex = read_db(open('db1.txt'))
# print(ex)

def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = []
    called = lambda x: sum([len(l) for l in db[x].values()])
    recieved = lambda name: sum([l[0] for l in [[len(v) for k,v in db[key].items() if k == name] for key in db.keys()] if len(l)!=0])
    for k in db.keys():
        result.append((k, called(k)+ recieved(k)))
    ...
    for caller in db.keys():
        for reciever in db[caller].keys():
            if caller in db[reciever].keys():
                result.add(tuple(sorted([caller,reciever])))
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    called = lambda x: sum([len(l) for l in db[x].values()])
    recieved = lambda name: sum([l[0] for l in [[len(v) for k,v in db[key].items() if k == name] for key in db.keys()] if len(l)!=0])
    return (sorted(sorted(sorted(db.keys()), key=recieved), key=called, reverse=True))
