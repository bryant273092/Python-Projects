...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    caller_reciever_set = set()
    for caller in db.keys():
        caller_reciever_set.add(caller)
        for recievers in db[caller].keys():
            caller_reciever_set.add(recievers)
    return caller_reciever_set
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        ...
        for reciever in db[caller].keys():
            count += len(db[caller][reciever])
        for i in db.keys():
            if caller in db[i].keys():
                count += len(db[i][caller])
        times_in_call.append((caller,count))
    return sorted(sorted(times_in_call),key = lambda x: x[1], reverse = True)            
        



def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    mutual_caller_set = set()
    for caller in db.keys():
        for reciever in db[caller].keys():
            if caller in db[reciever].keys() and reciever in db[caller].keys():
                if (reciever,caller) in mutual_caller_set:
                    pass
                else:
                    mutual_caller_set.add((caller,reciever))
    return mutual_caller_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for callers in db.keys():
        ...
        for recievers in db[callers].keys():
            times_initated += len(db[callers][recievers])
        times_as_initiator.append((callers, times_initated))
    return sorted(sorted(times_as_initiator), key = lambda x: x[1], reverse = True)
