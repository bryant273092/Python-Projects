...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    return_set = set()
    for caller, inner_dict in db.items():
        return_set.add(caller)
        for receiver, times_list in inner_dict.items():
            return_set.add(receiver)
    return return_set
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if new_list[0] not in return_dict.keys():
            return_dict[new_list[0]] = {new_list[1]: int_list}
        else:
            if new_list[1] in return_dict[new_list[0]].keys():
                return_dict[new_list[0]][new_list[1]].extend(int_list)
            else:
                return_dict[new_list[0]][new_list[1]] = int_list
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller, inner_dict in db.items(): 
        caller_time = 0
        for receiver, times_list in inner_dict.items():
            caller_time +=  len(times_list)
            if receiver not in return_dict.keys():
                return_dict[receiver] = len(times_list)
            else:
                original_rtime = return_dict[receiver]
                return_dict[receiver] = original_rtime + len(times_list)
        if caller not in return_dict.keys():
            return_dict[caller] = caller_time
        else:
            original_ctime = return_dict[caller]
            return_dict[caller] = original_ctime + caller_time
            
    return_list = []
    for key, item in return_dict.items():
        return_list.append((key, item))    
    
    return sorted(sorted(return_list, key = lambda x: x[0]), key = lambda x: -x[1])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller, inner_dict in db.items(): 
        for receiver, times_list in inner_dict.items():
            return_dict[caller].append(receiver)
            if receiver in return_dict.keys():
                if caller in return_dict[receiver]:
                    if caller > receiver:
                        return_set.add((receiver, caller))
                    else:
                        return_set.add((caller, receiver))
    return return_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller, inner_dict in db.items(): 
        ...
        for receiver, times_list in inner_dict.items():
            caller_time +=  len(times_list)
            if "receiver_times" not in return_dict[receiver].keys():
                return_dict[receiver]["receiver_times"] = len(times_list)
            else:
                orginal_time = return_dict[receiver]["receiver_times"]
                return_dict[receiver]["receiver_times"] = orginal_time + len(times_list)
        return_dict[caller]["caller_times"] = caller_time
    
    
        
    return [x[0] for x in sorted(sorted(sorted(return_dict.items()), key = lambda x: x[1]['receiver_times']), key = lambda x: -x[1]["caller_times"] )]


def matrix (db : {str:{str:[int]}}) -> [[int]]:
    """
    return_list = []
    default_set = all_parties(db)
    name_list = sorted(list(default_set))
