...
def read_db(file : open) -> {str:{str:[int]}}:
    db = defaultdict(dict)
    for line in file:
        caller, reciever, *times = line.rstrip().split(':')
        if reciever not in db[caller].keys():
            db[caller][reciever] = [int(x) for x in times]
        else:
            db[caller][reciever] += [int(x) for x in times]
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        caller_freq = 0
        for call in db[caller].values():
            caller_freq += len(call)
        for other_callers in (db.keys() - caller):
            if caller in db[other_callers]:
                caller_freq += len(db[other_callers][caller])
        talkers_list.append((caller, caller_freq))
    return sorted(talkers_list, key = lambda t: (-t[1], t[0]))


def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    mutual_callers_set = set()
    for caller in db.keys():
        reciever_list = sorted(list(db[caller].keys()), key = lambda t: sum(db[caller][t]))
        for item in reciever_list:
            if caller in db[item].keys():
                mutual_callers_set.add(tuple(sorted([caller, item])))
                break
    return mutual_callers_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db.keys():
        ...
        for other_callers in (db.keys() - caller):
            if caller in db[other_callers]:
                recieve_freq += len(db[other_callers][caller])
        caller_list.append((caller, caller_freq, recieve_freq))
    return [x[0] for x in sorted(caller_list, key= lambda t: (-t[1], t[2]))]
