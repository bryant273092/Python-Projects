...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    call = set()
    l = []
    for callers, call_r in db.items():
        for caller in call_r.keys():
            call.add(callers)
            call.add(caller)
    return call
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for information in file:
        new_info = information.strip('\n').split(':')
        caller, call_r =  new_info[0],new_info[1]
        new_time = new_info[2:]
        new_d[caller][call_r] = new_time
    return dict(new_d)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    m_c = set()
    for caller, call_r in db.items():
       for person in call_r.keys():
           if caller in db[person].keys():
              a = tuple(sorted((person,caller)))
              m_c.add(a)
    return m_c
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    l = []
    for caller, call_r in db.items():
       l.append((caller,len(call_r.keys())))
    return l
