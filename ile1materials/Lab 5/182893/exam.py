...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in text:
        info = line.split(":")
        person_calling = info[0]
        person_caller = info[1]
        time = [int(time) for time in info[2:]]
        if person_caller in result[person_calling]:
            for item in time:
                result[person_calling][person_caller].append(item)
        else:
            result[person_calling][person_caller] = time
    return dict(result)
    ...
    for c in db:
        for r in db[c]:
            for _ in db[c][r]:
                result[c] += 1
                result[r] += 1
    return sorted([(p, result[p]) for p in result], key = (lambda tup: (-1 * tup[1], tup[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db:
        for r in db[c]:
            if c in db.get(r) and r in db.get(c):
                step = tuple(sorted([c, r]))
                result.add(step)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    data = {c:[0, 0] for c in db}
    for c in db:
        for r in db[c]:
            for _ in db[c][r]:
                data[c][0] += 1
                data[r][1] += 1
    return [tup[0] for tup in sorted([(c, data[c][0], data[c][1]) for c in db], key = (lambda tup: (-1 * tup[1], tup[2], tup[0])))]
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    half_result = []
    for c_tup in enumerate(people):
        index, c = c_tup
        step = [0 for _ in range(index + 1)]
        for r_tup in enumerate(people[index + 1:]):
            _, r = r_tup
            if db[c].get(r, None) is None:
                step.append(0)
            else:
                total_time = 0
                for time in db[c][r]:
                    total_time += time
                if db[r].get(c, None) is not None:
                    for time in db[r][c]:
                        total_time += time
                step.append(total_time)
                            
        half_result.append(step)

        
    for i in range(len(half_result)):
        for j in range(len(half_result)):
            if half_result[i][j] != 0:
                result[i][j] = half_result[i][j]
                result[j][i] = half_result[i][j]
