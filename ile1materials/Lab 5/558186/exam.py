...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        c, r, ts = c_info[0], c_info[1], [int(t) for t in c_info[2:]]
        result[c].setdefault(r, [])
        result[c][r] += ts
    return dict(result)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db:
        for r in db[c]:
            if r in db and c in db[r]:
                result.append((c, r))
    result2 = set()
    for (a, b) in sorted(result):
        if (b, a) not in result2:
            result2.add((a, b))
    return result2           
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    def initiator_sort(person) -> (int, int, str):
        c_times, r_times = 0, 0
        for caller in db:
            for receiver in db[caller]:
                if person == receiver:
                    r_times += len(db[caller][receiver])
                if person == caller:
                    c_times += len(db[caller][receiver])
        return (-c_times, r_times, person)
    return sorted(db.keys(), key=initiator_sort)



def matrix (db : {str:{str:[int]}}) -> [[int]]:
    result = []
    for _ in sorted(db.keys()):
        call_times = []
        for _ in db:
            call_times.append(0)
        result.append(call_times)
    
    callers = sorted(db.keys())
    for i in range(len(callers)):
        for j in range(len(callers)):
            call_time = 0
            if callers[j] in db[callers[i]]:
                for time in db[callers[i]][callers[j]]:
                    call_time += time
            if callers[i] in db[callers[j]]:
                for time2 in db[callers[j]][callers[i]]:
                    call_time += time2
            result[i][j] = call_time
    return result
