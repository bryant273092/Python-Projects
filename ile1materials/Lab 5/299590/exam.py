...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in rd:
        ...
        if l[0] not in rdict:
            rdict[l[0]] = dict()
            if l[1] not in rdict[l[0]]:
                rdict[l[0]][l[1]] = list()
                for i in range(2, len(l)):
                    rdict[l[0]][l[1]].append(int(l[i]))
            else:
                for i in range(2, len(l)):
                    rdict[l[0]][l[1]].append(int(l[i]))
        else:
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    fdict = dict()
    for caller in db:
        if caller not in fdict:
            fdict[caller] = 0
            for rec in db[caller]:
                if rec not in fdict:
                    fdict[rec] = 0
                    fdict[caller] += len(db[caller][rec])
                    fdict[rec] += len(db[caller][rec])
                else:
                    fdict[caller] += len(db[caller][rec])
                    fdict[rec] += len(db[caller][rec])
        else:
            for rec in db[caller]:
                if rec not in fdict:
                    ...
                    fdict[rec] += len(db[caller][rec])
    #print(fdict)
    return sorted(sorted(fdict.items(), key=(lambda x:x[0])), key=(lambda x:x[1]), reverse=True)
        ...
        if caller not in idict:
            ...
            for rec in db[caller]:
                if rec not in idict:
                    ...
                    idict[rec]['c'] = 0
                    idict[rec]['r'] = 0
                    idict[caller]['c'] += len(db[caller][rec])
                    idict[rec]['r'] += len(db[caller][rec])
                else:
                    idict[caller]['c'] += len(db[caller][rec])
                    idict[rec]['r'] += len(db[caller][rec])
        else:
            for rec in db[caller]:
                if rec not in idict:
                    ...
                    idict[rec]['r'] += len(db[caller][rec])
    #print(idict)
    return sorted(sorted(sorted(idict.keys(), key=(lambda x:x)), key=(lambda x: idict[x]['r'])), key=(lambda x:idict[x]['c']), reverse=True)
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    nlist = list(alset)
    #print(nlist)
    nlist = sorted(nlist) # create a sorted name list
    mlist = list() # construct zeros matrix
    for i in range(0, len(nlist)):
        il = list((0 for j in range(0, len(nlist))))
        mlist.append(il)
    #print(mlist)
    for i in range(0, len(nlist)):
        for j in range(0, len(nlist)):
            if nlist[j] in db[nlist[i]]:
                for k in db[nlist[i]][nlist[j]]:
                    mlist[i][j] += k
                    mlist[j][i] += k
