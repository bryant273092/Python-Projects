...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    L={}
    for number in db.items():
        N=0
        for i in number[1].keys():
            N=N+len(number[1][i])
            if i not in L.keys():
                L[i]=len(number[1][i])
            if i in L.keys():
                L[i]=L[i]+len(number[1][i])
        if number[0] in L.keys():
            L[number[0]]=L[number[0]]+N
        if number[0] not in L.keys():
            L[number[0]]=N
    P=[]
    for everything in L.keys():
        P.append((everything,L[everything]))
    return sorted(set(P))
