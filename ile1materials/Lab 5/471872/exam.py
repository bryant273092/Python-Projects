...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    print(freq)
    freq = sorted(freq, key= lambda t: (-t[1], t[0]))
    return freq
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for p, calls in vals.items():
        ...
        for c in calls:
            ...
            if c is 'r':
                receiver += 1
        call_nums.append((p, caller, receiver ))
           
    call_nums = sorted(call_nums, key = lambda t: (-t[1], t[2], t[0]))
    
    for c in call_nums:
        initiator.append(c[0])
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    db1 = dict(sorted(db.items()))
    print(db1)
    for k in db1.keys():
        db1[k] = dict(sorted(db1[k].items()))
