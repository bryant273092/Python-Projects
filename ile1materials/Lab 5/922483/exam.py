...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        try: type(d[l_list[0]][l_list[1]])
        except: d[l_list[0]][l_list[1]] = []
        d[l_list[0]][l_list[1]].extend([int(i) for i in l_list[2:]])
    return dict(d)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k,v in db.items():
        for i in v.keys():
            d[(k,i)]+=len(v[i])
    mutuals=[]
    for caller_tuple in d.keys():
        if d[caller_tuple] > 0 and (caller_tuple[1],caller_tuple[0]) in d.keys():
            if d[(caller_tuple[1],caller_tuple[0])] > 0: 
                mutuals.append(sorted([caller_tuple,(caller_tuple[1],caller_tuple[0])], key=lambda x: x[0])[0])
    return set(sorted(mutuals))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k,v in db.items():
        for i in v.keys():
            callers[k]+=len(v[i])
            receivers[i]+=len(v[i])
    return sorted(callers.keys(), key=lambda x: (-callers[x],receivers[x]))
