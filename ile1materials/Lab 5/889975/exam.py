...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        caller = line.pop(0)
        reciever = line.pop(0)
        toInt = [int(x) for x in line]
        db[caller].update({reciever:toInt})
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        ...
        for rec in db[caller]:
            times += len(db[caller][rec])
        result[caller] = times
        
    for caller in db.keys():
        for rec in db[caller]:
            result[rec] += int(len(db[caller][rec]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    result = set()
    for caller in db.keys():
        for caller2 in db[caller].keys():
            if caller and caller2 not in result:
                pass 
    return result      
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    calltimes = {}
    rectimes = defaultdict(int)
    comparator = []
    result = []
    for caller in db.keys():
        calling = int()
        for rec in db[caller]:
            calling += len(db[caller][rec])
        calltimes[caller] = calling
    for calls in db.values():
        for rec in calls:
            rectimes[rec] += len(calls[rec])
    for caller in db:
        comparator.append((caller, calltimes[caller], rectimes[caller]))
        
    result = sorted(comparator, key = lambda t : -t[1])
