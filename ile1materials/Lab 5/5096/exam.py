...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for item in file:
        ...
        if main[0] in ret.keys():
            if main[1] in ret[main[0]].keys():
                ret[main[0]][main[1]].extend(new_time)
            else:
                ret[main[0]].update({main[1]:new_time})
        else:
            ret.setdefault(main[0], dict()).update({main[1]: new_time})
    return ret
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for item in db:
        ...
        for i in db[item]:
            time = time + len(db[item][i])
            
            if i in call:
                call[i]= call[i]+len(db[item][i])
            else:
                call.setdefault(i, int())+len(db[item][i])   
        if item in call.keys():
            call[item]=call[item]+time
        else:
            call.setdefault(item, int())+time
            
    ret_list = dict(call).items()
    ret_list = sorted(ret_list, key = lambda x: (-x[1], x[0]))
    return ret_list        
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for item in db:
        for i in db[item]:
            try:
                if len(db[item][i])>=1 and len(db[i][item])>=1:
                    ret.append((item, i))
            except:pass
    for item in ret:
        for i in ret:
            if item[0] == i[1] and item[1] == i[0]:
                ret.pop(ret.index(i))
    new_ret = []
    for item in ret:
        new_ret.append(tuple(sorted(item)))
    return set(sorted(new_ret, key= lambda x: (x[0], x[1])))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    return sorted(ret, key= lambda x: (-caller(x), receiver(x), name(x)))
