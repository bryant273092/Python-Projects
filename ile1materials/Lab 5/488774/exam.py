...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in lines:
        ...
        for value in range(2, len(split_line)):
            call_times.append(int(split_line[value].rstrip()))
        if split_line[0] not in result:
            result[split_line[0]] = {split_line[1]: call_times} 
        else:
            if split_line[1] not in result[split_line[0]]:
                result[split_line[0]].update({split_line[1]: call_times})
            else:
                result[split_line[0]][split_line[1]] = result[split_line[0]][split_line[1]] + call_times
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        calls = 0
        for receiver in db[caller]:
            calls += len(db[caller][receiver])
        result_dict[caller] += calls 
        for receiver in db[caller]:
            calls = 0 
            calls += len(db[caller][receiver])
            result_dict[receiver] += calls 
    for key in result_dict:
        result.append((key, result_dict[key]))
    ...
    for caller in db: 
        for receiver in db[caller]:
            if caller in db[receiver]:
                mutual_list.append(sorted((caller, receiver), key = lambda x: x[0]))
    for pair in mutual_list:
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db:
        made_calls = 0
        for receiver in db[caller]:
            made_calls += len(db[caller][receiver])
        received_calls = 0 
        for caller2 in db:
            if caller in db[caller2]:
                received_calls += len(db[caller2][caller])
        info_list.append((caller, made_calls, received_calls))
    for name in sorted(sorted(sorted(info_list, key = lambda x: x[0]), key = lambda x: x[2]), key = lambda x: x[1], reverse = True):
