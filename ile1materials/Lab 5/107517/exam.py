...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for c in db:
        ret_set.add(c)
        for r in db[c]:
            ret_set.add(r) #only adds if r is not a caller
    return ret_set
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        c, r, times = line_list[0], line_list[1], line_list[2:]
        for time in times:
            time = int(time)
            ret_dict[c].setdefault(r, []).append(time)
    return dict(ret_dict)



def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    talk_freq_dict = defaultdict(int)
    for c in db:
        for r, ts in db[c].items():
            for _ in range(len(ts)):
                talk_freq_dict[c] += 1
                talk_freq_dict[r] += 1
    return sorted(talk_freq_dict.items(), key = lambda x: (-x[1], x[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ret_set = set()
    for c in db:
        for r, _ in db[c].items():
            if (r in db) and (c in db[r]):
                list_to_sort = [c, r]
                ret_set.add(tuple(sorted(list_to_sort)))
    return ret_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db:
        for r, ts, in db[c].items():
            for _ in range(len(ts)):
                c_r_dict.setdefault(c, [0,0])[called] += 1
                c_r_dict.setdefault(r, [0,0])[recieved] += 1
    print(c_r_dict.items())
    return sorted(c_r_dict, key = lambda x: (-c_r_dict[x][called], c_r_dict[x][recieved], x))
            

def matrix (db : {str:{str:[int]}}) -> [[int]]:
    '''
    call_time_dict = {}
    for outer_person in all_parties(db):
        for inner_person in all_parties(db):
            call_time_dict[outer_person][inner_person] = 0
    for c, (r, ts) in db.items():
        call_time_dict[c] += sum(ts)
        call_time_dict[r] += sum(ts)
    ret_list = []
    for out_pers, in_pers in sorted(call_time_dict.items()):
        if out_person i
        ret_list.append([call_time_dict[out_pers][in_pers]])
    '''
    pass
