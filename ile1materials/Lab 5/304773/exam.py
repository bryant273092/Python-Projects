...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    result = set()
    for call in db.items():
        result.add(call[0])
        for r in call[1].items():
            result.add(r[0])
    return result
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        result[line[0]] = defaultdict(dict)
    
       # caller = result[line[0]]
        #print(caller)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = {}
    for pair in db.items(): #create list of all people involved
        result[pair[0]] = 0
    for pair in db.items():
        count = 0
        for call in pair[1].items():
            count = len(call[1])
            result[pair[0]] = result[pair[0]] + count
            result[call[0]] = result[call[0]] + count
    final = []
    for counts in result.items():
        final.append((counts[0],counts[1]))
    #return sorted(final, key = lambda x: x[1])
    return sorted(sorted(final, key = lambda x: x[0]),key = lambda x:x[1], reverse = True)

def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    result = set()
    for pair in db.items():
        caller = pair[0]
        for receiver in pair[1]:
            if caller in db[receiver]:
                first = min(caller,receiver)
                second = max(caller,receiver)
                result.add((first,second))   
    return result

def initiators (db : {str:{str:[int]}}) -> [str]:
    people = {} #Temporary data structure
    for caller in db.items():
        people[caller[0]] = [0,0] #where index 0 is caller and index 2 is receiving
        
    for person in db.items():
        call_count = 0
        caller = person[0]
        for r in person[1].items(): #person[1] is dict of calls made
            receiver = r[0]
            call_count += len(r[1])
            people[caller][0] = people[caller][0] + call_count
            people[receiver][1] = people[receiver][1] + call_count

    return ['Barb','Carl','Alan','Elan','Deja']       
