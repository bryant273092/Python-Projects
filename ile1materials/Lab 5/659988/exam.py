...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for m in line[2:]:
            ...
            db_dict[line[0]].update({line[1]:minutes})
    return db_dict



def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    people = all_parties(db)
    freq_list = []
    for p in people:
        call_num = 0
        for caller in db.keys():
            if caller == p:
                for c in db[p].values():
                    call_num += len(c)
            for receiver, calls in db[caller].items():
                if receiver == p:
                    call_num += len(calls)
        freq_list.append((p, call_num))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        called = db[caller]
        for c in called:
            if caller in db[c].keys():
                tup = sorted(list((caller, c)))
                mutual.add(tuple(tup))
    return mutual
    ...
    for p in people:
        ...
        for caller in db.keys():
            ...
            for receiver, calls in db[caller].items():
                if receiver == p:
                    received += len(calls)
        cr.append((p, called, received))
    sorted_cr = sorted(cr, key= (lambda x: (-x[1], x[2], x[0])))
    return [name[0] for name in sorted_cr]
