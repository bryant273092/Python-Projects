...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for openfile in file.readlines():
        content = openfile.rstrip().split(':')
        #for x in range(2, len(content)):
        db[content[0]][content[1]] = content[2:]
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        for receiver in db[caller]:
            call_amt = db[caller][receiver]
            call_amt = len(db[caller][receiver])
            t= tuple((receiver, call_amt))
            lst.append(t)
    return sorted(lst, key = lambda i: (-i[1], i[0]))    
