...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    result=set()
    for caller,detail in db.items():
        result.add(str(caller))
        for reciever, _ in detail.items():
            result.add(str(reciever))
    return result
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    inner_result={}
    for info in file:
        inner_result.setdefault(info[0], dict()).setdefault(info[1], list()).append([int(x) for x in info[2:]])
    for _,detail in inner_result.items():
        for reciever,call in detail.items():
            new_lst=[]
            for item in call:
                for i_item in item:
                    new_lst.append(i_item)
            detail[reciever]=new_lst
    return inner_result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller,detail in db.items():
        for reciever, call in detail.items():
            result[caller]+=len(call)
    for caller,detail in db.items():
        for reciever, call in detail.items():
            result[reciever]+=len(call)
    for name,num in result.items():
        result_list.append((name,num))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller,detail in db.items():
        for reciever, _ in detail.items():
            if caller in db.get(reciever).keys():
                if caller[0]>reciever[0]:
                    result.add((reciever,caller))
                else:
                    result.add((caller,reciever))
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller,detail in db.items():
        for reciever, call in detail.items():
            result1[caller]+=len(call)
    result2=defaultdict(int)
    for caller,detail in db.items():
        for reciever, call in detail.items():
            result2[reciever]+=len(call)
    new_result={}
    for name,num in result1.items():
        new_result.setdefault(name, list()).append(num)
    
    for name,num in result2.items():
        
        new_result[name].append(num)
    
    result_lst=[]
    for k,_ in sorted(new_result.items(), key= lambda x:(-x[1][0],x[1][1],x[0])):
        result_lst.append(k)
