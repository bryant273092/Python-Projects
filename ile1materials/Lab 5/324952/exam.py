...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        nums = []
        sline = line.strip('\n').split(':')
        if sline[1] in result[sline[0]]:
            nums.extend(result[sline[0]][sline[1]])
        for num in sline[2:]:
            nums.append(int(num))
        inside[sline[1]].extend(nums)
        result[sline[0]].update(inside)
        inside.clear()
    return result
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k, v in db.items():
        person = k
        for other_person in v.keys():
            if person in db[other_person] and other_person in db[person] and (other_person, person) not in result:
                result.add(tuple(sorted([person,other_person], key = lambda x : x)))
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    momen = defaultdict(list)
    for k, v in db.items():
        caller = 0
        for k1, v1 in v.items():
            caller += len(v1)
        momen[k].append(caller)
    for k, v in db.items():
        for k1, v1 in v.items():
            momen[k1].append(len(v1))
    for k, v in momen.items():
        momen[k].append(sum(momen[k][1:]))
    return sorted(momen.values(), key = lambda x: (-x[0], x[-1]))
