        ...
        for r in db[c]:
            for each in db[c][r]:
                ...
                if r not in result:
                    ...
                    result[r] += r_count  #issue with the count
    
   
    for each in sorted(result, key = lambda x: result[x], reverse = True):
        final_result.append((each, result[each])) #sort by num
