...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    db_list = [elements.strip('\n').split(':') for elements in file.readlines()]
    for ppl in db_list:
        in_dict = dict()
        call_list = []
        for calls in range(1,len(ppl[2:])):
            call_list.append(ppl[calls+1:])
            in_dict[ppl[calls]] = ppl[calls+1:]
            if ppl[calls-1] in db_dict:
                db_dict[ppl[calls-1]] = in_dict
            else:
                db_dict[ppl[calls-1]] = in_dict
    return db_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = []
    my_set = set()
    for outer_k, outer_v in db.items():
        my_set.add(outer_k)
        for inner_k, inner_v in outer_v.items():
            my_set.add(inner_k)

    count = 0
    for people in my_set:
        result.append((people, count))
        for x in db[people]:
            for more in db[x].keys():
                pass
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for outer_k, outer_v in db.items():
#         my_set.add(outer_k)
#         print('person', outer_k, 'with', outer_v)
        for inner_k, inner_v in outer_v.items():
            if len(inner_v) >1:

                some_list.append(inner_k)
