...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller, reciever in db.items():
        involved.add(caller)
        for people in reciever.keys():
            if people not in involved: involved.add(people)
    return involved
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        recieved_dict = {}
        duration = []
        called_line = line.rstrip().split(":")
        for x in range(len(called_line)):
            if x > 1: duration.append(int(called_line[x]))
        
        recieved_dict[called_line[1]]=duration
        print(recieved_dict)
        final_dict[called_line[0]]=recieved_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    name_frequency = []
    for caller, reciever_dict in db.items():
        current_caller = []
        current_caller.append(caller)
        caller_frequency = 0
        for reciever, frequency in reciever_dict.items():
            caller_frequency+=len(frequency)
            if reciever in exclusive_people:
                caller_frequency += len(frequency)
            people_set = (caller,caller_frequency)
        name_frequency.append(people_set)
