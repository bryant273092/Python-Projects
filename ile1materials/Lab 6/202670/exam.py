...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    with open(file, 'r') as open_file:
        ...
        for line in open_file:
            ...
            receiver = line[0]
            duration = line[1:]
            duration = [int(num) for num in duration]
                #print('line', line)
                #print('crd', caller, receiver, duration)
                #print('yo', line[]) 
                
            if caller not in result.keys():
                result[caller][receiver] = duration
                    #print('testing', dict(result))
                    
            if receiver not in result[caller].keys():
                result[caller][receiver] = duration
                    #print('testing', dict(result))
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for info in db.items():
        ...
        history = info[1]
        #print('info', info)
        for call in history.items():
            receiver = call[0]
            duration = call[1]
          
            #print(person, receiver, duration)
            
            #print('a', result.keys())
            
            if person in result.keys():
                result[person] += len(duration)
                #print('1', 'added', len(call), 'to', person)
                
            if receiver in result.keys():
                result[receiver] += len(duration)
                #print('2', 'added', len(call), 'to', receiver)
            
            if person not in result.keys():
                result[person] = len(duration)
                #print('3', 'added', len(call), 'to', person)
                
            if receiver not in result.keys():
                result[receiver] = len(duration)
                #print('4', 'added', len(call), 'to', receiver)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for info in db.items():
        person = info[0]
        history = info[1]        #for person in history
        for call in history.keys():
            #print('person = ', person)
            #print('call =', call)
            #print(db[person].keys())
            #print(db[call].keys())
            
            if person in db[call].keys() and call in db[person].keys():
                #print('succ')
                #print(result)
                if (person, call) and (call, person) not in result:
                    result.add((person, call))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    for info in db.items():
        person = info[0]
        history = info[1]        #for person in history
        for call in history.items(): pass        #result[person]
