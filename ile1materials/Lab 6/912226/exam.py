...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for i in file:
        ...
        for i in line:
            ...
            if name in dictt:
                new.append(int(i))
                dictt[name].update({name1:empty + new})
            dictt[name].update({name1:empty})
        #dictt[name].update({name1:[int(i) for i in line]})
    return dict(dictt)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k, v in dictt.items():
        empty.append((k,v))
    return sorted(empty, key = lambda k: (-k[1], k[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k,v in dictt.items():
        ...
        print("REVERSE")
        print((k[1],k[0]))
        if k in dictt.keys() and ((k[1],k[0])) in dictt.keys():
            empty.append(k)
        else:
            pass
    for i in empty:
        if i in empty and ((i[1], i[0])) in empty:
            empty.pop
            (i[1],i[0])
    print(empty)
    print(dictt)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k, v in caller.items():
        call.append((k,v))
    for k, v in reciever.items():
        rec.append((k,v))
    for name, item in zip(sorted(call), sorted(rec)):
        print(name, item)
        new[name[0]]=(name[1],item[1])
    print(new)
    for k, v in sorted(new.items(), key = lambda k: (-k[1][0], k[1][1])):
        empty.append(k)
    print(empty)
    return empty
    print(new)
