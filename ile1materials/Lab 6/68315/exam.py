...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller,recievers in db.items():
        for reciever in recievers:
            all_parties.add(caller)
            all_parties.add(reciever)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        results[content[0]][content[1]] = [int(x) for x in content[2:]]
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for reciever in db[caller].keys():
            for times in db[caller][reciever]:
                results[caller] += 1
                results[reciever] += 1
     
    return sorted(sorted(results.items()), key=lambda x: -x[1])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller,info in db.items():
        for reciever in info.keys():
            if db[caller][reciever] and db[reciever][caller]:
                results.add((caller,reciever))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller,info in db.items():
        for reciever,times in info.items():
