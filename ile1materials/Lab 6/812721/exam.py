...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    talker_d = defaultdict(int)
    for c, receivers in db.items():
        for r, ts in receivers.items():
            talker_d[c] += 1*len(ts)
            talker_d[r] += 1*len(ts)
    return sorted(sorted(talker_d.items(), key = lambda item: item[0]), key = lambda item: item[1], reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in new_d:
        for c, receivers in db.items():
            for r, ts in receivers.items():
                if caller ==r:
                    new_d[caller][1] += len(ts)
    alpha_sort = dict(sorted(new_d.items(), key = lambda item: item))
    receive_sort = dict(sorted(alpha_sort.items(), key = lambda item: item[1][1]))
    call_sort = dict(sorted(receive_sort.items(), key = lambda item: item[1][0], reverse = True))
    return list(call_sort.keys())
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    all_p = all_parties(db)
    calls_ = defaultdict(dict)
    for party in all_p:
        if party in db:
            for c, receivers in db.items():
                if c == party:
                    for r, ts in receivers.items():
                        calls_[party][r] = sum(ts)
    combined_d = defaultdict(int)
    return calls_
#     for party in all_p:
#        
#         for c, calls in calls_.items():
#          
#             if party in calls:
#                 combined_d[party][c] = calls_[party][r]
#                 print(calls)
#                 combined_d[party][c] += calls[party]
#     return combined_d
