...
def read_db(file : open) -> {str:{str:[int]}}:
    read_file = defaultdict(list)
    sub_dict = {}
    for line in file:
        line = line.rstrip('\n').split(':')
#         print(line)
        sub_dict[line[1]] = [int(i) for i in line[2:]]
#         print(sub_dict)
        read_file[line[0]].append(sub_dict)
        sub_dict = {}
            ...
            if item[0] == name:
                ...
                for person in item[1]:
                    if person == name:
                        for call in item[1][person]:
                            tracker +=1
        call_times.append((name, tracker))
        tracker = 0
        name = ''
    return sorted(sorted(call_times, key = lambda x: x[0]), key = lambda x: x[1], reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key in db:
        for item in db[key]:
            callers = (key, item)
            testing.append(callers)
    for i in range(len(testing)):
        test = testing[i]
        for group in testing:
            if test[0] == group[1] and test[1] == group[0]:
                retest = (test[1], test[0])
                if retest in mutual_call:
                    pass
                else:
                    mutual_call.append(tuple(sorted(test)))

    return set(sorted(mutual_call, key = lambda x : x[0]))
