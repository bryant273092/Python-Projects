...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        call_data = line.rstrip().split(":")
        call_dict[call_data[0]].update({call_data[1]:[int(i) for i in call_data[2:]]})
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for person in participants:
        for caller, calls in db.items():
            ...
            for kcall, vcall in calls.items():
                if person == caller:
                    sumlen += len(vcall)
                    inter_dict[person].append(sumlen)
                
    part_list = []
    for k,v in inter_dict.items():   
        part_list.append((k,sum(v)))
    return sorted(part_list,key=(lambda t: (-t[1], t[0])))
    ...
    for caller, calls in db.items():
        for reciever in calls:
            if caller in db[reciever]:
                tup = sorted((caller, reciever), key=(lambda t: t))

                call_set.add(tuple(tup))
    return call_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller, calls in db.items():
        lencall =0
        for reciever, call in calls.items():
            lencall += len(call)
        lenrecieve = 0
        for reciever, num in calls.items():
            if caller == reciever:
                lenrecieve += len(num)
        temp_list.append([caller, lencall, lenrecieve])
