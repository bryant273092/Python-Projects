...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        for i in items[2:]:
            i = int(i)
            t.append(i)
        db[items[0]].setdefault(items[1],t)
    return db
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k in db:
        for key,value in db[k].items():
            calls[key] += int(len(value))
   
    return sorted(sorted([(k,v) for k,v in calls.items()],key=lambda k:k[0]), key=lambda k:-k[1])
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k in db:
        for key,value in db[k].items():
            return sorted([k for k in db], key = len(value),reverse = True)
