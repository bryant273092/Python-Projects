...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    parties = set()
    for call_receivers in db.items():
        parties.add(call_receivers[0])
        for receivers in call_receivers[1].keys():
            parties.add(receivers)
    return parties
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    call_data = file.readlines()
    for calls in call_data:
        calls = calls.split(':')
        key = calls.pop(0)
        parties[key] = defaultdict(list)
        sub_dict = {}
        sub_key = calls.pop(0)
        call_track = []
        for call in calls:
            call_track.append(int(call.strip()))
        sub_dict[sub_key] = call_track
        parties[key] = sub_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    talker_freq = []
    caller_call_count = defaultdict(int)
    for item in db.items(): 
        for call_list  in item[1].items():
            caller_call_count[item[0]] += len(call_list[1])
            caller_call_count[call_list[0]] += len(call_list[1])
            
    return sorted(caller_call_count.items(), key = lambda x:(-x[1],x[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for item in db.items():
        caller = item[0]
        for item in db.items():
            if caller in item[1].keys() and item[0] in db[caller].keys():
                call_pair = sorted([caller, item[0]])
                call_pairs.add((call_pair[0], call_pair[1]))
    return call_pairs
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    init  = []
    caller_init_count = defaultdict(int)
    caller_recev_count = defaultdict(int)
    for item in db.items(): 
        for call_list  in item[1].items():
            caller_init_count[item[0]] += len(call_list[1]) #amount of calls made by a person
            caller_recev_count[call_list[0]] += len(call_list[1]) #amount received by a person
    
    total_pop = defaultdict(list)
    
    for caller in caller_init_count.items():
        total_pop[caller[0]].append(caller[1])
    for receiver in caller_recev_count.items():
        total_pop[receiver[0]].append(receiver[1])
    
    pops = sorted(total_pop.items(), key = lambda x:(-x[1][0], x[1][1], x[0]))
