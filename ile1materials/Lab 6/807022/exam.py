...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if data[1] not in result[data[0]]:
            result[data[0]][data[1]] = [int(t) for t in data[2:]]
        else:
            result[data[0]][data[1]].extend([int(t) for t in data[2:]])
    return result
        ...
        if c not in result:
            ...
            for r,t in rt.items():
                ...
                if r not in result:
                    ...
                    result[r] += len(t)
    return sorted(sorted(result.items(), key = lambda x: x[0]),key = lambda x :-x[1])
    ...
    for c,rt in db.items():
        for r,t in rt.items():
            if r in temp:
                ...
                temp[r] = [0,len(t)]
    return [name for name,no in sorted(sorted(sorted(temp.items(), key = lambda x: x[0]),key = lambda x: x[1][1]),key = lambda x : -x[1][0])]
