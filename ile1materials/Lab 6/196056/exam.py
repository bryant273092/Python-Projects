...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        desc = line[:-1].split(':')
        outer_dict[desc[1]] = desc[2:]
        inner_dict[desc[0]] = outer_dict
    print(outer_dict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db:
        ...
        for r in db[c]:  
            n += len(db[c][r])
        call_amount.append((c,n))
    return sorted(call_amount, key = lambda x: x[1], reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for x in sorted(db):
        for y in db[x]:
            if x in db[y] and (y,x) not in mutual:
                mutual.add((x, y))
    return mutual

def initiators (db : {str:{str:[int]}}) -> [str]:
    initiators =[]
    for caller in sorted(db, reverse = True):
        initiators.append(caller)
        initiators1 = sorted(initiators, reverse = True)
    return sorted(initiators, key = lambda x: len(db[x]), reverse = True)
