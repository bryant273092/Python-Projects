...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        spli_line = line.rstrip().split(":")
        calls = [int(call) for call in spli_line[2:]]
        output[spli_line[0]][spli_line[1]] = calls
    return dict(output)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k in db:
        for sub_k in db[k]:
            output[k] += len(db[k][sub_k])
            output[sub_k] += len(db[k][sub_k])
            
    return sorted(sorted((k,v) for k, v in output.items()),key = lambda x: x[1], reverse = True)


def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    return {tuple(sorted((key,sub_key))) for key in db for sub_key in db[key] if key in db[sub_key]}



def initiators (db : {str:{str:[int]}}) -> [str]:
    sort_by_name = sorted(db)
     
    reciver_times = list()
    for k in db:
        for sub_key in db[k]:
            for time in db[k][sub_key]:
                reciver_times.append(sub_key)
             
    sort_by_reciver = sorted(sort_by_name, key = lambda k: reciver_times.count(k))
    
    caller_times = list()
    for k in db:
        for sub_key in db[k]:
            for time in db[k][sub_key]:
                caller_times.append(k)
    
    sort_by_caller = sorted(sort_by_reciver, key = lambda k: caller_times.count(k), reverse = True)
     
    return sort_by_caller
