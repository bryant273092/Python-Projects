...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        c  = call_info[0]
        r = call_info[1]
        times = [int(time) for time in call_info[2:]]
        res[c][r] = times
    return res
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    res = defaultdict(int)
    for c, calls in db.items():
        for r, times in calls.items():
            res[r] += len(times)
            res[c] += len(times)               
    return sorted( sorted([(c, frequency) for c, frequency in res.items()],
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c, calls in db.items():
        for r, times in calls.items():
            temp[c]['calls'] += len(times)
            temp[r]['recieved'] += len(times)
            
    print(temp)
    
    for person in temp:
        res.append((person, temp[person]['calls'], temp[person]['recieved']))
                
    
    return sorted( [caller[0] for caller in sorted([calls for calls in sorted([person for person in res], key = (lambda x: x[0]))], key = (lambda x : x[2]))], key = (lambda x: x[0]), reverse = True)
