...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        if a[0] not in result:
            result[a[0]] = dict([(a[1], a[2:])])
        else:
            result[a[0]][a[1]] = a[2:]
    for key in result.keys():
        for key2 in result[key]:
            temp_list = []
            for number in result[key][key2]:
                temp_list.append(int(number))
            result[key][key2] = temp_list
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key in db.keys():
        for key2 in db[key]:
                result[key] += int(len(db[key][key2]))
                result[key2] += int(len(db[key][key2]))
    a = sorted(result.items(), key = lambda x: x[0])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key in db.keys():
        ...
        for key3 in db.keys():
            if key3 in temp_list:
                result.append((key3, key))
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    receiver = defaultdict(int)
    for key in db.keys():
        for key2 in db[key]:
            caller[key] += len(db[key][key2])
            receiver[key2]+= len(db[key][key2])
    a = sorted(caller.items(), key = lambda x:x[1], reverse = True)
    for b in a:
        result.append(b[0])
    return result
