...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    answer = set()
    for k,v in db.items():
        for called,time in v.items():
            if called not in answer:
                answer.add(called)
    return answer
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    temp=[]
    for person,v in db.items():
        number = 0
        y_num = 0
        for called, time in v.items():
            number+=1
            temp.append((called,1))
        temp.append((person,number))
    for x in temp:
        for i in range(len(temp)):
            if x[0] == temp[i][0] and temp[i] != x:
                temp.append((x[0],x[1]+temp[i][1]))
                temp.remove(temp[i])
        temp.remove(x)
    ...
    for person,v in db.items():
        for called, time in v.items():
            if person not in c:
                ...
                c[person].update({called})
    for caller,v in c.items():
        for person in v:
            for i in c:
                if i!=caller and person in i:
                    if (caller,person) not in answer and (person,caller) not in answer:
                        answer.add((caller,person))
