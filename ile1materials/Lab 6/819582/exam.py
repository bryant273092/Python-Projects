...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        inner_dict = dict()
        caller,recvr, calls = line.strip('\n').split(':')[0], line.strip('\n').split(':')[1], line.strip('\n').split(':')[2:]
        int_calls = []
        for i in calls:
            int_calls.append(int(i))
        temp_dict[caller] = 'nothing'
       
        if temp_dict[caller] == 'nothing':
            temp_dict[caller] = {recvr: int_calls}
        else:
            temp_dict[caller].update({recvr: int_calls})
        
    return temp_dict
         
#         if recvr not in temp_dict[caller]:
#             temp_dict[caller] = {recvr: calls}
#         else:
#             temp_dict[caller].update({recvr: calls})
...
#             
#             
# talker_frequency({'alan':{'alan': [10,20]}})
#         
        ...
        for k1 in v.keys():
            ...
            for i in inner:
                if k in db[i]:
                    ...
                    other = sorted(other)
                    other = tuple(other)
                    result_set.add(other)
    return result_set
        
            
print(mutual_callers({'Alan': {'Barb': [10, 20], 'Carl': [10, 10]}, 'Barb': {'Alan': [10, 10], 'Deja': [5, 5, 5]}, 'Carl': {'Alan': [10, 5], 'Deja': [15], 'Elan': [5, 5]}, 'Deja': {'Elan': [5]}, 'Elan': {'Carl': [10]}}))
    ...
    for k,v in db.items():
        ...
        for k2,v2 in db.items():
            ...
            for k1,v1 in v2.items():
                ...
                if k == k1:
                    rcall+=1
        temp_info[k].append(rcall)
    print(temp_info)
    return [k for k,v in sorted(temp_info.items(), key = lambda x: (-x[1][0], -x[1][1]))]
  
        
        
            
print(initiators({'Alan': {'Barb': [10, 20], 'Carl': [10, 10]}, 'Barb': {'Alan': [10, 10], 'Deja': [5, 5, 5]}, 'Carl': {'Alan': [10, 5], 'Deja': [15], 'Elan': [5, 5]}, 'Deja': {'Elan': [5]}, 'Elan': {'Carl': [10]}}))
