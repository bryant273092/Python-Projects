...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for callers in db.keys():
        participate.add(callers)
        for recievers in db[callers].keys():
            participate.add(recievers)
    return participate
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        caller = line.pop(0)
        reciever = line.pop(0)
        ts = [int(t) for t in line]
        db[caller].update({reciever: ts})
    return dict(db)



def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    talk_f = defaultdict(int)
    for c in db.keys():
        for r in db[c].keys():
            talk_f[c] += len(db[c][r])
            talk_f[r] += len(db[c][r])
    talk_freq = [(c,f) for c,f in talk_f.items()]
    return sorted(talk_freq, key =(lambda x : (-x[1],x[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db.keys():
        for r in db[c].keys():
            if r in db[c].keys() and c in db[r].keys():
                if c<r:
                    mutual.add((c,r))
                else:
                    mutual.add((r,c))            
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db.keys():
        for r in db[c].keys():
            ...
            if c in db[r].keys():
                c_r['r'] += len(db[r][c])
                starters[c].update(c_r)
            else:
                starters[c].update(c_r)
        c_r = defaultdict(int)
    init = []
    for a in starters.keys():
        call_rec = [a]
        for b in starters[a].values():
            call_rec.append(b)
        init.append(tuple(call_rec))
