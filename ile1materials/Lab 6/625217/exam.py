...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if final == {}:
            final.update({new_line[0] : {new_line[1] : []}})
        else:
            if new_line[0] in final.keys():
                final[new_line[0]].update({new_line[1] : []})
            else:
                final.update({new_line[0] : {new_line[1] : []}})
        for num in range(2, len(new_line), 1):
            final[new_line[0]][new_line[1]].append(int(new_line[num]))
    return final
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for person in db.keys():
        if final == []:
            ...
            people_picked.append(person)
        elif person not in people_picked:
            final.append((person, 0))
            people_picked.append(person)
        else:
            pass
    for person in db.keys():
        for call1 in db[person].keys():
            for tup1 in final:
                if tup1[0] == person:
                    final.remove(tup1)
                    final.append((person, tup1[1] + len(db[person][call1])))
                    break
                else:
                    pass
    for person in db.keys():
        for call2 in db[person].keys():
            for tup in final:
                if tup[0] == call2:
                    final.remove(tup)
                    final.append((person, tup[1] + len(db[person][call2])))
                    break
                else:
                    pass
    return final
