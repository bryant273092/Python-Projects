...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    result = []
    for first_caller in db:
        for second_caller in db[first_caller]:
            result.append(second_caller)
    return set (result)
...
def read_db(file : open) -> {str:{str:[int]}}:
    result ={}
    for i in file:
        each_sentence = i.strip().split(":")
        caller = each_sentence[0] #iterating over the first word in each line (the caller)
        receiver = each_sentence[1]
        number = each_sentence[2:4]
        
        result[caller] = {receiver:number}
