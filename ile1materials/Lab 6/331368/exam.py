...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    result=set()
    for callers in db.keys():
        result.add(callers)
        for receivers in db[callers].keys():
            if receivers not in result:
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        if list[1] not in result[list[0]].keys():
            result[list[0]][list[1]]=[]
        for n in range(2,len(list)):
            result[list[0]][list[1]].append(int(list[n]))
    return dict(result)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    y=dict()
    for callers in db.keys():
        if callers not in y.keys():
            y[callers]=0
        for recievers in db[callers].keys():
            y[callers]+=len(db[callers][recievers])
    for callers2 in db.keys():
        for recievers2 in db[callers2].keys():
            y[recievers2]+=len(db[callers2][recievers2])
    for name in y.keys():
        result.append((name,y[name]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        for reciever in db[caller].keys():
            if caller in db[reciever].keys():
                temp2=list()
                if (caller,reciever) not in temp:
                    if (reciever,caller) not in temp:
                        temp2.append(reciever)
                        temp2.append(caller)
                        temp2=sorted(temp2,key=(lambda x:x))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    temp=list()
    for caller in db.keys():
        if caller not in new.keys():
            new[caller]["caller"]=0
            new[caller]["reciever"]=0
        for recievers in db[caller].keys():
            new[caller]["caller"]+=len(db[caller][recievers])
    for callers2 in db.keys():
        for recievers2 in db[callers2].keys():
            new[recievers2]["reciever"]+=len(db[callers2][recievers2])
    
    for name in new.keys():
        temp.append((name,new[name]["caller"],new[name]["reciever"]))
    temp=sorted(temp,key=(lambda x:-x[1]))
    for y in range(1,len(temp)):
        if temp[y-1][1]==temp[y][1]:
            if temp[y-1][2]>temp[y][2]:
                z=temp[y-1]
                temp[y-1]=temp[y]
                temp[y]=z
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for caller in db.keys():
        for reciever in db[caller].keys():
            temp=0
            for x in range(len(db[caller][reciever])):
                temp+=db[caller][reciever][x]
            if reciever not in new[caller].keys():
                new[caller][reciever]=temp
            else:
                new[caller][reciever]+=temp
    for caller2 in new.keys():
        for reciever2 in new[caller2].keys():
            if caller2 not in new[reciever2].keys():
                new[reciever2][caller2]=new[caller2][reciever2]
            else:
                new[reciever2][caller2]+=new[caller2][reciever2]
    
    for caller3 in new.keys():
        temp3=[]
        for reciever3 in new[caller3].keys():
            temp3.append(new[caller3][reciever3])
        result.append(temp3)
    return result
