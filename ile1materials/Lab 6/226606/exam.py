...
def read_db(file : open) -> {str:{str:[int]}}:#HELP
    d = dict()
    
    for line in file.readlines():
        item_list = line.split(':')
        for i in range(len(item_list[2:])):
            item_list[i+2] = int(item_list[i+2].strip())
        
        d_in = dict()
        
        if item_list[0] not in d.keys():             
            d_in.update({item_list[1]: item_list[2:]})
            d.update({item_list[0]: d_in})
            
        elif item_list[0] in d.keys():
            d_in.update({item_list[1]: item_list[2:]})
            d.update({item_list[0]: d_in})
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db: #Looks at all outer keys
        total = 0
        for r in db[c]: #Looks at inner dict for that key
            total += len(db[c][r])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:#DONE
    ...
    for c in db: #Looks at all outer keys
        for r in db[c]: #Looks at inner dict for that key
            if c in db[r] and (r, c) not in result:
                tup = tuple(sorted([r,c]))
                result.add(tup)             
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    result = sorted(result, key=lambda x: len(db[x]), reverse=True)
