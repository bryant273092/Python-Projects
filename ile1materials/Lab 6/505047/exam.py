...
def read_db(file : open) -> {str:{str:[int]}}:
    result = {}
    for words in file:
        words_list = words.rstrip().split(':')
        k = words_list[0]
       
        if k not in result:
            result[k] = {words_list[1]:list(int(i) for i in words_list[2:])}
        if k in result:
            result[k].update({words_list[1]:list(int(i) for i in words_list[2:])})
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k,v in db.items():
        for m,n in v.values():
            ...
            if len(db[k][m]) >=1 and db[m][k] in db[m] and len(db[m][k]) >= 1:
                result.add((k,m+4))
    return result
