...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        newvalue=line.split(":")
        newlist=[]
        for value in newvalue[2:]:
            newlist.append(int(value))
        if newvalue[1] in newdict[newvalue[0]]:  
            newdict[newvalue[0]][newvalue[1]]+=newlist
        else:
            newdict[newvalue[0]].update({newvalue[1]:newlist})
    return newdict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key, value in db.items():
        for key1,value1 in value.items():
            newdict[key]+=len(value1)
            newdict[key1]+=len(value1)
    for key,value in newdict.items():
        newlist.append((key,value))
    def element(tuple):
        return tuple[1]
    newlist.sort( key=element,reverse=True)
    return newlist
        ...
        for key1,value1 in value.items():
            ...
            for key2, value2 in db.items():
                for key3,value3 in value2.items():
                    if current==key2 and key==key3:
                        if (current,key) not in newset:
                            if current[0]<key[0]:
                                newset.add((current,key))
                            else:
                                newset.add((key,current))
    return newset
