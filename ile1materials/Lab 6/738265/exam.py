...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        caller[call][rcv] = [int(x) for x in time]
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    inner = db.values()
    for name, receivers in db.items():
    ...
    print(inner)
    for value in inner:
        for obj in value.items(): 
            talk[obj[0]] += len(obj[1])
            
    talk_list = [(x,y) for x,y in talk.items()]
    
    return sorted(talk_list, key = (lambda t: (-t[1],t[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for name, call in db.items():
        for item in call:
            new_dict[name].add(item)
    
    mut_dict = dict(new_dict)
   
    #print(mut_dict)
    
    for name, name2 in (mut_dict.items()):
        for item in name2:
            #print(item)
            if item in mut_dict[name] and name in mut_dict[item]:
                mutual[name].add(item)
                mut_set.add((item,name))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for name, calls in db.items():
        #print(name, calls)
        for item in calls.values():
            #print(item)
            caller[name] += len(item)
    ...
    for name, calls in caller.items():
        final[name].append(calls)
        
    for name, rcvs in rcv.items():
        final[name].append(rcvs)
    
    #print(dict(final))
    
    init_list = [(x,y[0],y[1]) for x, y in final.items()]
    
    #print(init_list)
    
    new_list = sorted(init_list, key = lambda t: (-t[1],t[2]))
    
    final_list = [x[0] for x in new_list]
