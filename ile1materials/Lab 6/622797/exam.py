...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        for t in range(2,len(new)):
            time.append(int(new[t]))
        caller[new[1]]= time
        if new[0] not in call_dict:
            call_dict[new[0]]= caller
        else:
            call_dict[new[0]].update(caller)
    return call_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    caller = []
    for c in db.items():
        if c[0] not in caller:
            caller.append(c[0])
            caller.append(0)
            for r in c[1].items():
                num = len(r[1])
                caller[caller.index(c[0])+1] += num
        else:
            for r in c[1].items():
                num = len(r[1])
                caller[caller.index(c[0])+1] += num
    
    for i in range(0,len(caller),2):      
        call_list.append((caller[i],caller[i+1]))
    return sorted(sorted(call_list), key=lambda x: x[1], reverse=True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    initial = dict()
    for c in db.items():
        if c[0] not in initial:
            num=0
            for r in c[1].items():
                num+= len(r[1])
            initial[c[0]]=num
    return sorted(initial, key=lambda x: x[1], reverse=False)
