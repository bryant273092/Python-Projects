...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        result_dict[caller].setdefault(receiver, []);
        result_dict[caller][receiver].extend(int_list);
    return result_dict;
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for receiver, call_lst in db[caller].items():
            result_dict[caller] += len(call_lst);
            result_dict[receiver] += len(call_lst);
    return sorted(result_dict.items(), key = (lambda t: (-t[1], t[0])));
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db:
        for receiver in db[caller]:
            if db[receiver].get(caller, None) != None:
                result_list.append([caller,receiver])
    return {tuple(sorted([person1, person2])) for person1, person2 in result_list};
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    result_dict = defaultdict(dict);
    for caller in db:
        result_dict[caller].setdefault('caller_num',0);
        for receiver, call_lst in db[caller].items():
            result_dict[caller]['caller_num'] += len(call_lst);
            result_dict[receiver].setdefault('receiver_num', 0);
            result_dict[receiver]['receiver_num'] += len(call_lst);
    return [person for person, call_record in sorted(result_dict.items(), key = (lambda t: (-t[1]['caller_num'], t[1]['receiver_num'], t[0])))];
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for caller in db:
        for receiver in db[caller]:
            full_name_set.add(caller);
            full_name_set.add(receiver);
    name_index = dict([(name, index) for index, name in enumerate(sorted(full_name_set), 0)]);
    for name in full_name_set:
        result_lst.append([0 for name in full_name_set]);
    for caller in db:
        for receiver, call_lst in db[caller].items():
            for duration in call_lst:
                result_lst[name_index[caller]][name_index[receiver]] += int(duration);
                result_lst[name_index[receiver]][name_index[caller]] += int(duration)
    return result_lst;
