...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for x, y in a_dict.items():
        a_list.append((x,y))
    return sorted(a_list, key = (lambda x: (-x[1],x[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k in db:
        for x in db[k]:
            a_list.append(((k,x)))
    for items in a_list:
        if (items[0], items[1]) in a_list and (items[1],items[0]) in a_list:
            if items[0] > items[1]:
                a_set.add((items[1], items[0]))
            else:
                a_set.add((items[0], items[1]))
    return a_set
                


def initiators (db : {str:{str:[int]}}) -> [str]:
    reciever_dict = defaultdict(int)
    call_dict = defaultdict(int)
    overall_tuple = []
    for k,v in db.items():
        for x, y in v.items():
            call_dict[k]+= len(y)
            reciever_dict[x]+= len(y)
    for k,v in db.items():
        overall_tuple.append((k,call_dict[k], reciever_dict[k]))
    return [a for a,b,c in sorted(overall_tuple, key = (lambda x: (-x[1],x[2], x)))]
