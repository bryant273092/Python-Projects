...
#         while line != []:
        ...
        if caller not in people:
            people[caller] = {r: time}
        else:
            if r in people[caller]:
                people[caller][r].extend(time)
            else:
                people[caller].update({r: time})
    return people
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k, v in db.items():
        for x, y in v.items():
            if x in db[k] and k in db[x]:
                pair = (k, x)
                sort_pair = sorted(pair)
                pair = (sort_pair[0], sort_pair[1])
                all.add(pair)
    return all
