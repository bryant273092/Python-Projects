...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        lane= line.strip().split(':')
        for i in lane:
            a= int(lane[2])
            #b= int(lane[3])
            #print(b)
            newdict[lane[0]][lane[1]]= [a]
    return newdict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    newlist= []
    for caller in db.keys():
        for receiver, times in db[caller].items():
            pass
            #print(times)
            #newdict[caller][]
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        for i in db[caller].keys():
            if i in db.keys():
                if caller in db[i].keys():
                    result= tuple(sorted([caller,i]))
            newset.add(result)
    return newset
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    newdict= dict()
    for caller in db.keys():
        if caller not in newdict.keys():
            newdict[caller]= [0,0]
        for receiver in db[caller].keys():
            newdict[caller][0] += len(db[caller][receiver])
            if receiver not in newdict.keys():
                newdict[receiver]= [0,0]
            newdict[receiver][1] = newdict[receiver][1] + len(db[caller][receiver])
    result= sorted(list(newdict.items()), key=(lambda x: (-x[1][0], x[1][1], x[0])))
    for i in result:
        newlist.append(i[0])
    return newlist
