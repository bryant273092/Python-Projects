...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        inner_list = []
        for x in range(2, len(line_list)):   
            inner_list.append(int(line_list[x]))
        db[line_list[0]].update({line_list[1]: inner_list})
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k in db:
        ...
        for v in db.values():
            ...
            if k in v:
                receiver_count += len(v[k])

        temp_list.append((k, caller_count, receiver_count))
    sorted_list = sorted(temp_list, key = lambda x: (-x[1], x[2], x[0]))
