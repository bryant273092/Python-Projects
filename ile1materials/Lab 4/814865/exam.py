...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        s = line.strip().split(':')
        if s[1] in result[s[0]]:
            for t in s[2:]:
                result[s[0]][s[1]].append(int(t))
        else:
            ts = []
            for t in s[2:]:
                ts.append(int(t))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for r in rt:
        if r not in result:
            result.append(r)
    return sorted(sorted(result, key = lambda x: rt[x]), key = lambda x: ct[x], reverse = True)
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for name in names:
        if name in db:
            for r, times in db[name].items():
                if r in ndict[name]:
                    ndict[name][r] += sum(times)
                else:
                    ndict[name][r] = sum(times)
                if name in ndict[r]:
                    ndict[r][name] += sum(times)
                else:
                    ndict[r][name] = sum(times)
    for name in names:
        line = []
