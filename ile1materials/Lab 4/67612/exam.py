...
#from prompt import command_prompt
import goody
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        time = [int(t) for t in line[2:]]
        inner_dict = {line[1]:time}
        return_dict[line[0]].update(inner_dict)
    return return_dict

def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    return_list = []
    everyone = all_parties(db)
    for p in everyone:
        count = 0
        for c in db:
            if p in db.keys():
...
if __name__ == '__main__':
    import prompt
    file = goody.safe_open('Enter file', 'r', 'Invalid file', 'db1.txt')
    read_db(file)
