    ...
    for c,y in db.items():
        for r,t in y.items():
            for i in t:
                cd[r].append(i)
                cd[c].append(i)
    return sorted([(c,len(t)) for c,t in sorted(dict(cd).items())],key = lambda x:x[1],reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k,v in ad.items():
        bd[k][v[0]] = v[1]
    print(bd)
    bd= sorted(sorted(bd.items(), key = lambda x:x[0]), key = lambda x:x[0][1])
    return [a[0] for a in bd]
