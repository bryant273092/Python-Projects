...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        for v in temp[2:]:
            dbs[temp[1]].append(int(v))
        db[temp[0]].update(dict(dbs))
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for d in db:
        for k in db[d]:
            freq[k] += len(db[d][k])
            freq[d] += len(db[d][k])
    sfreq = sorted(freq.items(), key = lambda item: (item[0]))
    return sorted(sfreq, key = lambda x: (x[1]), reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for d in db:
        ...
        for k in db[d]:
            first[d] += len(db[d][k])
            recieve[k] += len(db[d][k])
    skeys = sorted(keys, key = lambda x: recieve[x])
    return sorted(skeys, key = lambda x: first[x], reverse = True)
