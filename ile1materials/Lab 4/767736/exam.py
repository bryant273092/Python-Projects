    ...
    for line in lines:
        ...
        try:
            for i in range(len(lines)):
                if n_items[0] not in new_dict.keys():
                    new_dict[n_items[0]] = {n_items[1]:n_items[2:]}
                else:
                    in_dict = dict()
                    in_dict[n_items[1]] = n_items[2:]
                    new_dict[n_items[0]].update(in_dict)
        except KeyError:
            pass
    return new_dict
    ...
    for key in db.keys():
        for item in ppl_list:
            if key == item[0]:
                ...
                for v in v_list:
                    first_value = item[1]
                    ppl_list.remove(item)
                    total = first_value+ len(db[key][v])
                    break
            in_list = list(db[key])
            for i in in_list:
                if db[key][i] == item[0]:
...
#             ppl_list.remove(item)
#             total = first_value + len(db[key][v])
#             ppl_list.append((key, first_value))
    return ppl_list
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    final_set = set()
    for key in db.keys():
        receivers = list(db[key])
        print(receivers)
        for r in receivers:
            temp_list.append([key, r])
    try:
        for i in range(len(temp_list)):
            for j in range(len(temp_list)):
                if i != j:
                    if temp_list[i][0] == temp_list[j][0]:
                        if temp_list[i][1] == temp_list[j][1]:
                            temp_list.remove(temp_list[j])
                    elif temp_list[i][0] == temp_list[j][1]:
                        if temp_list[i][1] == temp_list[j][0]:
                            temp_list.remove(temp_list[j])
    except IndexError:
        pass
    for item in temp_list:
        in_tuple = (item[0], item[1])
        final_set.add(in_tuple)
    return temp_list
