...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    result = set(db.keys())                                                 #update names in keys to result first
    for values in db.values():                                              
        result.update(i[0] for i in values.items())                         #update names in values to result
    return result
...
def read_db(file : open) -> {str:{str:[int]}}:
    result = defaultdict(dict)                                              #default dictionary where values are dictionaries
    for line in file:
        file_list = line.rstrip().split(':')                                #make lines from files to list format
        time = []
        for i in file_list[2:]:                                             #only take the values from index 2 since first two are names
            time.append(int(i))                                                 
        if file_list[1] not in result[file_list[0]].keys():                 #result doesn't have record then add that dictionary entry directly
            result[file_list[0]][file_list[1]] = time
        else:                                                               #result has record then add the new time to the existing dictionary values
            result[file_list[0]][file_list[1]].extend(time)
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = set()
    name_set = set(db.keys())                                               #all the name as callers
    for values in db.values():
        name_set.update(i[0] for i in values.items())                       #add all the name as receivers to that set
    for i in name_set:
        k = 0
        for values in db[i].values():                                       #the calls person i make
            k += len(values)                                                #add the number of calls he makes to that person
        for l in db.values():
            for m in l.keys():                                              
                if i == m:                                                  #check if name i is in someone's call list
                    k += len(l[m])                                          #if so, add the number of calls 
        result.add((i, k))                                                  #add the tuple to result set
    return sorted(sorted(result), key = lambda x: x[1], reverse = True)     #sort it by person name first, then sort it by times of calls



def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    result = set()
    for i in db.keys():                                                     #all names in callers
        for j in db[i].keys():                                              #all names in inner dictionary callers
            if i in db[j].keys():                                           #if one caller is in someone's receiver list
                result.add(tuple(sorted((i,j))))                            #add that person to result set
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    time_list = []
    for i in db.keys():                                                     #all person in callers
        call_time = 0                           
        for j in db[i].values():                                            #leads to the list of calls he makes to every person
            call_time += len(j)                                             #add call length to him    
        receive_time = 0
        for k in db.values():                                               #all person in receivers
            if i in k.keys():                                               #if the person is in the list, i want his information
                receive_time += len(k[i])                                   #add receive length to him
        time_list.append((i, call_time, receive_time))                      #append tuple((person name, call time, receive time))
    time_list.sort()                                                        #sort by name
    time_list.sort(key = lambda x: x[2])                                    #sort by receive time
    time_list.sort(key = lambda x: x[1], reverse = True)                    #sort by call time    
    return [i[0] for i in time_list]                                        #return only the name
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    result = []
    for i in sorted(list(db.keys())):                                       #all person in callers
        time_list = []
        for j in sorted(list(db.keys())):                                   #all person in callers
            time = 0
            if j in db[i].keys():                                           #calls caller 1 to caller 2
                time += sum(db[i][j])
            if i in db[j].keys():                                           #calls caller 2 to caller 1    
                time += sum(db[j][i])
