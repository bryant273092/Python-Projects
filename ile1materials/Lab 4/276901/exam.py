...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for lines in file:
        caller = lines.rstrip().split(":")[0]
       # print(caller)
        receiver = lines.rstrip().split(":")[1]
       # print(receiver)
      # print(receiver)
        minutes = lines.rstrip().split(":")[2:]

      
       # for r, m in zip(receiver,minutes):
        result[caller][receiver] = minutes
#         for m in zip(re
#             result[c][r] = 
#             
    return result
