...
#                     if t not in db[c][r]:
#                         db[c][r].append(t)
        db[line[0]][line[1]] = [int(i) for i in line[2:]]
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    return sorted(list(zip(talkers, calls)))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    result = []
    person = [c for c in db]
    calls = [0 for c in db]
    for c in db:
        for r in db[c]:
            for t in db[c][r]:
                calls[person.index(c)] += 1
    for c in calls:
        m = max(calls)
        most_calls = person[calls.index(m)]
        person.remove[person[calls.index(m)]]
        result.append(most_calls)
