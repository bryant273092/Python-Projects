...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for l in file:
        ...
        if caller not in result:
            result.setdefault(caller, dict())
        
        call_history = result[caller]
        call_history.setdefault(receiver, list())
        
        call_length_history = call_history[receiver]
        [call_length_history.append(int(call)) for call in call_lengths]
        
        result[caller] = call_history
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for (k, values) in db.items():
        
        if k not in amounts:
            amounts.setdefault(k, 0)
            
        for v in values.values():
            new_amount = amounts[k] + len(v)
            amounts[k] = new_amount                        
                     
    for (values) in db.values():
        for key in db.keys():
            if key in values:
                new_amount = amounts[key] + len(v)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    [result.append(key) for key in counter.keys()]
