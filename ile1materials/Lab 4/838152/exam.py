...
def read_db(file : open) -> {str:{str:[int]}}:
    r_db = defaultdict(dict)
    for line in file:
        r_db[line.strip().split(':')[0]][line.strip().split(':')[1]] = [int(t) for t in line.strip().split(':')[2:]]
    return dict(r_db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c, rd in db.items():
        for r, ts in rd.items():
            temp_dict[c] += len(ts)
            temp_dict[r] += len(ts)
    temp_dict = dict(temp_dict)
    return [(name, calls) for name, calls in sorted(temp_dict.items(), key= lambda x: (-x[1],x[0]))]
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c, rd in db.items():
        for r, _ in rd.items():
            if r in db.keys() and c in db[r].keys():
                callers_pairs.add(tuple(sorted((c,r))))
    return callers_pairs
    



def initiators (db : {str:{str:[int]}}) -> [str]:
    temp_call = defaultdict(int)
    temp_receive = defaultdict(int)
    for c, rd in db.items():
        for r, ts in rd.items():
            temp_call[c] += len(ts)
            temp_receive[r] += len(ts)
    return sorted(temp_call.keys(), key= lambda x: (-temp_call[x], temp_receive[x],x))
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for c, rd in db.items():
        for r, ts in rd.items():
            for i in ts:
                temp_dict[tuple(sorted((c,r)))] += i 
    temp_dict = dict(temp_dict)
    final_list = []
    for i in sorted(db):
        inner_list = []
        for j in sorted(db):
            if tuple(sorted((i,j))) not in temp_dict:
                inner_list.append(0)
            else:
                inner_list.append(temp_dict[tuple(sorted((i,j)))])
