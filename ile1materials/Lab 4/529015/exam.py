...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if line[0] not in result.keys():
            result[line[0]] = {line[1]: int_list }
        else:
            if line[1] not in result[line[0]].keys():
                result[line[0]][line[1]] = int_list
            else:
                result[line[0]][line[1]].extend(int_list)
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for key in db.keys():
        if key not in idict.keys():
            idict[key] = [0,0]
        for k in db[key].keys():
            idict[key][0] += len(db[key][k])
            if k not in idict.keys():
                idict[k] = [0,0]
            idict[k][1] += len(db[key][k])
    result = sorted(list(idict.items()), key = lambda x: (-x[1][0], x[1][1], x[0]))   
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for key in db.keys():
        ...
        for k in db[key].keys():
            names.add(k)
            if k not in mdict[key].keys():
                mdict[key][k] = sum(db[key][k])
            else:
                mdict[key][k] += sum(db[key][k])
            if k not in mdict.keys():
                mdict[k] = dict()
            if key not in mdict[k].keys():
                mdict[k][key] = sum(db[key][k])
            else:
                mdict[k][key] += sum(db[key][k])
    ...
    for n in range(len(names)):
        result.append([])
        for name in names:
            if name not in mdict[names[n]].keys():
                result[n].append(0)
            elif name in mdict[names[n]].keys():
                result[n].append(mdict[names[n]][name])
    return result
