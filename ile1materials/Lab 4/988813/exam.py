...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for l in llist:
        content = l.split(":")
        time_list = [int(i) for i in content[2:]]
        if content[0] not in result_dict:
            result_dict[content[0]] = {content[1]:time_list}
        else:
            if content[1] not in result_dict[content[0]]:
                result_dict[content[0]][content[1]] = time_list
            else:
                result_dict[content[0]][content[1]] += time_list
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k in db:
        for name in db[k]:
            freq[k] += len(db[k][name])
            freq[name] += len(db[k][name])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k in db:
        for name in db[k]:
            if k in db[name]:
                pattern = sorted((k,name))
                result_set.add((pattern[0], pattern[1]))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k in db:
        for name in db[k]:
            inits[k] += len(db[k][name])
            recvs[name] += len(db[k][name])
    return sorted(db.keys(), key=lambda x:(-inits[x], recvs[x], x))
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    result_list = list()
    temp = list()
    for x in range(len(callers)):
        temp = list()
        for y in range(len(callers)):
            if x == y:
                temp.append(0)
            elif callers[y] not in db[callers[x]]:
                if callers[x] in db[callers[y]]:
                    temp.append(sum(db[callers[y]][callers[x]]))
                else:
                    temp.append(0)
            else:
                if callers[x] in db[callers[y]]:
                    temp.append(sum(db[callers[x]][callers[y]])+sum(db[callers[y]][callers[x]]))
                else:
                    temp.append(sum(db[callers[x]][callers[y]]))
