...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        yes = line.strip('\n').split(':')
        inner_list = []
        for t in range(2, len(yes)):
            inner_list.append(int(yes[t]))
        if yes[0] not in final:
            inner_dict = {}
            inner_dict[yes[1]] = inner_list
            final[yes[0]] = inner_dict
        else:
            inner_dict = {}
            inner_dict[yes[1]] = inner_list
            final[yes[0]].update(inner_dict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for i in final:
        if i[0] in ha:
            final2.append((i[0], i[1] + ha[i[0]]))
    
    return sorted(final2, key = lambda x: (-x[1], x[0]))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    yes = sorted(tl, key = lambda x: (x[1], x[0]), reverse = True)
