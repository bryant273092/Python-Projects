...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    database = set()
    for caller in db:
        for (called, time) in db[caller].items():
            people = caller and called
            database.add(people)
    return database
...
def read_db(file : open) -> {str:{str:[int]}}:
    file = goody.safe_open('enter file name', 'r', 'Error: Illegal file')
...
'''
    ...
    for caller in db:
        for (called, time) in db[caller].items():
            people = caller and called
            if caller or called in db or db[caller]:
                num_of_calls += 1   
                freq[people].append(num_of_calls)
    return freq
'''
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    pass
'''
    talked = defaultdict(set)
    for caller in db:
        for (called, time) in db[caller].items():
            if caller in db and called in db[caller]:
                talked[caller].add(called)
                
    return sorted(talked.items(), key = lambda x: ( (x[0], x[1])))
