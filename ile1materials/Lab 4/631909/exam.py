...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if temp[1] not in result[temp[0]]:
            result[temp[0]].update({temp[1]: [int(t) for t in temp[2:len(temp)]]})
        else:
            result[temp[0]][temp[1]] += [int(t) for t in temp[2:len(temp)]]
    return dict(result)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key in db:
        for val in db[key]:
            for call in db[key][val]:
                result[key] += 1
                result[val] += 1
    return sorted(sorted(list(result.items()), key = lambda x: x[0]),key = lambda x: x[1], reverse = True )
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key in db:
        for val in db[key]:
            if val in db:
                if key in db[val]:
                    mutuals.add(tuple(sorted([key, val])))
    return mutuals
    ...
    for key in db:
        for val in db[key]:
            for call in db[key][val]:
                results[key][0] += 1
                results[val][1] += 1
    return [k[0] for k in sorted(sorted(sorted(list(results.items()), key = lambda x: x[0]), key = lambda x: x[1][1]), key = lambda x: x[1][0], reverse = True)]
