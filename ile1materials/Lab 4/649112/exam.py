...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for i in db:
        for j in db[i]:
            result_dict[i].append(len(db[i][j]))
            
            result_dict[j].append(len(db[i][j]))
            
    for person in result_dict:
        result.append((person, sum(result_dict[person])))
        
    return sorted(sorted(result, key=lambda x: (x[0], x[1])), key=lambda x: (x[1],x[1]), reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    reciever_dict = defaultdict(list)
    
    for i in db:
        for j in db[i]:
            caller_dict[i].append(len(db[i][j]))
            
            reciever_dict[j].append(len(db[i][j]))

    
    for person in caller_dict:
        result.append((person, sum(caller_dict[person]), sum(reciever_dict[person]) ))
        
    print(result)
    
    alphabetical = sorted(result, key=lambda x : (x[0],x[0],x[0]))
    increasing = sorted(alphabetical, key=lambda x: (x[2],x[2],x[2]))
    decreasing = sorted(increasing, key=lambda x: (x[1],x[1],x[1]), reverse = True)
    for n in decreasing:
        final.append(n[0])
    return final
