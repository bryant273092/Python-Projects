...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if (line_split[0] not in result.keys()):
            result[line_split[0]] = defaultdict(list) #caller
        for time in line_split[2:]:
            result[line_split[0]][line_split[1]].append(int(time))
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    call_nums = defaultdict(int) # person : number of calls
    for caller in db.keys():
        for reciever in db[caller].keys():
            for time in db[caller][reciever]:
                call_nums[caller] += 1
                call_nums[reciever] += 1
    result = list(((x, y) for x,y in call_nums.items()))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        for reciever in db[caller].keys():
            all_calls.append((caller, reciever))
    for call in all_calls:
        reverse_call = (call[1], call[0])
        if reverse_call not in result and reverse_call in all_calls:
            result.add(tuple(sorted(call)))
    return result



def initiators (db : {str:{str:[int]}}) -> [str]:
    call_num_db = dict() # {name:(times initiated, times recieved), ...}
    for caller in db.keys():
        if caller not in call_num_db.keys():
            call_num_db[caller] = [0,0]
        for reciever in db[caller].keys():
            if reciever not in call_num_db.keys():
                call_num_db[reciever] = [0,0]
            for time in db[caller][reciever]:
                call_num_db[caller][0] += 1
                call_num_db[reciever][1] += 1
    return sorted(list(call_num_db.keys()), key = lambda x: (-call_num_db[x][0], call_num_db[x][1], x))
