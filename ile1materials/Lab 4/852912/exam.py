...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        small_tuple = (line_list[0],line_list[1],line_list[2:])
            
        final.append(small_tuple)    
    print("f",final)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k,v in db.items():
        for h in v.keys():
            for t in db.keys():
                if t == h and k in db[h].keys():
                    small = (k,h)
                    
        result.append(small)
        f = set(result)
        for a in f:
            if tuple((a[0],a[1])) == tuple((a[1],a[0])):
                z = f.remove(tuple((a[0],a[1])))
