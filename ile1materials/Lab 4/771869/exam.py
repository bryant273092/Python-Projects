...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    parties=set()
    for call in db.keys():
        parties.add(call).update(set(db[call].keys()))
    return(parties)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        db_list=line.rstrip('\n').split(':')
        caller,called=db_list.pop(0),db_list.pop(0)
        if called in db_dict[caller]:
            db_dict[caller][called]+=db_list
        else:
            db_dict[caller][called]=db_list
    return(db_dict)
dbt=read_db(open('db1.txt'))



def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    fre_dict=defaultdict(int)
    for call in db.keys():
        for called in db[call].keys():
            fre_dict[call]+=len(db[call][called])
            fre_dict[called]+=len(db[call][called])
    return(sorted(fre_dict.items(),key=lambda x:-x[1]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for call in db.keys():
        for called in db[call].keys():
            mc_set.add(sorted(call,called))
    return(mc_set)
print(mutual_callers(dbt))


def initiators (db : {str:{str:[int]}}) -> [str]:
    def calldict(): 
        return [0,0]
    init_dict=defaultdict(calldict)
    for call in db.keys():
        for called in db[call].keys():
            init_dict[call][0]-=len(db[call][called])
            init_dict[called][1]+=len(db[call][called])
    init_list=init_dict.keys()
    return(sorted(init_list, key=lambda x:init_dict[x]))
