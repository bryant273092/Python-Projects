...
def read_db(file : open) -> {str:{str:[int]}}: # DONE
    file = file.readlines()
    db = defaultdict(dict)
    for line in file:
        line = [x.strip() for x in line.split(":")]
        db[line[0]][line[1]] = [int(x) for x in line[2:]]
    return(db)


def all_parties(db : {str:{str:[int]}}) -> {str}: # DONE
    cr = set()
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]: # DONE
    lt = []
    for person in db.keys() : 
        count = 0
        for x in db[person]:
            for z in db[person][x]:
                count +=1   
            if person in db[x]:
                for y in db[x][person]:
                    count +=1
        lt.append((person, count))
    return sorted(lt, key = lambda x: x[1], reverse = True) 



def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}: # DONE
    lt = set()
    for person in db.keys() : 
        for x in db[person]:   
            if person in db[x]:
                mutual = [person,x]
        lt.add(tuple(sorted(mutual)))
    return lt       




def initiators (db : {str:{str:[int]}}) -> [str]: # DONE
    lt = []
    for person in db.keys() : 
        count = 0
        r_count = 0 
        for x in db[person]:
            ...
            if person in db[x]:
                for y in db[x][person]:
                    r_count +=1
        personal = (person, count, r_count)
        lt.append(personal)
    lt = sorted(lt, key = lambda x: x[1], reverse = True)
    return [x[0] for x in lt]
...
if __name__ == '__main__':
    ...
    db = read_db(goody.safe_open("Please input a file to open" , "r" ,"Error: File not Found")) 
    print(all_parties(db))
    print(talker_frequency(db))
    print(initiators(db))
