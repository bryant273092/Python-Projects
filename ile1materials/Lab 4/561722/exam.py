...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        if line_list[1] not in result[line_list[0]]:
            result[line_list[0]][line_list[1]]=list()
            for i in line_list[2:]:
                result[line_list[0]][line_list[1]].append(int(i))
        else:
            for i in line_list[2:]:
                result[line_list[0]][line_list[1]].append(int(i))
    return dict(result)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k1 in db:
        result[k1]=0
        for k2,v in db[k1].items():
            if k2 in result.keys():
                result[k2]+=len(db[k1][k2])
            result[k1]+=len(v)
    for caller,num in result.items():
        lst.append((caller,num))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k1 in db:
        for k2,v in db[k1].items():
            result.add((k1,k2))
    #for cr in result:
        #for i in db[cr[0]][cr[1]]:
            #if i in db[cr[1]][cr[0]]:
                #final.add((db[cr[0]],db[cr[1]]))
    return final
