...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        indic = dict()
        if l[0] not in dic:
            indic[l[1]] = [int(i)for i in l[2:]]
            dic[l[0]] = indic
        else:
            indic = dic[l[0]]
            if l[1] in indic:
                for i in l[2:]:
                    indic[l[1]].append(int(i))
            else: indic[l[1]] = [int(i)for i in l[2:]]
            dic[l[0]] = indic
    return dict(dic)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for i in db:
        for j in db[i]:
            if i not in dic:
                dic[i] = len(db[i][j])
                if j not in dic: dic[j] = len(db[i][j])
                else: dic[j] = dic[j]+len(db[i][j])
            else:
                if j not in dic:
                    dic[j] = len(db[i][j])
                    dic[i] = dic[i]+len(db[i][j])
                else:
                    dic[i] = dic[i]+len(db[i][j])
                    dic[j] = dic[j]+len(db[i][j])
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k in dic:
        receive = 0
        for l in db:
            if k in db[l]: receive += len(db[l][k])
        dic[k].append(receive)
    new_dic = sorted(dic.items(), key=lambda x: (-x[1][0],x[1][1],x[0]))
    for m in new_dic:
        lis.append(m[0])
    return lis
