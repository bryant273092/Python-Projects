...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    pc = set(db.keys())
    for inner_db in db.values():
        for r in inner_db.keys():
            if r not in pc: pc.add(r)
    return pc
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        pc_dict[infos[0]].setdefault(infos[1], list()).extend([int(t) for t in infos[2:]])
    return dict(pc_dict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    tf = []
    for p in all_parties(db):
        n = 0
        for c, inner_dict in db.items():
            for r, ts in inner_dict.items():
                if p == c or p == r: 
                    n += len(ts)
        tf.append((p, n))
    return sorted(tf, key=lambda x: (-x[1], x[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    result = set()
    for c, inner_dict in db.items():
        for r in inner_dict.keys():
            if c in db.keys() and r in db[c].keys() and r in db.keys() and c in db[r].keys():
                result.add(tuple(sorted((c, r))))
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for p in all_parties(db):
        ...
        result[p] = (cs, rs)
    return sorted([x for x in result.keys()], key = lambda x: (-result[x][0], result[x][1]))
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    mx = []
    sorted_parties = sorted(list(all_parties(db)))
    for p1 in sorted_parties:
        ts = []
        for p2 in sorted_parties:
            t = 0
            if p1 in db.keys() and p2 in db[p1].keys():
                t += sum(db[p1][p2])
            if p2 in db.keys() and p1 in db[p2].keys():
                t += sum(db[p2][p1])
            ts.append(t)
        mx.append(ts)
    return mx
