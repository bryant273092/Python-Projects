...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    new_set = set()
    for p1,p2 in db.items():
        if p1 not in new_set:
            new_set.add(p1)
        for p3 in p2.keys():
            if p3 not in new_set:
                new_set.add(p3)
    return new_set
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    new_line = [lines.strip().split(":") for lines in file]
    for elements in new_line:
        dict2 = {}
        list1 = []
        for i in elements[2:]:
            i = int(i)
            list1.append(1)
        dict2[elements[1]] = list1
        new_list[elements[0]] = dict2
    file.close()
        ...
        if p not in new_list:
            ...
            if p in nt.keys():
                for n,t in nt.items():
                    for i in t:
                        count+=1
        new_list.append((p,count))
    return new_list


def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    new_set = set()
    for p,nc in db.items():
        for n,c in nc.items():
            new_set.add((p,n))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    return [x for x in sorted(db, key = lambda x: len(db.values()))]
