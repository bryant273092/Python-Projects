...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for i in range(2, len(values)):
            if values[1] not in db[values[0]].keys():
                db[values[0]].update({values[1]: [int(values[i])]})
            else:
                db[values[0]][values[1]].append(int(values[i]))
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k, v in db.items():
        for caller in v.keys():
            frequency[k] += len(v[caller])
            frequency[caller] += len(v[caller])
    return sorted([(k, v) for k, v in dict(frequency).items()], key = (lambda x: (-x[1], x[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k, v in db.items():
        for caller in v.keys():
            if k in db[caller].keys():
                if mutual == set():
                    mutual.add(tuple(sorted((k, caller))))
                else:
                    for x in mutual:
                        if k in x and caller in x:
                            break
                    else:
                        mutual.add(tuple(sorted([k, caller])))
    return mutual
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k, v in db.items():
        for name, calls in v.items():
            total_calls[k] += len(calls)
            total_received[name] += len(calls)
    order = sorted([(k, total_calls[k], total_received[k]) for k in total_calls.keys()], key = (lambda x: (-x[1], x[2], x[0])))
    return [name[0] for name in order]



def matrix (db : {str:{str:[int]}}) -> [[int]]:
    matrix_list = [[0 for i in range(len(db.keys()))] for x in range(len(db.keys()))]
    person_index = {}
    index = 0
    for k in db.keys():
        person_index.update({k: index})
        index += 1
    for k, v in db.items():
        for receiver, calls in v.items():
            for time in calls:
                if matrix_list[person_index[k]][person_index[receiver]] == 0:
                    matrix_list[person_index[k]][person_index[receiver]] += time
                    matrix_list[person_index[receiver]][person_index[k]] += time
                else:
                    pass
    return matrix_list
