...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db.keys():
        for r in db[c].keys():
            call_freq[c] += len(db[c][r])
            call_freq[r] += len(db[c][r])
    #print(dict(call_freq))
    
    freq = []
    for key in call_freq:
        freq.append((key, call_freq[key]))
        ...
        for r in db[c].keys():
            if c in db[r].keys():
                if ((r,c)) not in mutual:
                    c_greater = (len(db[c][r]) >= len(db[r][c]))
                    if c_greater:
                        mutual.add((c,r))
                    else:
                        mutual.add((r,c))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    init_list =sorted(init, key = lambda v: init[v][0], reverse = True)   
    return init_list



def matrix (db : {str:{str:[int]}}) -> [[int]]:
    callers = sorted(all_parties(db))
    print(callers)
    matr_dict = {}
    '''for c in callers: 
        for r in db[c].keys():
            matr_dict[c] = [{r:0} for i in range(len(callers))]
        matr_dict[c] = 
    print(matr_dict)  ''' 
