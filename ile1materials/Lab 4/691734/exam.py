...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        if c not in call_dict:
            call_dict[c] = dict()
            call_dict[c][r] = [int(time) for time in ts]
        elif c in call_dict:
            if r in call_dict[c]:
                for time in ts:
                    call_dict[c][r].append(int(time))
            else:
                call_dict[c][r] = [int(time) for time in ts]
    return call_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    # Finding the individual names and adding them to caller_set
    for key, value in db.items():
        caller_set.add(key)
        for inner_key in value.keys():
            caller_set.add(inner_key)
    
    # Search for each caller's frequency of calls
    for caller in caller_set:
        call_count = 0
        for key, value in db.items():
            if key == caller:
                for inner_value in value.values():
                    call_count += len(inner_value)
            else:
                if caller in value:
                    call_count += len(value[caller])
        return_list.append((caller, call_count))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    # Checking to find mutual callers
    for caller in caller_set:
        # Find the people the caller has called
        called_names = list()
        if caller in db:
            called_names.extend(inner_key for inner_key in db[caller])
        for name in called_names:
            # Check if the called people also called caller back
            if name in db:
                if caller in db[name]:
                    mutuals = [caller, name]
                    mutuals_set.add(tuple(sorted(mutuals, key=lambda x:x)))
    return mutuals_set        



def initiators (db : {str:{str:[int]}}) -> [str]:
    # Stores list of: [(caller, # of calls made, # of calls received)]
    total_list = list()
    # Set of all caller names
    caller_set = set()
    ...
    for caller in caller_set:
        # Create new temporary data structure for each caller
        caller_info = [caller]
        caller_count = 0
        receiver_count = 0
        if caller in db:
            for value in db[caller].values():
                caller_count += len(value)
        caller_info.append(caller_count)
        for key, value in db.items():
            if caller in value:
                receiver_count += len(value[caller])
        caller_info.append(receiver_count)
        total_list.append(tuple(caller_info))
    # Return a new list of the callers from the sorted total_list
    return [item[0] for item in sorted(total_list, key = lambda x:(-x[1], x[2], x[0]))]
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    # List 'matrix' is the list that will be returned
    matrix = list()
    for r_name in row_names:
        # List 'new_list' will be appended to 'matrix'
        new_list = list()
        for c_name in col_names:
            if r_name == c_name:
                ...
                minutes = 0
                # Check the minutes the caller called for
                if c_name in db[r_name]:
                    for time in db[r_name][c_name]:
                        minutes += time
                # Also check the minutes the caller received a call
                if c_name in db:
                    if r_name in db[c_name]:
                        for time in db[c_name][r_name]:
                            minutes += time
                new_list.append(minutes)
        # For each row, append 'new_list'
