...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        x = line.strip().split(':')
        if x[0] not in db:
            db[x[0]] = {x[1]: [int(n) for n in x[2:]]}
        elif x[0] in db and x[1] in db[x[0]]:
            for i in x[2:]:
                db[x[0]][x[1]].append(int(i))
            
        else:
            db[x[0]].update({x[1]: [int(n) for n in x[2:]]})
    return db
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db:
        for r in db[c]:
            people.append((c, len(db[c][r])))
            people.append((r, len(db[c][r])))
    for i in people:
        if i[0] not in p:
            p[i[0]] = [i[1]]
        else:
    ...
    for j in p:
        frequency.append((j, sum(p[j])))
    
    return sorted(sorted(frequency), key = lambda x: x[1], reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db:
        for r in db[c]:
            if c in db[r]:
                if len(db[c][r]) >= 1 and len(db[r][c]) >=1 :
                    if (r,c) not in mutual:
                        x = sorted([c,r])
                        mutual.add((x[0], x[1]))
    return mutual
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db:
        for r in db[c]:
            c_names.append((c, len(db[c][r])))
            r_names.append((r, len(db[c][r])))
    
    for c in c_names:
        if c[0] not in c_dict:
            c_dict[c[0]] = [c[1]]
        else:
    ...
    for i in c_dict:
        callers[i] = [sum(c_dict[i])]
        for j in r_dict:
            if j == i:
                callers[j].append(sum(r_dict[j]))
    
    
    return sorted(sorted(sorted(callers), key = lambda x: callers[x]), key = lambda x: callers[x][1])
...
if __name__ == '__main__':
    ...
    print(initiators(read_db(open('db1.txt'))))
