...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    rdict = defaultdict(dict)
    for line in file:
        tlist = []
        nline = line.strip().split(':')
        for n in nline[2:]:
            tlist.append(int(n))   
            rdict[nline[0]][nline[1]] = tlist
    return dict(rdict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key,value in db.items():
        for rec, time in value.items():
            tdict[key].append(len(time))
            tdict[rec].append(len(time))
    for k,v in sorted(tdict.items()):
        tdict[k] = sum(v)
    
    return sorted(tdict.items(), key = (lambda t:(-t[1], t[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key,value in db.items():
        for rec,time in value.items():
            if key in db[rec].keys():
                if rec not in mdict.keys():
                    mdict[key].append(rec)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    #print(db)
    idict = defaultdict(lambda: [0,0])
    ilist = []
    for key,value in db.items():
        for rec,time in value.items():
            #print(rec,time)
            idict[key][0] += len(time)
            idict[rec][1] += len(time) 
    #print(dict(idict))
   # print(sorted(idict, key = (lambda t: (-idict[t][0], idict[t][1]) )))
    return sorted(idict, key = (lambda t: (-idict[t][0], idict[t][1]) ))
    #for k,v in sorted(idict.items(), key = (lambda t: (-v))):
     #   ilist.append(k)
   # print(ilist)
   # print(sorted(idict), key = (lambda t: (-idict[t[0]], idict[t[1]])))
...
if __name__ == '__main__':
    ...
    db2 = {'Alan': {'Barb': [10, 20], 'Carl': [10, 10]}, 'Barb': {'Alan': [10, 10], 'Deja': [5, 5, 5]}, 'Carl': {'Alan': [10, 5], 'Deja': [15], 'Elan': [5, 5]}}
    db3 = {'Gary': {'Barb': [1], 'Carl': [19, 12, 2], 'Hope': [8], 'Alan': [25, 26]}, 'Carl': {'Hope': [10], 'Gary': [11, 8], 'Deja': [12], 'Alan': [15, 11, 4], 'Barb': [34]}, 'Elan': {'Fern': [16], 'Deja': [6, 27], 'Carl': [3], 'Hope': [29]}, 'Barb': {'Gary': [14], 'Alan': [14], 'Fern': [4]}, 'Deja': {'Gary': [30, 26], 'Barb': [25, 3], 'Fern': [10, 3, 27], 'Hope': [4], 'Elan': [5]}, 'Alan': {'Barb': [3, 15], 'Hope': [2, 46], 'Gary': [36]}, 'Fern': {'Alan': [7], 'Carl': [37, 46, 43], 'Elan': [5], 'Gary': [7]}, 'Hope': {'Elan': [15], 'Fern': [9, 2, 36], 'Alan': [6, 16], 'Barb': [29]}}
    #all_parties(db1)
    '''
    file = open('db1.txt')
    read_db(file)
