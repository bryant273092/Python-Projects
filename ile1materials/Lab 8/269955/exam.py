...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for every_line in file:
        ...
        if receiver in db[caller]:
            for i in int_time:
                db[caller][receiver].append(i)
        else:
            db[caller][receiver] = int_time
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    L = []
    for key, value in db.items():
        for person, calls in db[key].items():
            D[key]+=len(calls)
        for k, v in value.items():
            D[k] += len(v)
    ...
    return sorted(sorted(L), key = lambda x : x[1], reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key, value in db.items():
        for k, v in value.items():
            if k in db[key] and key in db[k]:
                if sorted((key, k)) in L:
                    pass
                else:
                    L.append(sorted(((key, k))))
    for i in L:
        S.add(tuple(i))
    return S
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for key, value in db.items():
        for person, calls in db[key].items():
            caller_dict[key]+=len(calls)
        for k, v in value.items():
            receiver_dict[k] += len(v)
    for person, calls in dict(caller_dict).items():
        L.append((person, calls, dict(receiver_dict)[person]))
    new_L = sorted(sorted(sorted([i for i in L]),key = lambda x: x[2]), key = lambda x: x[1], reverse = True)
    for i in new_L:
        final.append(i[0])
    return final
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    final=[]
    alph_names = sorted(list(all_parties(db)))
    for row in alph_names:
        person_list = []
        for column in alph_names:
            if column not in db[row]:
                person_list.append(0)
            elif (row, column) in mutual_callers(db):
                person_list.append(sum(db[row][column])+sum(db[column][row]))
            else:
                person_list.append(sum(db[row][column]))
        final.append(person_list)
    return final
