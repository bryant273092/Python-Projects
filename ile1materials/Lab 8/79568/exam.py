...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if line[1] in result[line[0]]:
            result[line[0]][line[1]] += [int(t) for t in line[2:]]
        else:
            result[line[0]][line[1]] = [int(t) for t in line[2:]]
    file.close()
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = {}
    for c in db:
        result[c] = result.get(c,0) + sum(len(ts) for ts in db[c].values())
        for r in db[c]:
            result[r] = result.get(r,0) + len(db[c][r])
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db:
        for r in db[c]:
            result[c]['c'] = result[c].get('c', 0) + len(db[c][r])
            result[r]['r'] = result[r].get('r', 0) + len(db[c][r])
    return sorted(result, key = (lambda x: (-result[x]['c'], result[x]['r'], x)))
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    cp = dict(enumerate(sorted({cp[0] for cp in count}), 0))

    result = []
    for i in range(len(cp)):
        result.append([])
        for j in range(len(cp)):
            result[i].append(count.get((cp[i],cp[j]),0))
    return result
