...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    nested_list = [i.strip().split(':') for i in file.readlines()]
    for line in nested_list:
        result[line[0]].append((line[1],list(int(i) for i in line[2::])))
    for c in result:
        result[c]= dict(result[c])
    return dict(result)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    return sorted(sorted_result, key = lambda x:x[1], reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    receive_counter = defaultdict(int)
    result = []
    for c in db:
        for r in db[c]:
            call_counter[c] += len(db[c][r])
            receive_counter[r] += len(db[c][r])
    for c in call_counter:
        for r in receive_counter:
            if r == c:
                help_dict[c] = (call_counter[c],receive_counter[r])
    sorter_result = sorted(help_dict.items(), key = lambda x:x[1][1])
    
    for i in sorted(sorter_result, key = (lambda x:x[1][0]),reverse = True):
        result.append(i[0])
