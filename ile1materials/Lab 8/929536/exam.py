...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        caller=x[0]
        rece=x[1]
        time=[int(each) for each in x[2:]]
        if rece not in a[caller].keys():
            a[caller][rece]=time
        else:
            a[caller][rece]+=time
    return a
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    a=defaultdict(int)
    for caller,r_time in db.items():
        for rece,freq in r_time.items():
            a[caller]+=len(freq)
            a[rece]+=len(freq)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    a=set()
    for caller,r_fre in db.items():
        for rece,fre in r_fre.items():
            if caller in db[rece].keys():
                a.add(tuple(sorted([caller,rece])))
    return a
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller,r_fre in db.items():
        r=0
        c_n=0
        for c,r_f in db.items():
            if c==caller:
                for ra,f in r_f.items():
                    c_n+=len(f)
            elif c!=caller:
                for rec,fre in r_f.items():
                    if caller==rec:
                        r+=len(fre)              
        a[caller]=[c_n,r]
    return sorted(a.keys(),key=(lambda x:(-a[x][0],a[x][1])))

def matrix (db : {str:{str:[int]}}) -> [[int]]:
    a=defaultdict(int)
    for caller,r_time in db.items():
        for rece,time in r_time.items():
            pair=tuple(sorted((caller,rece)))
            a[pair]+=sum(time)
    z=a.copy()  #avoid changing a while iterating through      
    for pair,time in a.items():
        z[(pair[1],pair[0])]=a[pair]
    a=z.copy()      
    num=len(db.keys())
    x=[[0 for a in range(num)] for i in range(num)]
    name_list=sorted(db.keys())
    for i in range(num):
        for j in range(num):
            x[i][j]=a[name_list[i],name_list[j]] if i!=j else 0
    return x
