...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key1, value in db.items():
        for name in value.keys():
            #print(x)   
            if len(value.values()) > 1 and name is key1: #key1 in list(value.keys()):
                #for r in list(value.keys()):
                mutuals.add((key1, name))
