...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        c,r,t=line.rstrip().split(':',2)
        if r not in result[c]:
            result[c][r]=[]
        result[c][r].extend(list( int(i) for i in t.split(':')))
    return dict(result)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db:
        for r in db[c]:
            for t in range(len(db[c][r])):
                result[c]+=1
                result[r]+=1
    return sorted((tuple(i) for i in result.items()),key=(lambda x : (-x[1],x[0])))
    ...
    for c in db:
        for r in db[c]:
            for t in range(len(db[c][r])):
                ...
                names.add(c)
                names.add(r)
    return sorted(names,key=(lambda name: (-call_times[name],receive_times[name],name)))


def matrix (db : {str:{str:[int]}}) -> [[int]]:
    result=[]
    for c in sorted(all_parties(db)):
        row=[]
        for r in sorted(all_parties(db)):
            ct=0
            if c in db and r in db[c]:
                ct+=sum(db[c][r])
            if r in db and c in db[r]:
                ct+=sum(db[r][c])
            row.append(ct)
