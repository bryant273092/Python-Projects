...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    setresult=set()
    for people in db:
        setresult.add(people)
        for recivers in db[people]:
            setresult.add(recivers)
    return setresult
...
def read_db(file : open) -> {str:{str:[int]}}:
    finaldict=dict()
    chunks=[elements.strip('\n').split(':') for elements in file.readlines()]
    for listofcalls in chunks:
        tempdict=dict()
        for thing in listofcalls:
            liist=[listofcalls[2:]]
            templist=[]
            for num in liist:
                for numm in num:
                    templist.append(int(numm))
            tempdict[listofcalls[1]]=templist
        finaldict[listofcalls[0]]=tempdict
    return finaldict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    finallist=[]
    finalset=set()
    for people in db.keys():
        finallistt=[]
        for recivers in db[people]:
            finallistt.append(recivers)
        finalset.add((recivers,finallistt.count(recivers)))
    lisst=list(finalset)
    sortedlist=sorted(lisst)
    return sortedlist
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    finalset=set()
    for people in db:
        if people == db[people].keys():
            finalset.add((people,db[people].keys()))
    return finalset



def initiators (db : {str:{str:[int]}}) -> [str]:
    return[[k] for k in sorted(db.keys(),key=lambda k: (-(db[k]),db[k][0]))]
