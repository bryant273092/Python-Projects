...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    '''takes dict argument returns a set of all names
        {'Deja','Barb','Elan','Carl'}'''
    people = set()
    for k,v in db.items():
        for k2 in v:
...
def read_db(file : open) -> {str:{str:[int]}}:
    '''takes a dictionary and return a dict'''
    result = {}
    inner_dict = {}
    in_file = file.read().split('\n')
    for line in in_file:
        new_line = line.split(':')
        if new_line[0] not in result:
            result[new_line[0]] = {new_line[1]:new_line[2:]}
        else:
            result[new_line[0]] += {new_line[1:]}
...
#         if new_line[1] not in inner_dict:
#             inner_dict[new_line[1]] = new_line[2:]
#         else:
#             inner_dict += new_line[2:]
#         print(inner_dict)
#         if new_line[1] in inner_dict:
#             inner_dict[new_line[0]] += new_line[1:]
#         else:
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    '''take a dict and return list of 2-tuples'''
    pass



def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    '''take a dictionary and return a set with 2-tuple'''
    pass
