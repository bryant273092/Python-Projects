...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        line = line.strip('\n').split(':')
        caller, rec, info = line[0], line[1], [int(num) for num in line[2:]]
        if rec in db[caller]:
            db[caller][rec]+= info
        else:
            db[caller].update({rec:info})
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        tf[caller]
        for person, call in db[caller].items():
            tf[caller] += len(call)
            tf[person] += len(call)
    return sorted(tf.items(), key = (lambda x : (-x[1], x[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db:
        for rec in db[caller]:
            if rec in db and caller in db[rec]:
                if (caller, rec) not in m_set and (rec, caller) not in m_set:
                    a = sorted((caller, rec))
                    m_set.add(tuple(a))
    return m_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db:
        for rec in db[caller]:
            a[caller] += [len(db[caller][rec])]
            a[caller] = [sum(a[caller])]
