...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    party = set()
    for c,r_and_time in db.items():
        for r in r_and_time.keys():
            party.add(c)
            party.add(r)
    return party
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c,r_and_time in db.items():
        for r,time in r_and_time.items():
            freq[c] += len(time)
            freq[r] += len(time)
    
    return sorted(freq.items(), key = lambda x: (-x[1],x[0]))
    
    #return sorted(db.items(),key = lambda x: -x[1])



def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    #callers = set()
    #for c,rt in db.items():
        #for r in rt.keys():
            #for caller in db.keys(): # r is keys, time is values
               # if rt.get(caller,None)  and db.get(r,None) != None: 
                  #  callers.add((caller,r))
    #return callers
    callers = set()
    for c,rt in db.items():
        for r in rt.keys():
            if db.get(r,None) != None and c in db[r].keys() and db.get(c,None) != None and r in db[c].keys():
                callers.add((r,c))
    return callers
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    #initiate = defaultdict(dict) # name :{call:4,receive:3}}
    #receive_count = 0
    #for c,rt in db.items():
        #for r,t in rt.items():
            #call_count = sum([len(v) for v in db[c].values()])
            #receive_count +=1 
            #initiate[c]['call'] = call_count
            
            #initiate = len(r.values())
            
    initiate = defaultdict(dict)
    receive_count = 0
    rcv_list= []
    for c,rt in db.items():
        call_count = sum([len(v) for v in list(db[c].values())])
        initiate[c]['call'] = call_count
        for r in rt.keys():
            rcv_list.append(r)
        for k in rcv_list:
            if k == c:
                receive_count += 1
        initiate[c]['rcv'] = receive_count
    return dict(initiate)
