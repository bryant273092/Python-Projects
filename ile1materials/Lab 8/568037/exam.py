...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    calls = [caller for caller in db]
    for recievers in db.values():
        for reciever in recievers:
            calls.append(reciever)
    return set(calls)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if line[1] not in db[line[0]]:
            db[line[0]].update({line[1]: [int(time) for time in line[2:]]})
        else:
            db[line[0]][line[1]].extend([int(call) for call in line[2:]])
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for calls_to in db[caller]:
            calls[caller] += len(db[caller][calls_to])
            calls[calls_to] += len(db[caller][calls_to])
    return sorted(sorted(calls.items(), key = lambda x: x[0]), key = lambda x: x[1], reverse = True)



def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    mutal = []
    for caller in db: # caller = key of db
        for call_to in db[caller]:
            if len(db[caller][call_to]) >= 1: # if called a receiver more than once
                if call_to in db: # Pass KeyError
                    if caller in db[call_to]: # Pass KeyError
                        if len(db[call_to][caller]) >= 1: # If called a caller more than once
                            mutal.append((caller, call_to))
    return set([tuple(sorted(pair)) for pair in mutal])
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db:
        ...
        calls_to = 0
        for calls in db[caller]:
            calls_to += len(db[caller][calls])
        track[caller].append(calls_to)
        
        calls_from = 0
        for recievers in db.values():
            for reciever in recievers:
                if caller == reciever:
                    calls_from += len(recievers[reciever])
        track[caller].append(calls_from)
        
    track = sorted( sorted( sorted(track.items()), key = lambda x: x[1][1]), key = lambda x: x[1][0], reverse = True )
    return [name[0] for name in track]
