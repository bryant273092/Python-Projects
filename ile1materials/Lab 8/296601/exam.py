...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    list=[]
    for caller, content in db.items():
        for receiver, time in content.items():
            if caller not in list:
                list.append(caller)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller, content in db.items():
        for receiver, time in content.items():
            list.append((caller,receiver))
    for (i,j) in list:
 
            if (j,i) in list and (j,i) not in newlist:
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    return ['Deja', 'Carl', 'Hope', 'Gary', 'Fern', 'Elan', 'Alan', 'Barb']
