...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller, inner_dict in db.items():
        d.append(caller)
        for reciever, times in inner_dict.items():
            d.append(reciever)
    return set(d)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller, inner_dict in db.items():
        for reciever, times in inner_dict.items():
            d[caller] += len((db[caller][reciever]))
            d[reciever] += len(db[caller][reciever])
    return sorted(list(d.items()),key = lambda x: (-x[1],x[0]))
