...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller in db.keys():
        parties.add(caller)
        for receivers in db[caller].keys():
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    callers = set()
    for line in file:
        caller, *receiver = line.strip().split(':')
        final[caller]=receiver
        timescalled = []
        for i in range(1, len(receiver)):
            timescalled.append(int(receiver[i]))
        callers.add(caller)
        result[caller] = {receiver[0]: timescalled }
    print (final)
    for i in callers:
        items = {}
        for j in final.keys(): 
            if j == i:
                timescalled = []
                for k in range(1, len(final[j])):
                    timescalled.append(int(final[j][k]))
                items[final[j][0]]=timescalled
        result[i] = items
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        for receivers in db[caller].keys():
            frequency[caller]+=len(db[caller][receivers])
            frequency[receivers] += len (db[caller][receivers])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        for receivers in db[caller].keys():
            if caller in db[receivers].keys():
                tuple = ()
                if caller < receivers:
                    tuple = (caller, receivers)
                else:
                    tuple = (receivers, caller)
                mutuals.add(tuple)
    return mutuals
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db.keys():
        for receivers in db[caller].keys():
            frequency[caller][0]+=len(db[caller][receivers])
            frequency[receivers][1]+= len(db[caller][receivers])
    for caller in frequency.keys():
        calls.append(frequency[caller][0])
        received.append(frequency[caller][1])
    calls = sorted(calls, reverse = True)
    for i in calls:
        temp = []
        for caller in frequency.keys():
            if i == frequency[caller][0]:
                tuple = []
                tuple = [caller, frequency[caller][1]]
                temp.append(tuple)
            for i in (sorted(temp, key = lambda t: [t[1], t[0]])):
