...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for i in lines:
        ...
        for j in range(2, len(temp)):
            if temp[0] not in db:
                db[temp[0]] = dict(defaultdict(dict))
                db[temp[0]][temp[1]] = list()
                db[temp[0]][temp[1]].append(temp[j])
            else:
                if temp[1] not in db[temp[0]]:
                    db[temp[0]][temp[1]] = list()
                    db[temp[0]][temp[1]].append(temp[j])
                else:
                    db[temp[0]][temp[1]].append(temp[j])
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    return sorted(list(dict(dd).items()), key = lambda t: (-t[1], t[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for i in temp4:
        finalanswer.add(tuple(sorted(i, key = lambda t:t[0])))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    temp = list(dict(dd).items())
    
    temp = sorted(temp, key = lambda t: (-t[1][0], t[1][1]))
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for i in db.keys():
        for j in db[i].keys():
            if i in db[j].keys():
                temp.append((i, j, db[i][j] + db[j][i]))
            else:
                temp.append((i, j, db[i][j]))
                
    for i in range(len(db.keys())):
        answer.append([])
        for j in range(len(db.keys())):
            answer[i].append(0)
