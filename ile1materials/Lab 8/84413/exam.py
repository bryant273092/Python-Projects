...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    s1 = {c for c in db}
    s2 = {r for c in db for r in db[c]}
    for i in s2:
        s1.add(i)
    return s1
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if r not in result[c]:
            result[c].update({r: ts})
        else:
            result[c][r].extend(ts)
    return result
    ...
    for c in db:
        ...
        for i in db:
            if i in s:
                for j in db[i]:
                    if j == c:
                        result.add(tuple(sorted((c, i))))
    return result



def initiators (db : {str:{str:[int]}}) -> [str]:
    n_as_c = defaultdict(int)
    n_as_r = defaultdict(int)
    for c in db:
        for r in db[c]:
            n_as_c[c] += len(db[c][r])
            n_as_r[r] += len(db[c][r])
    all_info = {p:[0, 0] for p in all_parties(db)}
    for c in n_as_c:
        all_info[c][0] = n_as_c[c]
    for r in n_as_r:
        all_info[r][1] = n_as_r[r]
    result = [p for (p,[c, r]) in sorted(all_info.items(), key = lambda item: (-item[1][0], item[1][1], item[0]))]
    return result



def matrix (db : {str:{str:[int]}}) -> [[int]]:
    names = sorted([n for n in all_parties(db)])
    result = []
    for i in names:
        result.append(list())
        for j in names:
    ...
    for n in names:
        ...
        for m in names:
            if m in db[n]:
                result[i][j] += sum(db[n][m])
                result[j][i] += sum(db[n][m])
            j += 1
        i += 1
    return result
