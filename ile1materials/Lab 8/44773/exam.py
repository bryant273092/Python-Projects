...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        caller, called, *times= line.strip("\n").split(':')
        d[caller][called] = times
     
    for caller, calls in d.items():
        for called, times in calls.items():
            result[caller][called] = [int(t) for t in times]
    return dict(result)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller, calls in db.items():
        for called, times in calls.items():
            d[caller].add(called)
            if caller in d[called]:
                a,b = sorted((caller, called))
                result.add((a,b))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for name, times in result.items():
        k,v = times[0], times[1]
        answer.append((name,k,v))
    
    anotherlist = sorted(answer, key = lambda x: (-x[1],x[2],x[0]))
