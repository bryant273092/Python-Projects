...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for i in range(2, len(line)):
            listt.append(int(line[i]))
        if r not in d[c].keys():
            d[c][r] = listt
        else:
            orig_list = d[c][r]
            orig_list.extend(listt)
            d[c][r] = orig_list
        listt = []
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in call_d:
        temp_d[c] = call_d[c]+rec_d[c]
        
    for key, value in sorted(temp_d.items(), key = lambda x: (-x[1], x[0])):
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for key, value in sorted(temp_d.items(), key = lambda x: (-x[1][0], x[1][1], x[0])):
        listt.append(key)
