...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller in db.keys():
        caller_set.append(caller)
        for reciever in db[caller].keys():
            caller_set.append(reciever)
            
    return set(caller_set)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        time_list = []
        for time in parsed[2:]:
            time_list.append(int(time))
        
        
        
        if parsed[0] in new_dict.keys():
            new_dict[parsed[0]][parsed[1]] = time_list
        else:
            new_dict[parsed[0]] = {parsed[1]: time_list}
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        ...
        for reciever in db[caller].keys():
            name_list.append(reciever)
            for time in db[caller][reciever]:
                name_list.append(caller)
                
    
    hold = list(set(name_list))
    
    counter = []
    for u in hold:
        counter.append(name_list.count(u))
    
    temp = list(zip(hold, counter))
