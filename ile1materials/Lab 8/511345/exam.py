...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        inner_dict = {}
        caller = word_list[0]
        receiver = word_list[1]
        new = []
        for i in word_list[2:]:
            new.append(int(i))
        if receiver in inner_dict.keys():
            inner_dict[receiver] = inner_dict[receiver].extend(new)
        else:
            inner_dict[receiver] = new
...
#         result.append((caller, len(receiver.values())))
#     return result
    pass
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    result = set()
    for caller, receiver_dict in db.items():
        for receiver in receiver_dict.keys():
            if caller in db[receiver]:
                tup_list = sorted([caller,receiver])
                result.add(tuple(tup_list))      
    return result
