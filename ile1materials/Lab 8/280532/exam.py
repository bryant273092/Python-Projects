...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller,calls in db.items():
        for receiver in calls: everyone.add(receiver)
        everyone.add(caller)
    return everyone
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    db = defaultdict(list)
    for dict1 in content:
        caller,receiver,*length = dict1.rstrip().split(':')
        for i in range(len(length.copy())): length[i] = int(length[i])
        db.setdefault(caller, {receiver:[]})
        if receiver in db[caller]: db[caller][receiver].extend(length)
        else: db[caller][receiver] = length
    return db
    file.close()
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller, calls in db.items():
        for receiver in calls:
            for times in db[caller][receiver]:
                dict1[caller] += 1
                dict1[receiver] += 1         
    return [(name,dict1[name]) for name in sorted(dict1, key = lambda x : (-dict1[x],x))]

def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    set1 = set()
    for caller,calls in db.items():
        for receiver in calls:
            if receiver in db[caller] and caller in db[receiver]:
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller, calls in db.items():
        for receiver in calls:
            for times in db[caller][receiver]: dict1[caller] += 1
    for caller, calls in db.items():
        for receiver in calls:
            for times in db[caller][receiver]: dict2[receiver] += 1    
    return [name for name in sorted(dict1, key = lambda x : (-dict1[x],dict2[x],x))]
