...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if line[0] not in dictionary.keys():
            dictionary[line[0]] = {line[1] : [int(x) for x in line[2:]]}
        elif line[0] in dictionary.keys() and line[1] not in dictionary[line[0]].keys():
            dictionary[line[0]].update({line[1] : [int(x) for x in line[2:]]})
        elif line[0] in dictionary.keys() and line[1] in dictionary[line[0]].keys():
            dictionary[line[0]][line[1]] += [int(x) for x in line[2:]]
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = []
    for x in list(all_parties(db)):
        times = 0
        if x in db.keys():
            for _x1, x2 in db[x].items():
                times += len(x2)
        for _y1, y2 in db.items():
            if x in y2.keys():
                times += len(y2[x])
        result.append((x, times))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for x in list(all_parties(db)):
        ...
        for _y1, y2 in db.items():
            if x in y2.keys():
                reciever += len(y2[x])
        result.append((x, caller, reciever))
    return [x[0] for x in sorted(result, key = lambda x : (-x[1], x[2], x[0]))]
