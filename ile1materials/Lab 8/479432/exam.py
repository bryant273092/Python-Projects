...
def read_db(file : open) -> {str:{str:[int]}}:
    get_dict = defaultdict()
    for line in file:
        int_list = []
        split_line = line.strip().split(':')
        for each in split_line[2:]:
            int_list.append(int(each))
        if split_line[0] not in get_dict:
            get_dict[split_line[0]] = {}
            get_dict[split_line[0]][split_line[1]] = int_list
        elif split_line[0] in get_dict:
            get_dict[split_line[0]].update({split_line[1]:int_list})
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for key, value in db.items():
        for k, v in value.items():
            caller_count[key] += len(v)
            rec_count[k] += len(v)
            
    a = sorted(caller_count.items(), key=lambda x: -x[1])
    b = sorted(rec_count.items(), key=lambda x: x[1])
    
    for each in a:
        initiator.append(each[0])
