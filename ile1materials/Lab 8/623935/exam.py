...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for i in file:
        ...
        receiver = [lines[1]]
        time = [lines[2:]]
        d[lines[0]].update(dict(zip(receiver, time)))
    return d
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for i in dcop:
        ...
        for j in dcop[i]:
            second.append(j)         
    for people1 in first:       
        for people2 in second:
            if people1 in people2 and people2 in people1:
                d.update({tuple((people1, people2))})
    return d
