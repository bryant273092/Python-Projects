...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for time in line[2:]:
            time = int(time)
            sub_dict[line[1]].append(time)
        if line[1] in result[line[0]]:
            result[line[0]][line[1]].extend(sub_dict[line[1]])
        else: 
            result[line[0]].update(dict(sub_dict))
    return dict(result)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db:
        for receiver in db[caller]:
            if receiver in db and caller in db[receiver]:
                result.add(tuple(sorted([caller, receiver])))
    return result



def initiators (db : {str:{str:[int]}}) -> [str]:
    frequency = dict(talker_frequency(db))
    info = {}
    for caller in db:
        info[caller] = [0,0]
        for receiver in db[caller]:
            info[caller][0] += len(db[caller][receiver])
        info[caller][1] = frequency[caller] - info[caller][0]
    return sorted(info.keys(), key = lambda x: (-info[x][0],info[x][1],x))
