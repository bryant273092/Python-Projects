...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        temp = line.strip('\n').split(':')
        callers.append(temp[0])
        receivers.append(temp[1])
        #print(temp[2::1])
        calls.append(temp[2::])
    '''
    '''
    ...
    for x in range(len(calls)):
        #print(calls[x])
        for y in range(len(calls[x])):
            calls[x][y] = int(calls[x][y])
    
    for c in range(len(callers)):
        result[callers[c]][receivers[c]] = calls[c]
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    final = list(zip(list(result.keys()), list(result.values())))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db:
        for receiver in db[caller]:
            #print(db[receiver].keys())
            if caller in list(db[receiver].keys()):
                temp = [caller,receiver]
                temp1 = tuple(sorted(temp))
                #temp1 = sorted(temp, key = lambda x: x[0][0])
                #print(temp)
                result.add(temp1) 
    
    #result = sorted(temp, key = lambda x: x[0][0])
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    temp_dict = dict()
    '''
    {NAME: [CALLED, RECEIVED]
    '''
    
    for caller in db:
        times_called = len(list(db[caller].keys()))
        times_received = 0
        for c in db:
            for r in list(db[c].keys()):
                if caller == r:
                    times_received += 1
        
        temp_dict[caller] = (times_called, times_received)
        times_received = 0
    
    temp1 = sorted(temp_dict.items(), key = lambda x: (-x[1][1], x[1][0], x[0][1]), reverse = True) 
    #print(temp1)
        #for receiver in db[caller]:
    #print(list(temp_dict.items()))
    #print(list(temp_dict.items())[0][0][0])
    #print(sorted(temp_dict.items(), key = lambda x: x[0][0][1][0]))
    #temp1_dict = sorted(temp_dict.items(), key = lambda)
    
    for x in range(len(temp1)):
        result.append(temp1[x][0])
        
    return result #sorted(temp_dict.items(), key = lambda x: x[1])
