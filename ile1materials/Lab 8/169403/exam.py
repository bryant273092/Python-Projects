...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        c, r, ts = info[0], info[1], map(lambda x: int(x),info[2:])
        if r in db[c].keys():
            db[c][r].extend(ts)
        else:
            db[c][r] = list(ts)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db:
        for r in db[c]:
            answer[c] += len(db[c][r])
            answer[r] += len(db[c][r])
    return sorted(zip(answer.keys(), answer.values()), key = lambda x: (-x[1], x[0]))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    cal, rec= defaultdict(int), defaultdict(int)
    for c in db:
        for r in db[c]:
            cal[c] += len(db[c][r])
            rec[r] += len(db[c][r])
    return sorted(db.keys(), key = lambda x: (-cal[x], rec[x], x))
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    d = defaultdict(list) 
    for c in sorted(db.keys()):
        for r in sorted(db.keys()):
            tsc,tsr = db[c].get(r), db[r].get(c)
            d[c].append((sum(tsc) if tsc else 0) + (sum(tsr) if tsr else 0))
    return [k for k in d.values()]
