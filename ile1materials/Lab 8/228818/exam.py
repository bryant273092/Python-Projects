...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if line[1] not in d[line[0]]:
            d[line[0]][line[1]]=[int(i) for i in line[2:]]
        else:
            d[line[0]][line[1]].append(int(*line[2:]))
    return dict(d)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db:
        for r in db[c]:
            d[c].append(len(db[c][r]))
            d[r].append(len(db[c][r]))
    return sorted([(name,sum(time)) for name,time in d.items()],key=lambda x:(-x[1],x[0]),reverse=False)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db:
        for c2 in db_copy:
            if c in db_copy[c2] and c2 in db[c]:
                ts.add(tuple(sorted([c,c2],key=lambda x:x,reverse=False)))
    return ts
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for c in db:
        for r in db[c]:
            d[c].append(len(db[c][r])) # call times
            d2[r].append(len(db[c][r])) # receive times
    call_d={name:sum(value) for name,value in d.items()}
    receive_d={name:sum(value) for name,value in d2.items()}
    return sorted([name for name in db],key=lambda x:(call_d[x],-receive_d[x]),reverse=True)

# def matrix (db : {str:{str:[int]}}) -> [[int]]:
#     name_list=sorted(list(all_parties(db)))
#     matrix=[[0]*len(all_parties(db))]*len(all_parties(db))
#     db_copy=db
#     ts=[]
#     for c in db:
#         for c2 in db_copy:
...
#             ts+=[[c,c2,c_to_c2]]
#     for row in ts:
#         name_list.index(row[0])
#         print(name_list.index(row[0]),name_list.index(row[1]))
#         matrix[name_list.index(row[0])][name_list.index(row[1])]+=row[2]
#         print(matrix[name_list.index(row[0])][name_list.index(row[1])])
#     print(ts)
#     return matrix
#     
