...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        caller = args.pop(0)
        receiver = args.pop(0)
        if caller in db.keys():
            if receiver in db[caller].keys():
                for n in args:
                    db[caller][receiver].append(int(n))
            else:
                db[caller][receiver] = [int(n) for n in args]
        else:
            db[caller] = {receiver: [int(n) for n in args]}
    return db
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    freq = []
    for person in all_parties(db):
        count = 0
        for caller in db.keys(): 
            if person == caller:
                for receiver in db[caller]:
                    count += len(db[caller][receiver]) 
            else:
                for receiver in db[caller].keys():
                    if person == receiver:
                        count += len(db[caller][receiver])
        freq.append((person,count))
    return sorted(sorted(freq, key= lambda n: n[0], reverse= False), key = lambda n: n[1], reverse= True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        for receiver in db[caller].keys():
            if receiver in db.keys():
                if caller in db[receiver].keys():
                    l = sorted([caller, receiver])
                    mutual.add((l[0], l[1]))
    return mutual
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for person in all_parties(db):
        ...
        for caller in db.keys():
            if caller == person:
                for receiver in db[caller].keys():
                    callerCount += len(db[caller][receiver])
            else:
                for receiver in db[caller].keys():
                    if receiver == person:
                        receiverCount += len(db[caller][receiver])
        people[person] = (callerCount, receiverCount)
    
    sortedAlpha = sorted([p for p in people.keys()])
    sortedReceiver = sorted(sortedAlpha, key= lambda p: people[p][1])
    sortedCaller = sorted(sortedReceiver, key= lambda p: people[p][0], reverse= True)
    return sortedCaller
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    matrix = []
    for person in sorted(all_parties(db)):
        row = []
        for person2 in sorted(all_parties(db)):
            if person == person2:
                row.append(0)
            else:
                col = 0
                if person2 in db[person].keys():
                    for minutes in db[person][person2]:
                        col += minutes
                if person2 in db.keys():
                    if person in db[person2].keys():
                        for minutes in db[person2][person]:
                            col += minutes 
