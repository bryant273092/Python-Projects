...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        sweet=line.split(':')
        newd[sweet[0]]={}
        el=[int(n) for n in sweet[2:]]
        niced= {sweet[1]:el}
        if sweet[0] in newd.keys():
            newd[sweet[0]]=niced
        else: newd[sweet[0]]=niced
        #edict[sweet.pop(0)]=el
    return dict(newd.items())
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k, v in db.items():
        vlist=[]
        for n in v.values():
            for x in n: vlist.append(x)
        v2list=[]
        for c,d in v.items():
            for n in d: v2list.append(n)
        if k not in news:
            news.append((k,len(vlist)))
        if c not in news:
            news.append((c,len(v2list)))
        elif c in news:
            news.add(len(v2list))
    return news
