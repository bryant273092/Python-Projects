...
def read_db(file : open) -> {str:{str:[int]}}:
    result,number = defaultdict(dict),[]
    for line in file:
        data = line.rstrip().split(':')
        for x in data[2:]: number.append(int(x))
        if data[0] in result:
            if data[1] in result[data[0]]:
                for x in number:result[data[0]][data[1]].append(x)
                number = []
            else:
                result[data[0]].update({data[1]:number})
                number = []
        else: 
            result[data[0]] = {data[1]:number}
            number = []
    return dict(result)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k,v in db.items():
        for key,value in v.items():
            if k in temp:
                temp[k] = temp[k] + len(value)
            if key in temp:
                temp[key] = temp[key] + len(value)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k,v in db.items():
        for key,value in v.items():
            temp.add((k,key))
    for k,v in temp:
        if (k,v) and (v,k) in temp: result.add(tuple(sorted((k,v))))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k,v in db.items():
        for key,value in v.items():
            if k in temp:
                temp[k] = (temp[k][0] +len(value) , temp[k][1])
            if key in temp:
                temp[key] =  (temp[key][0] , temp[key][1]+len(value))
    result = sorted(temp,key = lambda x :(temp[x][0],-temp[x][1]),reverse = True)
    return result
