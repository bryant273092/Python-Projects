...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    result = set()
    for caller, receiver_minutes in db.items():
        for receiver,minutes in receiver_minutes.items():
            result.add(receiver)
    return result
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        while(True):
            if len(data)>0:
                temp.append(int(data.pop(0)))
            else:
                break
        if caller not in result:
            result[caller] = {receiver:temp}
        else: 
            result[caller][receiver] = temp
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for name in name_set:
        ...
        for caller, receiver_minutes in db.items():
            for receiver,minutes in receiver_minutes.items():
                if caller==name or receiver==name:
                    for m in minutes:
                        total+=1
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for name in name_set:
        ...
        for caller, receiver_minutes in db.items():
            for receiver,minutes in receiver_minutes.items():
                if name in receiver_minutes.keys():
                    save = caller
                    temp+=1
                if save in db[caller]:
                    temp+=1
            if temp==2:
        ...
        for caller, receiver_minutes in db.items():
            for receiver,minutes in receiver_minutes.items():
                ...
                if receiver==name:
                    for m in minutes:
                        total_received+=1
        temp.add((name,total_caller,total_received))
    for n,c,r in sorted(temp,key=lambda x:(-x[1],x[2])):
        result.append(n)
    return result
