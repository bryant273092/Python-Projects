...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    reciever = defaultdict(int)
    for key,value in db.items():
        for k,v in value.items():
            caller[key] += len(v)
            reciever[k] += len(v)
    sc = sorted(caller.items())
    sr = sorted(reciever.items())
    for i in range(len(sc)):
        answer.append((sc[i][0],sc[i][1],sr[i][1]))
    for h in sorted(answer,key = lambda x: (-x[1],x[2],x[0])):
        ret.append(h[0])
    return ret
