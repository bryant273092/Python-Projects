...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if line[0] not in calldict:
            call = {line[0]: {line[1]: numbers}}
            calldict.update(call)
        else:
            call = {line[1]: numbers}
            for k,v in calldict.items():
                if line[0] == k:
                    calldict[k].update(call)
    return calldict
        ...
        for name,mins in v.items():
            ...
            if name in total:
                for tup in total:
                    ...
                    print('tup[0]',tup(0))
                    if tup[0] == name:
                        new_tuple = name,counter + int(tup[1])
                        total.append(new_tuple)
                        total.remove(tup)
            else:
                total.append((k,counter))
    print(total)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:####
    ...
    for k,v in db.items():
        for name, mins in v.items():
            if name in db.keys() and k in db[name]:
                called = [name, k]
                names.update((called))
    return names
