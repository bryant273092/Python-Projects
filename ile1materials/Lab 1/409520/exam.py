...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    call_set = set()
    for call in db:
        for receive in db[call]:
            call_set.add(receive)
        call_set.add(call)
    return(call_set)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        record = line.strip('\n').split(':')
        if record[1] in final[record[0]]: final[record[0]][record[1]].extend(list(int(i) for i in record[2:]))
        else: final[record[0]][record[1]] = list(int(i) for i in record[2:])
    return (dict(final))
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for call in db:
        for receive in db[call]:
            call_count[call] += len(db[call][receive])
            call_count[receive] += len(db[call][receive])
    return sorted([(x,call_count[x])for x in call_count],key = (lambda x: (x[1]*-1,x[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    final = set()
    for call in db:
        for receive in db[call]: #if receive in db and 
            if (receive in db) and (call in db[receive]):
                final.add(tuple(sorted((call, receive))))
    return(final)



def initiators (db : {str:{str:[int]}}) -> [str]:
    call_count = defaultdict(list)
    for call in db:
        for receive in db[call]:
            if call not in call_count:
                call_count[call] = [0,0]
            if receive not in call_count:
                call_count[receive] = [0,0]
            call_count[call][0] += len(db[call][receive])
            call_count[receive][1] += len(db[call][receive])
    return sorted([i for i in call_count],key= (lambda x:(call_count[x][0]*-1,call_count[x][1])))

def matrix (db : {str:{str:[int]}}) -> [[int]]:
    name_ord,matrix = set(),defaultdict(list)
    for call in db: 
        name_ord.add(call)
        for receive in db: name_ord.add(receive)
    name_ord = sorted(name_ord) # get ordered name
    for name in name_ord: matrix[name] = [0 for i in range(len(name_ord))] #set up matrix
    for call in db:
        for receive in db[call]:
            matrix[call][name_ord.index(receive)] += sum(db[call][receive])
            matrix[receive][name_ord.index(call)] += sum(db[call][receive])
    return[matrix[i] for i in sorted(matrix)]
