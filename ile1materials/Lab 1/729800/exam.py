...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    caller_log = set()
    for caller, reciever in db.items():
        if caller not in caller_log:
            caller_log.add(caller)
            for reciever1, time in reciever.items():
                if reciever1 not in caller_log:
                    caller_log.add(reciever1)
        else:
            for reciever1, time in reciever.items():
                if reciever1 not in caller_log:
                    caller_log.add(reciever1)         
...
def read_db(file : open) -> {str:{str:[int]}}:
    #bug in not being able to append value to list of already existing caller
    new_db = defaultdict(dict)
    for line in file:
        call = line.strip('\n').split(':')
        if call[1] in new_db[call[0]]:
            for calls in call[2:]:
                new_db[call[0]][call[1]].append(eval(calls))
        else:
            new_db[call[0]][call[1]] = [eval(x) for x in call[2:]]
    return dict(new_db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller, reciever in db.items():
        count = 0
        for caller1 in call_log:
            for reciever1, calls in reciever.items():
                if caller not in caller1:
                    call_log.append(tuple([caller, len(db[caller][reciever1])]))
        else: 
            call_count = call_log[count][1] + len(db[caller][reciever1])
            del call_log[count]
            call_log.append(tuple([caller, call_count]))
        count +=1     
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    caller_set = set()
    for caller, reciever in db.items():
        for caller_2, call_number in reciever.items():
            if caller in db[caller_2]:
                for pair in caller_set:
                    if caller in pair and caller_2 in pair:
                        break
                else:
                    caller_set.add((tuple(sorted([caller, caller_2]))))
    return caller_set
