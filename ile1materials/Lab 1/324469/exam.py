...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for item in file:
        caller = item.split(':')[0]
        revi = item.split(':')[1]
        time = item.strip().split(':')[2:]
        timer = [int(time) for time in time]
        read_dict[caller][revi] = timer
    return read_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key, value in db.items():
        for name, time in value.items():
            talker[key] += len(time)
            talker[name] += len(time)
    return sorted(talker.items(), key = (lambda x: (-x[1], x[0])))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ini = defaultdict(int)
    inti = defaultdict(int)
    for key, value in db.items():
        for name, time in value.items():
            ini[key] += (len(time))
            inti[name] += (len(time))
    return sorted(name, sorted(ini.values(), key = (lambda x: -x[0])) for name in sorted(inti.values(), key = (lambda x: x[0])))
