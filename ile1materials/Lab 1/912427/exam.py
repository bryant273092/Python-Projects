...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    set_of_names = set()
    for receiver_number in db.values():
        for name in receiver_number.keys():
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    list_of_all_items = []
    for line in file:
        list_of_items = line.rstrip("\n").split(":")
        list_of_all_items.append(list_of_items)
    temp_dict = defaultdict(list)
    for x in list_of_all_items:
        temp_dict[x[0]].append((x[1:len(x)]))
    for caller in temp_dict.keys():
        new_list = temp_dict.get(caller)
        temp_dict1 = {}
        for x in new_list:
            even_new_list = []
            for y in range(1, len(x)):
                even_new_list.append(y)
                temp_dict1[x[0]] = even_new_list
            final_dict[caller] = temp_dict1 
    return final_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for x in db.values():
        for y in x.items():
            number_of_calls[y[0]] += len(y[1])
    for x in db.keys():
        dict_inside = db.get(x)
        for y in dict_inside.items():
            number_of_calls[x] += len(y[1])
    return sorted(sorted(number_of_calls.items(), key = lambda x: x[0]), key = lambda x: x[1], reverse = True)
        



def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    mutual_pairs = defaultdict(list)
