...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    everyone = set()
    for c,r in db.items():
        if c not in everyone:
            everyone.add(c)
        for person, times in r.items():
            if person not in everyone:
                everyone.add(person)
    return everyone
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        another_dict={text[1]: int_list}
        call_dict[text[0]].add(text[1])
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:            
    ...
    for c,r in db.items():
        counter = 0 
        for person,time in r.items():
            counter += len(time)
        freq_dict[c].add((c, counter))
