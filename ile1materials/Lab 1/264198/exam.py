...
def read_db(file : open) -> {str:{str:[int]}}:
    db_dict_outer = defaultdict(lambda : defaultdict(list))
    db_dict_inner = defaultdict(list)
    for line in file:
        data = line.strip().split(':')
#         if data[0] not in db_dict_outer.keys():
#             db_dict_inner.values() = {}
        db_dict_inner[data[1]] = data[2:]
        db_dict_outer[data[0]] = db_dict_inner
    return db_dict_outer
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller, log in db.items():
        call_info[caller] = 0
        for _, time in log.items():
            call_info[caller] += len(time)
    for name in call_info:
        for _, l in db.items():
            for r, t in l.items():
                if name == r:
                    call_info[name] += len(t)
    i = [(person, frequency) for person, frequency in call_info.items()]  
    return sorted(i, key = lambda t: (-t[1], t[0]))
