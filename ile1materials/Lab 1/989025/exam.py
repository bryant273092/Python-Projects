    ...
    for caller, value in db.items():
        if caller not in result.keys():
            ...
            for r,time in value.items():
                for r1 in r:
                    if r1 in caller:
                         result[caller] += len(time)
                         
    return sorted(sorted([(k, v) for k, v in result.items()], key = lambda x:x[0]), key = lambda x:x[1], reverse = True)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller, value in db.items():
        for r, time in value.items():
            result[caller] = len(time)
    return sorted([k for k,v in result.items()], key = lambda x:x[1], reverse = True)
