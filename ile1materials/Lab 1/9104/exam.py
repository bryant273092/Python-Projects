...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    'calls on a dictionary and returns a set of all callers and recivers'
    caller_set= set()
    for c in db.keys():
        for r in db[c].keys():
            caller_set.add(r)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if calls[-1] == '\n':
            calls[-1] = calls[-1][:-1]
        for ints in calls[2:]:
            if calls[0] in caller_dict.keys():
                if calls[1] in caller_dict[calls[0]].keys():
                    caller_dict[calls[0]][calls[1]].append(int(ints))
                else:
                    caller_dict[calls[0]][calls[1]] = []
                    caller_dict[calls[0]][calls[1]].append(int(ints)) 
            else:
                caller_dict[calls[0]] = dict()
                if calls[1] in caller_dict[calls[0]].keys():
                    caller_dict[calls[0]][calls[1]].append(int(ints))
                else:
                    caller_dict[calls[0]][calls[1]] = []
    ...
    for c in caller_set:
        ...
        for keys in db.keys():
            for key in db[keys].keys():
                if key == c:
                    ts += len(db[keys][key])
        frequency_list.append((c, ts))
    return sorted(sorted(frequency_list, key = (lambda x: x[0])), key = (lambda x: x[1]), reverse = True)

def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    mutual_callerset = set()
    for c in db.keys():
        for r in db[c].keys():
            if len(db[c][r]) >= 1:
                if c in db[r].keys():
                    if len(db[r][c]) >= 1:
                        if (r, c) not in mutual_callerset:
                            if c < r:
                                mutual_callerset.add((c,r))
                            else:
                                mutual_callerset.add((r,c))
    mclist = sorted(sorted(mutual_callerset, key = (lambda x: x[0])), key =(lambda x: x[1]), reverse = True)
    final_set = set()
    for i in mclist:
        final_set.add(i)
    return final_set
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    times_called = defaultdict(dict)
    for c in db.keys():
        for r in db[c].keys():
            if r not in times_called[c].keys():
                times_called[c][r] = 0
                times_called[c][r] += len(db[c][r])
            else:
                times_called[c][r] += len(db[c][r])
    print(times_called)
