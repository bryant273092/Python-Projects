...
db = {'Alan': {'Barb': [10, 20], 'Carl': [10, 10]}, 'Barb': {'Alan': [10, 10], 'Deja': [5, 5, 5]}, 'Carl': {'Alan': [10, 5], 'Deja': [15], 'Elan': [5, 5]}}
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if c not in result.keys():
            inner = dict()
            inner[r] = [int(time) for time in t]
            result[c] = inner
        else:
            
            if r not in result.get(c).keys():
                result[c][r] = [int(time) for time in t]
            else:
                for entry in t:
                    result[c][r].append(int(entry))
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    #list of two tuples (name, number of calls participated)
    # sort decreasing order of calls and INCREASING ALPHABETICAL
    result = []

            
    draft_dict = {}
    for k,v in db.items():
        if k not in draft_dict.keys():
            for i in v.values():
                for time in i:
                    draft_dict[k] +=1
            ...
            for keys in v.keys():
                ...
                if keys not in draft_dict.keys():
                    draft_dict[keys] = len(db.get(keys).values())
                else:
                    draft_dict[keys]+=len(db.get(keys).values())
        else:
            draft_dict[k]+= len(db.get(keys).values())
           
    
    for key,val in draft_dict.items():
