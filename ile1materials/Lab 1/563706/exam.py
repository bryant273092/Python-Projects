...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    return_set=set()
    for x in db:
       for name in(db[x].keys()):
           name_list.append(name)
    for name in name_list:
        if name not in return_set:
            return_set.add(name)
    return(return_set)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        first=line[0]
        second=line[1]
        return_dict[first][second]=[]
        for num in line[2:]:
            num_list.append(num)
        for num in num_list:
            return_dict[first][second].append(int(num))
            temp=(return_dict[first][second])
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for reciever in db[caller]:
            return_dict[caller]+=len(db[caller][reciever])
            return_dict[reciever]+=len(db[caller][reciever])
            
    for value in return_dict.items():
        value=tuple(value)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db:
        for reciever in db[caller]:
            call_list.append((caller, reciever))    
    for name in call_list:
        first=name[0]
        second=name[1]
        if (second,first) in call_list:
                if (second, first) not in master_list:
                    if second>first:
                        master_list.append((first,second))
                    else:
                        master_list.append((second,first))
    return (set((master_list)))



def initiators (db : {str:{str:[int]}}) -> [str]:
    recieve_dict=defaultdict(int)
    ...
    for caller in db:
        for reciever in db[caller]:
            calls=(len(db[caller][reciever]))
            call_dict[caller]+=calls
            recieve_dict[reciever]+=calls
    a=dict(call_dict)
    b=dict(recieve_dict)
    for item in b.items():
        recieve_list.append(item)
    for item in a.items():
        finallist.append(item)
    finallist=(sorted(finallist,key=lambda x: -x[1]))
    for name in finallist:
        newlist.append(name[0])
    for name in finallist:
        cey=name[0]
        bigset.add((cey,int(call_dict[cey]),int(recieve_dict[cey])))
    bigset=(sorted(bigset, key= lambda x: (-x[1],x[2])))
    for name in bigset:
