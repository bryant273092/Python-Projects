...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller,reciever in db.items():
        if caller in db.items():
            party_set.add(caller)
        for call in reciever:
            if call in db.get(caller):
                party_set.add(call)
    return party_set
        ...
        for element in line:
            try:
                if type(int(element))==int:
                    call_list.append(int(element))
            except ValueError:
                if line[0] not in db:
                    db[line[0]]
                elif line[0] in db and element!=line[0]:
                    db[line[0]]=inner_dict
                    inner_dict[element]=call_list
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    talker=defaultdict(int)
    frequency=[]
    for call,recieve in db.items():
        for person in db.get(call):
            talker[call]+=len((db.get(call)).get(person))
        for person in recieve:
            talker[person]+=len(recieve.get(person))
    for pair in dict(talker).items():
        frequency.append((pair[0],pair[1]))
    return sorted(sorted(frequency, key=lambda x: (x[0]) ), key=lambda y: y[1],reverse=True )
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for person,calls in db.items():
        for reciever in calls:
            inner_dict[reciever]+=len(calls.get(reciever))
            outer_list.append((person,(reciever,inner_dict[reciever])))
    
    for pair in outer_list:
        if pair[1][1]>1:
            mutual.add((pair[0],pair[1][0]))
    return (mutual)
