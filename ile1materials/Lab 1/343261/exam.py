...
dbo= {"Alan": {'Barb': [10,20], "Carl": [10,10]}, "Barb": {'Alan':[10,10], 'Deja':[5,5,5]}}

def all_parties(db : {str:{str:[int]}}) -> {str}:
    emptyset=[]
    for k,v in db.items():
        if k not in emptyset:
            emptyset.append(k)
        for s,t in v.items():
            if s not in emptyset:
                emptyset.append(s)
    return (set(emptyset))
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    lo=[]
    newfile= open(file)
    string_file= (newfile.readlines())
    for ch in string_file:
        lo.append(ch.split(":"))
    for lis in lo:
        k[lis[0]]= p[lis[1]]= lis[2:]
    print(k)
#read_db("db1.txt")
...
print(talker_frequency(dbo))

def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
 #Spent too much time on read_db :/
