...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        args = line.split(':')
        db.setdefault(args[0], dict())
        db[args[0]].setdefault(args[1], [])
        for arg in args[2:]:
            db[args[0]][args[1]].append(int(arg))
    return db
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        calls = 0
        for repeat_caller in db:
            if repeat_caller == caller:
                for caller_reciever in db[caller]:
                    calls += len(db[repeat_caller][caller_reciever])
            for reciever in db[repeat_caller]:
                    if reciever == caller:
                        calls += len(db[repeat_caller][reciever])
        freq.append((caller, calls))
    return sorted(freq, reverse=True, key=lambda x: x[1])
        ...
        for reciever in db[caller]:
            if reciever in db:
                if caller in db[reciever]:
                    if (len(db[caller][reciever]) >= 1) and (len(db[reciever][caller]) >= 1):
                        pair = sorted([caller, reciever])
                        mutual.add(tuple(pair))
    return mutual
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    data = dict()
    for caller in db:
        data.setdefault(caller, [0, 0])
        data[caller][0] += 1
        for reciever in db[caller]:
            data[caller][0] += 1
            if caller in reciever:
                data[caller][1] += 1
            else:
                data.setdefault(reciever, [0, 0])
                data[reciever][1] += 1
    listing = sorted(list(data), reverse=True, key=lambda x: data[x][0])
    return listing
