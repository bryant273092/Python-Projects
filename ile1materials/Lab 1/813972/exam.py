...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    result = set()
    for k in db.keys():
        for inner_key in db[k].keys():
            result.add(k)
            result.add(inner_key)
    return result
    ...
    for line in file.readlines():
        ...
        for i in range(2,len(line),1):
            ...
            if line[0] not in result:
                ...
                result[line[0]].update({line[1]:line[2:]})
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key in db:
        for inner_key in db[key]:
            sum = 0
            sum += len(db[key][inner_key])
            for a in db:
                if inner_key in db[a].keys():
                    sum += len(db[a][inner_key])
            if key not in done:
                done.append(key)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    filter = []
    for key in db:
        for value_key in db[key].keys():
            lst.append([key,value_key])
    for ele in lst:
        for other_ele in lst:
            if ele[0] == other_ele[1] and ele[1] == other_ele[0]:
                filter.append(ele)
    for ele in filter:
        ele.sort()
        result.add(tuple(ele))
    return result

def initiators (db : {str:{str:[int]}}) -> [str]:
    return sorted(db.items(),key = lambda x:len(x[1]),reverse = True)
