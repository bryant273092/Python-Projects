...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    returnset = set()
    for caller, receiver in db.items():
        returnset.add(caller)
        for person in receiver.keys():
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in linelist:
        ...
        if line[1] in returndict[line[0]].keys():
            returndict[line[0]][line[1]].extend(intlist)
        else:
            returndict[line[0]].update({line[1]:intlist}) #DB1.TXT DOES NOT MATCH THE DB1.TXT ON PAPER == CRITICAL ERROR
    return dict(returndict)


def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    returnlist = []
    for person,outgoing in db.items():
        counter = 0
        for receive in db.values():
            for receiver,amount in receive.items():
                if person == receiver:
                    counter += len(amount)
        for receive in outgoing.values():
            counter += len(receive)
        returnlist.append((person, counter))
    return sorted(returnlist, key=lambda x:(x[1],x), reverse = True) #MUST MAKE ALPHABETICAL

def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    returnset = set()
    for caller, receive in db.items():
        for called in receive.keys():
            if caller in db[called].keys():
                sorttuple = sorted([caller, called])
                returnset.add(tuple(sorttuple))
    return returnset


def initiators (db : {str:{str:[int]}}) -> [str]:
    callers = {} #Is a dictionary whose keys are people and values are a tuple of calls made and calls received
    for caller,calls in db.items():
        callsmade = 0
        callsreceived = 0
        for time in calls.values():
            callsmade += len(time)
        for pcalls in db.values():
            if caller in pcalls.keys():
                callsreceived += len(pcalls[caller])
        callers[caller] = (callsmade,callsreceived)
    return sorted(list(callers.keys()), key=lambda  x:(x[1],x[2],x)) #MUST SORT IN REVERSE RECEIVEING
