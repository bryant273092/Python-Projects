...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    parties = set()
    for c,r_dict in db.items():
        for r,ts in r_dict.items():
            if c not in parties:
                parties.add(c)
            else:
                parties.add(r)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        c,r,ts = line_list[0], line_list[1], line_list[2:]
        new_ts = [int(t) for t in ts]
        db_dict[c][r] = new_ts
#        print(new_ts)
#        print(c, r, ts)
    return db_dict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    c_talker = defaultdict(list)
    r_talker = defaultdict(list)
    tot_calls = 0
    for c,r_dict in db.items():
        for r,ts in r_dict.items():
            c_talker[c].append(len(ts))
            r_talker[r].append(len(ts))
#            print(r_dict.keys())
#            if c in r_dict.keys():
 #               tot_calls += len(ts)
 #               talker_list.append((c,tot_calls))
#            elif r == c:
#                tot_calls += len(ts)
#                talker_list.append((c,tot_calls))
#            else:
...
#            len(ts)
#            print(c, r_dict.keys())
#            print(c in r_dict.keys())
#            if c in r_dict.keys() and r in db.keys():
#                if len(ts) >= 1:
 #                   mutual.add((c,r))
#    return mutual
