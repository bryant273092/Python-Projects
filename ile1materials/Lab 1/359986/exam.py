...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        line = line.rstrip()
        s_line = line.split(':')
        if not s_line[0] in result.keys():
            result[s_line[0]] = dict()
        if not s_line[1] in result[s_line[0]].keys():
            result[s_line[0]][s_line[1]] = []
        for time in s_line[2:]:
            result[s_line[0]][s_line[1]].append(int(time))
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for receiver in db[caller]:
            if not caller in result.keys():
                result[caller] = 0
            if not receiver in result.keys():
                result[receiver] = len(db[caller][receiver])
                result[caller] += len(db[caller][receiver])
            else:
                result[receiver] += len(db[caller][receiver])
                result[caller] += len(db[caller][receiver])
    #result = ((v,k) for v,k in result.items())
    return sorted(sorted(((v,k) for v,k in result.items()),key = lambda x:x[0]),key = lambda x:x[1],reverse=True)
    ...
    for caller in db:
        for receiver in db[caller]:
            if not caller in result.keys():
                result[caller] = [0,0]
            if not receiver in result.keys():
                result[receiver] = [0,len(db[caller][receiver])]
                result[caller][0] += len(db[caller][receiver])
            else:
                result[receiver][1] += len(db[caller][receiver])
                result[caller][0] += len(db[caller][receiver])
    return sorted(sorted(sorted(result.keys()),key = lambda x:result[x][1]),key = lambda x:result[x][0],reverse = True)



def matrix (db : {str:{str:[int]}}) -> [[int]]:
    result = dict()
    for caller in sorted(db.keys()):
        if not caller in result:
            result[caller] = []
        for receiver in sorted(db.keys()):
            minutes = 0
            if receiver in db[caller]:
                for i in db[caller][receiver]:
                        minutes += i
            if caller in db[receiver]:
                for i in db[receiver][caller]:
                    minutes += i
            result[caller].append(minutes)
