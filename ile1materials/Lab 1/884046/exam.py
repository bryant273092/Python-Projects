...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    participant_set = {k for k in db}
    for v in db.values():
        for sk in v:
            participant_set.add(sk)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        line = line.rstrip('\n').split(':')
        call_times = [int(t) for t in line[2:]]
        rvd[line[1]]=call_times
        db_dict[line[0]]=rvd
    return db_dict        
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for name in all_parties(db):
        ...
        for v in db.values():
            for rn in v.keys():
                if rn == name:
                    talker_freq+=len(v.get(rn))
        tft.append((name,talker_freq))
    return tft
