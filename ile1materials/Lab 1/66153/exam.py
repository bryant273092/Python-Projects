...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for lines in file:
        inner_list = []
        intro_list = lines.strip().split(':')
        for i in range(2,len(intro_list)):
            inner_list.append(int(intro_list[i]))
            result[intro_list[0]][intro_list[1]] = inner_list
    return dict(result) 
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = []
    inner_result = defaultdict(int)
    for i in db.items():
        for items,x in i[1].items():
            ch = len(x)
            if items not in inner_result.keys():
                inner_result[items] = ch
            else:
                inner_result[items] += ch 
    return dict(inner_result)
