...
#         print(line)
        ...
        if line[0] in list(d.keys()):
            if line[1] in list(d[line[0]].keys()):
                for t in ts:
                    d[line[0]][line[1]].append(t)
            else:
                d[line[0]].update({line[1]:ts})
        else:
            d.update({line[0]:{line[1]:ts}})
#         print(d)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    all_party = all_parties(db)
    tf = list()
    
    for names in all_party:
        outer_count = 0 
        for ts in db[names].values():
            outer_count += len(ts)
        ...
        tf.append((names,outer_count+inner_count))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    intor = list()
    for tple in sorted(tf, key= lambda x : (-x[1][0],x[1][1])):
        intor.append(tple[0])
