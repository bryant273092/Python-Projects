...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if elements[0] not in dictionary: # key assigns a new dictionary inside
            dictionary[elements[0]] = {elements[1] : [int(e) for e in elements[2:]]}
        elif elements[1] not in dictionary[elements[0]]: # inside dictionary assigns a list
            dictionary[elements[0]][elements[1]] = [int(e) for e in elements[2:]]     
        else:
            dictionary[elements[0]][elements[1]].append(int(elements[2]))
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k in db:
        calls = 0
        for dbv in db[k].values(): # {[10, 20], [10, 10]}
            calls += len(dbv)
            
        result.append((k,calls))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for callers in db:
        for receivers in db[callers]:
            if (receivers in db[callers]) and (callers in db[receivers]) and ((receivers, callers) not in result):
                if callers[0] < receivers[0]:
                    result.add((callers, receivers))
                else:
                    result.add((receivers, callers))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for callers in db:
        calls = 0
        receives = 0
        for receivers in db[callers].values():
            calls += len(receivers)
            
        for all_receivers in db.values():
            if callers in all_receivers:
                receives += 1
            
        result.append((callers, calls, receives))
    
    for a,b,c in sorted(result, key = lambda i : -i[1]):
