...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for people in db.values():
        for receivers in people.items():
            s_everyone.add(receivers[0])
...
def read_db(file : open) -> {str:{str:[int]}}:
    d_callers = defaultdict(dict)
    
    for line in file:
        the_caller = []
        split_lines = line.rstrip().split(':')
        the_caller.append(split_lines[0])
        #print(split_lines)
        
        for i in range(1, 2):
            minutes = []
            minutes.extend(split_lines[(i+1):])
            
            for x in range(len(minutes)):
                minutes[x] = int(minutes[x])
        
            #print('minutes', minutes)
            d_callers[the_caller[0]][split_lines[i]] = minutes
            #print(dict(d_callers))
        
    #print('dict', dict(d_callers))
    return dict(d_callers)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for callers in db.items():
        sum1 = 0
        #print('callers', callers)
        for min in callers[1].values():
            sum1 += len(min)
        d_sum[callers[0]] += sum1
            
        #print('sum', sum1)
    
    for key in db.items():
        #print(key)
        for d in key[1].items():
            sum2 = 0
            sum2 += len(d[1])
#             print('second dict', d)
#             print('sum2', sum2)
    ...
    for x, y in d_sum.items():
        l_talkfreq.append((x, y))
    
    return sorted(sorted(l_talkfreq, key = lambda x: x[0]), key = lambda y: y[1], reverse = True)


def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    pass
#     draft_set = defaultdict(set)
#     bffs = set()
#     track_caller = []
#     count2 = 0
#     
...
#         print('ppl', ppl)
#         
#         for p2 in ppl[1].items():
#             count1=0
#             count1 += len(p2)
#             track_caller.append((ppl[0], p2[0], count1))
#             #print('count1', count1)
#     
#     print('track calls', track_caller)
#             
#     for x, y, z in track_caller:
#         #print()
#         #print('xyz', x, y, z) 
#         
#         if z >= 1:
#             draft_set[x].add(y)
#     
#     print('draft set', draft_set)
# 
# #     for x, y in draft_set:
# #         print('elements of draft set', draft_set)
# #         pass
#     pass
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for callers in db.items():
        placehold.append(callers[0])
        sum1 = 0
        
        for min in callers[1].values():
            sum1 += len(min)
            
        d_sums[placehold[0]] = sum1
        print(d_sums)
        
#     for callers in db.items():
#         for rec in callers[1].items():
#             calls = d_sums[placehold]
#             if placehold == rec[0]:
#                 d_sums[placehold] = (calls, len(rec[1]))
#                 print('d_sums before', d_sums)
#             print('rec: ', rec)
#     
    #print('d_sums', d_sums)
...
#     
#     print('db', db)
#     participants = all_parties(db)
#     file = input('Open the file: ')
#     d_rep = read_db(open(file))
#     talk_freq = talker_frequency(db1)
#     mutual_friends = mutual_callers(db1)
#     ini = initiators(db1)
#     
#     print('finished my tests here')
#     print('-----------------------------------------')
#     print()
