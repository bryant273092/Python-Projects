...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        for num in range(len(new_line)):
            if num>1:
                new_line[num]=int(new_line[num])
        ts=[]
        ts.append(new_line[2:])
        adding={new_line[1]:ts[0]}
        c_dict[new_line[0]].update(adding)
    return dict(c_dict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    c_list=[]
    for person in c_r:
        c_list.append((person,c_r[person]))
    return sorted(sorted(c_list,key=lambda x:(x[0]),reverse=False),key=lambda x: x[1],reverse=True)
    ...
    for c in db:
        for r in db[c]:
            ...
            if r not in c_r:
                ...
                c_r[r]+=len(db[c][r])
    d=dict(c_r)
    sorted_d=sorted(sorted(d,key=lambda x: x),key=lambda x: d[x])
    return sorted_d
...
if __name__ == '__main__':
    ...
    #print(initiators(db2))
    #print(read_db(open('db1.txt')))
