...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    parties = []
    for caller in db:
        parties.append(caller)
    for recievers in db.values():
        for reciever in recievers:
            parties.append(reciever)
    return set(parties)
...
def read_db(file : open) -> {str:{str:[int]}}:
    db = defaultdict(list)
    for line in file:
        call_info = line.strip('\n').split(':')
        db[call_info[0]].append({call_info[1]: call_info[2:]})
    return dict(db)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db.keys():
        for receiver in db[caller]:
            if caller in db[receiver] and receiver in db[caller]:
                if (caller,receiver) and (receiver,caller) not in mutuals:
                    mutuals.add((caller,receiver))
    return mutuals
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db.keys():
        ...
        r = 0
        for receiver in db[caller]:
            c += len(db[caller][receiver])
        for cal in db.keys():
            for rec in db[cal]:
                if caller == rec:
                    r += len(db[cal][rec])
        temp_dict[caller] = [c,r]
