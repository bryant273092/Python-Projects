...
from copy import deepcopy   # If you choose to use defaultdict  
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for i in names:
        new_dict[i]=deepcopy(inner_dict)
    file.seek(0)
    for i in file.readlines():
        words=i.strip().split(':')
        for a in words[2:]:
            new_dict[words[0]][words[1]].append(int(a))
    for i in new_dict:
        new_dict[i]=dict(new_dict[i])   
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for i in db:
        for j in db[i]:
            calls[i]+=len(db[i][j])
            calls[j]+=len(db[i][j])
    call_list=[(a,b) for a,b in calls.items()]
    return sorted(call_list,key=lambda x:(-x[1],x[0]))    
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for i in db:
        for j in db[i]:
            if i in db[j] and len(db[j][i]) > 0:
                mutual.append(tuple(sorted([i,j])))
    return set(mutual)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for i in db:
        for j in db[i]:
            callers[i]+=len(db[i][j])
            receivers[j]+=len(db[i][j])
    return sorted(callers,key=lambda x: (-callers[x],receivers[x]))
