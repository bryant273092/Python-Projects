...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        temp_dict,temp_list = dict(),list()
        for min in range(2,len(current_line)):
            temp_list.append(int(current_line[min]))
        if current_line[1] not in temp_dict:
            temp_dict[current_line[1]] = temp_list
        else:
            result[current_line[0]][current_line[1]].append(temp_list)
        if current_line[0] not in result:
            result[current_line[0]] = temp_dict
        else:
            result[current_line[0]][current_line[1]] = temp_list
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = list()
    for looping in db.keys():
        counter = 0
        for values in db[looping]:
            counter+= (len(db[looping][values]))
        for v in db.values():
            for v1 in v:
                if v1 == looping:
                    counter+= len(v[v1])
        #temp_list.append((looping, counter)
        result.append((looping,counter))
    return sorted(result, key = (lambda x : (-(x[1]),x[0])), reverse= False)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for looping in db.keys():
        call, receive = 0,0
        for values in db[looping]:
            call += (len(db[looping][values]))
        for v in db.values():
            for v1 in v:
                if v1 == looping:
                    receive += len(v[v1])
        result[looping] = call, receive
    return (sorted(result, key = lambda x : (result[x][0],-result[x][1],result[x]), reverse= True))
...
if __name__ == '__main__':
    ...
    db1 = {'Alan': {'Barb': [10, 20], 'Carl': [10, 10]},
               ...
               'Deja': {'Elan': [5]},
               'Elan': {'Carl': [10]}}
    print (all_parties(db1))
    opened_file = open('db1.txt')
    print ( read_db(opened_file))
    print (talker_frequency(db1))
    print (mutual_callers(db1))
    print (initiators(db1))
