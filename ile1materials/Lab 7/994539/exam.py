...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller, info in db.items():
        result.add(caller)
        for reciever, minutes in info.items():
            if reciever not in caller:
                result.add(reciever)
    return result
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        info = zip(call[1::4], call[2::2], call[3::2], call[4::2])
        for reciever, minutes, z, y in info:
            result[call[0]].update({reciever: [int(minutes), int(z), int(y)]})
    return dict(result)
