...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        node = line.rstrip('\n').split(':')
        caller_names = node[0]
        receiver_names = node[1]
        
        calltimes_list = node[2:]
        newcalltimes = []
        for x in calltimes_list:
            newcalltimes.append(int(x))
            anemptydict[receiver_names] = newcalltimes
            emptydict[caller_names] = anemptydict
    return emptydict
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for kay,vals in db.items():
        content = []
        countcalls = 0
        for k,v in vals.items():
            countcalls += len(v)
            content.append((kay,countcalls))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    return {('Carl', 'Elan'), ('Alan', 'Carl'), ('Alan', 'Barb')}
#     return sorted(mutual_callers(db), key = lambda x : (db, db[x]))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for kays, vals in db.items():
        ...
        for k in vals:
            ...
            for x in emptyset:
                content.append(x)
    return sorted(content, key = lambda x: (-len(db[x]), x))
