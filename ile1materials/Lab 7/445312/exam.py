...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    print(db)
    answer_set = set()
    for caller_name, value_dict in db.items():
        answer_set.add(caller_name)
        for receiver_name in value_dict.keys():
            answer_set.add(receiver_name)

    return answer_set

def read_db(file : open) -> {str:{str:[int]}}:
    initial_file = file.readlines()
    initial_list = []
    answer_dict = {}
    temp_list = []
    for each_line in initial_file:
       
        initial_list.append(each_line.strip().split(":"))
   
    for index in range(len(initial_list)):
        
        print('1', initial_list[index], '0', initial_list[index][0], '1', initial_list[index][1], '2', initial_list[index][2:])
        if initial_list[index][0] not in answer_dict.keys():
    
            answer_dict[initial_list[index][0]] = {initial_list[index][1]:initial_list[index][2:]}
        else:
            answer_dict[initial_list[index][0]].update({initial_list[index][1]:initial_list[index][2:]})    
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller_name, receiver_list in db.items():
        for receiver_name, receiver_time in receiver_list.items():
            #print(receiver_name, len(receiver_list[receiver_name]))
            print(caller_name,receiver_name, receiver_time, len(receiver_time))
            if caller_name not in answer_dict.keys():
                answer_dict[caller_name] = len(receiver_time)
               
            """ elif caller_name == caller_name or caller_name in answer_dict.values():
                print()
                 answer_dict[caller_name] += len(receiver_list[receiver_name])    
            """
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller_name, value_list in db.items():
        for receiver_name in value_list.keys():
            #print(caller_name, receiver_name)

            temp_list.append((caller_name, receiver_name))
            """
              if caller_name in db[receiver_name].keys() and receiver_name in db[caller_name].keys():
                pass
                #print(caller_name, receiver_name)
            """
    ...
    for i in range(len(temp_list)):
        for j in range(len(temp_list_copy)):
            #print(temp_list[i], temp_list_copy[j])
            if temp_list[i] == temp_list_copy[j]:
              #  if (temp_list[i], temp_list_copy[j])not in answer_list:
                answer_list.append(temp_list[i])
            #print(temp_list[i][0], ',',temp_list[i][1])
            #if temp_list[i][0] == temp_list_copy[j][1] and temp_list[i][1] == temp_list_copy[j][1]:
            #    print('!!!!!!!!!!!!!!!!!', temp_list[i][0], temp_list[i][1])
...
if __name__ == '__main__':
    ...
    file = goody.safe_open('enter db1', 'r', 'error message')
    read_db(file)
