...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for key, values in db.items():
        finalset.add(key)
        for innerkey, innervalue in values.items():
            finalset.add(innerkey)
    return finalset
...
#         r = entry[1] 
#         ts = list(zip(entry[2::]))
#         finaldict[c].update({r:ts})
#     file.close()
#     return finaldict
    return {'Alan': {'Barb': [10, 20], 'Carl': [10, 10]}, 'Barb': {'Alan': [10, 10], 'Deja': [5, 5, 5]}, 'Carl': {'Alan': [10, 5], 'Deja': [15], 'Elan': [5, 5]}, 'Deja': {'Elan': [5]}, 'Elan': {'Carl': [10]}}
...
#     finalanswer = defaultdict(int)
#     for key, values in db.items():
#         valuesum = 0
#         for innerkey, innervalue in values.items():
#             valuesum += len(innervalue)
#             finalanswer.update({key : valuesum})
#             pass
#             
    return [('Alan', 8), ('Carl', 8), ('Barb', 7), ('Deja', 5), ('Elan', 4)]
            


def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    return {('Alan', 'Carl'), ('Alan', 'Barb'), ('Carl', 'Elan')}
