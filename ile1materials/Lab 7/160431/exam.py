...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        db[line_list[0]].update({line_list[1]:[int(i) for i in line_list[2:]]})
    return dict(db)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for receiver in db[caller]:
            times[receiver] += len(db[caller][receiver])
        times[caller] += sum(len(db[caller][receiver]) for receiver in db[caller])
    return sorted(sorted([(t, times[t]) for t in times]), key = lambda x: x[1], reverse = True)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db:
        for receiver in db[caller]:
            t.append([caller, receiver])
    l = list(t)
    for item in l:
        item.reverse()
        if item in t:
            result.add(tuple(set(item)))
    return set(sorted(result, key = lambda x: (x[0], [x[1]])))
