...
def read_db(file : open) -> {str:{str:[int]}}:
    call_dict = defaultdict(dict)
    for line in file:
        linelist = line.rstrip('\n').split(':')
        caller, receiver = linelist.pop(0), linelist.pop(0)
        if receiver not in call_dict[caller]:
            call_dict[caller][receiver] = [int(x) for x in linelist]
        else:
            call_dict[caller][receiver].extend([int(x) for x in linelist])
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        for receiver in db[caller]:
            calls_dict[caller] += len(db[caller][receiver])
            calls_dict[receiver] += len(db[caller][receiver])
    for new_caller in calls_dict:
        calls_list.append((new_caller, calls_dict[new_caller]))
    return sorted(calls_list, key = lambda x: (-x[1], x[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller in db:
        for receiver in db[caller]:
            if caller in db[receiver] and (receiver, caller) not in mutual:
                mutual_tuple = (caller, receiver)
                mutual.add(tuple(sorted(list(mutual_tuple))))
    return mutual
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db:
        ...
        for reciever in db[caller]:
            participants.add(reciever)
    init_list = list(participants)
    c_dict = defaultdict(list)
    r_dict = defaultdict(list)
    for caller in db:
        for receiver in db[caller]:
            c_dict[caller].append(len(db[caller][receiver]))
            r_dict[receiver].append(len(db[caller][receiver]))
    return sorted(sorted(sorted(init_list), key = lambda y: sum(r_dict[y])), key = lambda z: -sum(c_dict[z]))
