...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    final_set = set()
    for caller in db:
        final_set.add(caller)
        for receiver in db[caller]:
            final_set.add(receiver)
    return final_set
...
#         print(line[0],line[1],line[2:])
        ...
        new_dict[line[0]][line[1]] = [int(i.rstrip()) for i in line[2:]] 
#     print(new_dict)
...
#         line = i.split(':')
#         print(line)
#         if new_dict[line[0]][line[1]]:
#             new_dict[line[0]][line[1]]=[int(i.rstrip()) for i in line[2:]]
#         else:
#             new_dict[line[0]][line[1]].extend([int(i.rstrip()) for i in line[2:]])
#     return dict(new_dict)
#         
#         
#     pass
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for i in db:
        if i in new:
            new[i]+=len([j for i in list(db[i].values()) for j in i] )
        else:
            new[i] = len([j for i in list(db[i].values()) for j in i] )
        for s in list(db[i]):
            print(s)
...
#     print(new)
    new = list(new.items())
    return(sorted(new,key = lambda x:(-x[1],x[0])))


def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    mutual_set = set()
    for key in db:
#         print(key,db[key].keys())
        for subkey in list(db[key].keys()):
            if key in list(db[subkey].keys()):
                k = [key,subkey]
                print(k)
                mutual_set.add(tuple(sorted(k,key = lambda x:x[0])))
#                 mutual_set.add(tuple([key,subkey].sort()))
#         if key in list(db[key].keys()):
#             mutual_set.add((key,db[key]))
    return mutual_set
...
#     for key in new_dict:
#         final_dict[key].insert(0,[j for i in db[key].values()for j in i])
#     
#     for key in new_dict:
#         for i in new_dict[key]:
#             if not final_dict[i][1]:
#                 final_dict[i].insert(1,len(new_dict[key][i]))
#             else:
#                 final_dict[i][1]+=len(new_dict[key][i])
    print([new_dict[key].values() for key in new_dict])
    k =[new_dict[key].values() for key in new_dict]
    caller_value = [j for i in k for j in i]
    print(caller_value)
#     e_list = []
    l = []
    e = {}
    def receiver(new_dict):
        for key in new_dict:
            for i in new_dict[key].keys():
                if i in e:
                    e[i]+=len(new_dict[key][i])
                else:
                    e[i]=len(new_dict[key][i])
        return e
    
    e_list = sorted([key for key in new_dict], key = lambda x: (len([new_dict[key].values() for key in new_dict])))
#     sorted([key for key in new_dict],key = lambda x: new_dict[key].values())
#     print(final_dict)
#     print(new_dict)
#     print(e_list)
    return(e_list)
