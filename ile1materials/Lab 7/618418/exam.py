...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        line.rstrip()
        line=line.split(':')
        if not line[1] in d[line[0]].keys():
            d[line[0]][line[1]]=[int(x) for x in line[2:]]
        else:
            d[line[0]][line[1]]+=[int(x) for x in line[2:]]
    return d
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db.keys():
        for receiver in db[caller].keys():
            e[caller]+=len(db[caller][receiver])
            e[receiver]+=len(db[caller][receiver])
    return sorted(sorted(e.items()),key=(lambda x: x[1]),reverse=True)



def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    f=set()
    def _rerange(name:(str,str)):
        if name[0]>name[1]:
    ...
    for caller in db.keys():
        for receiver in db[caller].keys(): 
            if receiver in db.keys() and caller in db[receiver].keys():
                f.add(_rerange((caller,receiver)))
    return f
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db.keys():
        for receiver in db[caller].keys():
            if not caller in g.keys():
                g[caller]=[len(db[caller][receiver]),0]
            else:
                g[caller][0]+=len(db[caller][receiver])
            if not receiver in g.keys():
                g[receiver]=[0,len(db[caller][receiver])]
            else:
                g[receiver][1]+=len(db[caller][receiver])
    g1=sorted(g.items())
    g2=sorted(g1,key=(lambda x:x[1][1]))
    g3=sorted(g2,key=(lambda x:x[1][0]),reverse=True)
    return [x[0] for x in g3]
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for caller in db.keys():
        for receiver in db[caller].keys():
            if not receiver in h[caller].keys():
                h[caller][receiver]=sum(db[caller][receiver])
            else:
                h[caller][receiver]+=sum(db[caller][receiver])
            if not caller in h[receiver].keys():
                h[receiver][caller]=sum(db[caller][receiver])
            else:
                h[receiver][caller]+=sum(db[caller][receiver])
    i=sorted(h.keys())
    num=len(i)
    ...
    for k in range(num):
        j.append([])
        for l in range(num):
            if i[l] in h[i[k]].keys():
                j[k].append(h[i[k]][i[l]])
            else:
                j[k].append(0)
    return j
