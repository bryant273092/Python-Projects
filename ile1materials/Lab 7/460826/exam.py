...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        if sline[0] in result: result[sline[0]].update({sline[1]: sline[2:]})
        else: result.update({sline[0]:{sline[1]:sline[2:]}})
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    result = {}
    for caller, calls in db.items():
        for called, number_of_calls  in calls.items():
            if caller in result: result[caller] += len(number_of_calls)
            else: result.update({caller:len(number_of_calls)})
            if called in result: result[called] += len(number_of_calls)
            else: result.update({called:len(number_of_calls)})
    ...
    for caller, calls in db.items():
        for called  in calls:
            ...
            if caller in db[called].keys():
                ...
                result.add((sort_temp[0],(sort_temp[1])))
    return result
...
#         for called, number_of_calls  in calls.items():       
#             if caller in db[called].keys(): 
#                 callss =  (len(db[called][caller])  )
#                 if called in result:
#                     
#                     
#                     result[called][1] += calls
#                     
#             calls_made +=    len(number_of_calls)
#                 
#         result.update({caller:  [calls_made, callss ]   })
#             #if called in result: result[called] += len(number_of_calls)
#             #else: result.update({called:len(number_of_calls)})
#         
# 
