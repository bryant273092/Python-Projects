...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db.keys():
        for r in db[c].keys():
            total[c] += len(db[c][r])
            total[r] += len(db[c][r])
    return(sorted(sorted(total.items()), key = lambda x:x[1], reverse = True))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db.keys():
        for r in db[c].keys():
            if c in db[r].keys():
                if (c,r) or (r,c) not in mutuals:
                    ms = []
                    ms.append(c)
                    ms.append(r)
                    ms = sorted(ms)
                    mutuals.add((ms[0],ms[1]))
    return mutuals
    ...
    for c in db.keys():
        for r in db[c].keys():
            if c not in total.keys():
                total[c].update({'caller':len(db[c][r])})
                total[c].update({'receiver':0})
            elif c in total.keys():
                total[c]['caller'] += len(db[c][r])
            if r not in total.keys():
                total[r].update({'receiver':len(db[c][r])})
                total[r].update({'caller':0})
            elif r in total.keys():
                total[r]['receiver'] += len(db[c][r])
    total = sorted(total.items())
    total = sorted(total, key = (lambda x : x[1]['receiver']))
    total = sorted(total, key = lambda x : x[1]['caller'], reverse = True)
    final = []
    for person in total:
