...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for num in range(2, len(line)):
            r_dict[reciever].append(int(line[num]))
        if reciever in final_dict[caller]:
            final_dict[caller][reciever].append(r_dict[reciever][0])
        else:
            final_dict[caller].update(dict(r_dict))
        r_dict.clear()
    return dict(final_dict)

def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    caller_list = defaultdict()
    caller=''
    count=0
    for key, value in db.items():
        caller= key
        for name, num_list in value.items():
            for num in range(len(num_list)):
                count+=1
        for key in db:
            if caller in db[key]:
                for num in range(len( db[key][caller])):
                    count+=1
        caller_list[caller]=count
        caller =''
        count=0
    return list([(names, caller_list[names]) for names in sorted(caller_list, key= lambda t :( -caller_list[t],t))])
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for key, value in db.items():
        ...
        for name in value:
            reciever = name
            if reciever in db.keys():
                if caller in db[reciever]:
                    caller_list.append({caller,reciever})
                    
    for set_names in caller_list:
        if set_names not in caller_set:
            caller_set.append(set_names)
            names =list(set_names)
            final_list.append((names[0],names[1]))
    for element in final_list:
        new_tup = sorted(element)
        final.add((new_tup[0], new_tup[1]))
    return final

def initiators (db : {str:{str:[int]}}) -> [str]:
    caller= defaultdict(list)
    caller_name=''
    caller_count=0
    recieve_count=0 
    for key, value in db.items():
        ...
        for name, num_list in value.items():
            for num in range(len(num_list)):
                caller_count+=1
        for key in db:
            if caller_name in db[key]:
                for num in range(len( db[key][caller_name])):
                    recieve_count+=1
        caller[caller_name].append([int(caller_count),int(recieve_count)])
        caller_count=0
        recieve_count=0
    return list(the_name for the_name in sorted(caller, key=lambda t:( -(caller[t][0][0]),caller[t][0][1])))
