...
def read_db(file : open) -> {str:{str:[int]}}:
    db = defaultdict(dict)
    db2 = defaultdict(list)
    for line in file:
        line = line.strip('\n').split(':')
        for t in line[2::1]:
            db2[line[1]].append(int(t))
        db[line[0]].update(db2)
        db2.clear()
    return dict(db)


def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    counter, talker_dict = 1, defaultdict(int)
    for c in db.keys():
        for receiver_dict in db.values():
            counter += list(receiver_dict.keys()).count(c)
        talker_dict.setdefault(c, counter)
        counter = 1
    return sorted([(name, t) for (name, t) in talker_dict.items()], key = lambda frequency: (-frequency[1], frequency[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db.keys():
        for r in db[c].keys():
            if (c in db[r].keys()) and (len(db[c][r]) >= 1) and (len(db[r][c]) >= 1):
                if (r,c) not in mutual_set:
                    mutual_set.add((c, r))
    return mutual_set
