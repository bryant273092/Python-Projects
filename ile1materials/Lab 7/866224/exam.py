...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for i in line[2::]:
            call_time.append(int(i))
        d1[line[0]][line[1]] = call_time
        call_time = []    
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    s = set()
    count = 0
    #sort list decreasing number of calls then if same sort
    # by increasing alphabetic order
    for c in db: #returns key
        s.add(c)
        count += 1
        for v in db[c]: 
            d3[c] += len(db[c][v]) + len(list1)
        count = 0
    return sorted(sorted(d3.items(),key = lambda x:x[1]), key = lambda x:x[0])
