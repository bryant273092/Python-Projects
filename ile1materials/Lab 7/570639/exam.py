...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    call_participants = set(db.keys())
    for receiver in db.values():
        call_participants.update(receiver.keys())
    return call_participants
    # participants = set(db.keys()).union({person for _caller,receiver in db.items() for person in receiver.keys()})



def read_db(file : open) -> {str:{str:[int]}}:
    call_dict = defaultdict(lambda: defaultdict(list))
    for line in file:
        caller, recipient, *durations = line.rstrip().split(':')
        call_dict[caller][recipient].extend(int(dur) for dur in durations)    
    return {caller:dict(recipients) for caller,recipients in call_dict.items()}



def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    frequency_list = []
    for participant in all_parties(db):
        count = 0
        if participant in db.keys():
            count += sum(len(val) for val in db[participant].values())
        for call_receivers in db.values():
            if participant in call_receivers:
                count += len(call_receivers[participant])
        frequency_list.append((participant, count))        
    return sorted(frequency_list, key = lambda x: (-x[1],x[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    m_callers = []
    for caller in db:
        for recipient in db[caller]:
            if recipient in db and caller in db[recipient]:
                m_callers.append(sorted((caller, recipient)))
    return {tuple(pair) for pair in m_callers}
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db:
        num_times_caller = sum(len(val) for val in db[caller].values())
        num_times_receiver = sum([len(val[caller]) for val in db.values() if caller in val])
        temp_dict[caller].extend([num_times_caller, num_times_receiver])    
    return sorted(temp_dict, key = lambda caller: (-temp_dict[caller][0], temp_dict[caller][1], caller))
