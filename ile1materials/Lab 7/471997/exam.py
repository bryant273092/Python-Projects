...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for number in line[2:]:
            call_time.append(int(number))
        return_dict[line[0]][line[1]] = call_time
    return return_dict

def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    return_list = [] 
    test_dict = defaultdict(int)
    for name,caller_list in db.items():
        counter = 0
        for num_calls in caller_list.values():
            counter += len(num_calls)
        for name_list in db.values():
            if name in name_list.keys():
                counter += len(name_list[name])
        return_list.append((name,counter))
    return sorted(sorted(return_list), key = lambda x: x[1], reverse = True)


def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    return_set = set()
    for name, caller_list in db.items():
        for receiver in caller_list.keys():
            if name in db[receiver].keys():
                return_list = sorted([receiver, name])
                return_tuple = tuple(return_list)
                return_set.add(return_tuple)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k,v in db.items():
        ...
        for value in v.values():
            calls_made += len(value)
        calls_received = 0
        for receiver in db.values():
            if k in receiver.keys():
                calls_received += len(receiver[k]) 
        temp_dict[k] = (calls_made, calls_received)
#     sorted_list = sorted(temp_dict.items(), key = lambda x: x[1], reverse = True)
    sorted_list = sorted(sorted(temp_dict.items(), key = lambda x: x[1][1]),key =  lambda x: x[1][0], reverse = True)
