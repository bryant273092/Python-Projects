...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        for i in range(2,len(list0)):
            time.append(int(list0[i]))
        try: 
            if receiver in dict0[caller].keys():
                dict0[caller][receiver].extend(time)
        except:
            pass
        dict0[caller].update({receiver:time})
    return dict0
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for key, value in db.items():
        ...
        for i in value.values():
            for_caller.extend(i)
        dict0[key]+=len(for_caller)
        for receiver, times in value.items():
            dict0[receiver]+=len(times)
    return sorted(dict0.items(),key= lambda x:(-x[-1],x[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    finallist=[]
    for i in midlist:
        finallist.append(tuple(sorted(list(i))))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for i,j in dict0.items():
        dict2[i].append(j)
    for g,h in dict1.items():
        dict2[g].append(h)
    final=sorted(dict2.items(),key=lambda x:(-x[-1][0],x[-1][-1]))
    f=[]
    for q in final:
        f.append(q[0])
    return f
