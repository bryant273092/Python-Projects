...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller, v in db.items():
        for recieve, talk in v.items():
            if caller not in final:
                final.append(caller)
            if recieve not in final:
                final.append(recieve)
    return set(final)
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        m = []
        in_dict = []
        for meh in options[2:]:
            meh = int(meh)
            m.append(meh)
        in_dict.append((options[1],m))
        final[options[0]] = dict(in_dict)
    return final      
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    meh = []
    for caller, v in db.items():
        for rec, time in v.items():
            inner = {}
            meh.append((caller,len(time)))
            meh.append((rec,len(time)))
            for name, num in meh:
                if name not in inner:
                    inner[name] = num
                else:
                    inner[name] += num
    final = sorted(list(inner.items()))
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    meh = []
    inner = []
    for call,v in db.items():
        for rec, time in v.items():
            counter = 0
            meh.append(call)
            if call in meh:
                counter += 1
            if call not in inner:
                inner.append(call)    
    inner = sorted(inner, key = lambda counter: counter) 
    return inner
