...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        receiver = parsed[1]
        ts = parsed[2:]
        final_dict.update({caller: {receiver: [int(t) for t in ts]}})
    return final_dict
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    callers = []
    called_list = dict(defaultdict(list))
    mutual = set()
    
    for c,v in db.items():
        callers.append(c)
        called_list[c] = v.keys()
    
    for caller in callers:
        for k,v in called_list.items():
            if caller in v:
                mutual.add((caller, k))
    return mutual
