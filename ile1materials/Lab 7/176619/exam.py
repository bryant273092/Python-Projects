...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for lines in file.readlines():
        ...
        curperson = newlines.pop(0)
        pcalled = newlines.pop(0)
        for i in range(len(newlines)):
            newlines[i] = int(newlines[i])
        inputval = {pcalled:newlines}
        if curperson not in allppl:
            allppl[curperson] = set()
        allppl[curperson].add(inputval)
    return allppl


def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    allppl = all_parties(db)
    retlst = list()
    for person in allppl:
        ...
        for x in db.values():
            ...
            for p in tup:
                if p == person:
                    count += len(x[p])
        retlst.append((person,count))
    return sorted(retlst, key = lambda item: (-item[1],item[0]), reverse = False)
