...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if not word[1] in d[word[0]]:
            d[word[0]][word[1]] = []
        for i in word[2:]:
            d[word[0]][word[1]].append(int(i))
    file.close()
    return dict(d)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k, v in db.items():
        for k1, v1 in v.items():
            freq[k1] = freq.get(k1,0) + len(v1)
            freq[k] = freq.get(k,0) + len(v1)
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for k,v in db.items():
        name = list(v.keys())
        for i in name:
            if i in db and k in db[i]:
                result.add(tuple(c for c in sorted((k,i))))
    return result
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for k,v in db.items():
        ...
        for k1,v1 in v.items():
            ...
            c[k]+=len(v1)
            r[k1]+=len(v1)
    result = sorted(name, key = lambda x: (-c[x], r[x], x))
    return result



def matrix (db : {str:{str:[int]}}) -> [[int]]:
    names = all_parties(db)
    new_d = defaultdict(dict)
    for i in names:
        for k in db[i]:
            new_d[i][k] = sum(db[i][k]+sum(db[k].get(i,[])))
    print(new_d)
