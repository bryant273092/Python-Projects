...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for lines in file:
        ...
        new = [line[1]]
#         for j in new:
        new_dict.append(dict(zip(new, [line[2:]])))
        for keys in key:
            for j in new_dict:
                d[keys] = j
    print(d)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    new = []
    for x in db.keys():
        for k in db[x].values():
            calls += len(k)
        for y in db.values():
            for i, j in y.items():
                if x == i:
                    calls += len(j)
        num_calls.append((x, calls))
        calls = 0
    return sorted(sorted(num_calls, key = lambda t : t[0]), key = lambda t : t[1], reverse = True)
