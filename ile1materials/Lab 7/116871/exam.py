...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in content:
        ...
        for x in l[2:]:
            temp.append(int(x))
        if l[1] not in result[l[0]]:
            result[l[0]][l[1]]=[]
        result[l[0]][l[1]]=result[l[0]][l[1]]+temp
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for call in db:
        ...
        for rece in db[call]:
            if rece not in temp:
                temp[rece]=0
            for minute in db[call][rece]:
                temp[call]+=1
                temp[rece]+=1
    for x in sorted(temp.items(),key=(lambda y:(-y[1],y[0]))):
        result.append(x)
    return result
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for call in db:
        for rece in db[call]:
            if rece in db and call in db[rece]:
                temp=sorted([call,rece])
                if (temp[0],temp[1]) not in result:
    ...
    for call in db:
        ...
        for rece in db[call]:
            ...
            for minute in db[call][rece]:
                temp[call][0]+=1
                temp[rece][1]+=1
    for x in sorted(temp.items(),key=(lambda y:(-y[1][0],y[1][1]))):
        result.append(x[0])
    return result
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for x in dic:
        temp1=[]
        for n in dic:
            temp1.append(set((x[0],n[0])))
        result.append(temp1)
    temp={}
    for call in dic:
        for rece in call[1]:
            if call[0]+';'+rece not in temp:
                temp[call[0]+';'+rece]=0
            for number in call[1][rece]:
                temp[call[0]+';'+rece]+=number
    for x in temp:
        new=x.split(';')
        key=set((new[0],new[1]))
        count1=0
        ...
        while count1< len(result):
            while count2<len(result):
                if result[count1][count2] == key:
                    result[count1][count2]=0
                    if new[1]+';'+new[0] in temp:
                        result[count1][count2]=temp[x]+temp[new[1]+';'+new[0]]
                    else:
                        result[count1][count2]+=temp[x]
                count2+=1
            count1+=1
            count2=0
    count1=0
    ...
    while count1< len(result):
        while count2<len(result):
            if type(result[count1][count2])!=int:
                result[count1][count2]=0
            count2+=1
        count1+=1
        count2=0
