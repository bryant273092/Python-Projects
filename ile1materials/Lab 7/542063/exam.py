...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for outgoing_caller, incoming_dict in db.items():
        callers.add(outgoing_caller)
        for incoming_caller in incoming_dict.keys():
            callers.add(incoming_caller)
    return (callers)
...
def read_db(file : open) -> {str:{str:[int]}}:
    actual_dict = defaultdict(dict)
    
    for line in file:
        new_line = line.strip().split(":")
        
        outgoing_caller = new_line[0]
        incoming_caller = new_line[1]
        minutes = new_line[2:]
        new_minutes = []
        
        for time in minutes:
            new_minutes.append(int(time))
        
        actual_dict[outgoing_caller].update({incoming_caller:new_minutes})
  
    return dict(actual_dict)

read_db(open("db1.txt"))

def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    number_calls = defaultdict(int)
    
    for outgoing_call, incoming_dict in db.items():
        for reciever, minutes in incoming_dict.items():
            for time in minutes:
                number_calls[outgoing_call]+=1
                number_calls[reciever]+=1
    return(sorted(list(number_calls.items()),key = lambda x:(-x[1],x[0])))
    

def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    mutual_dict = defaultdict(set)
    mutual_set = set()
    
    for outgoing_caller, incoming_dict in db.items():
        for incoming_caller in incoming_dict.keys():
            mutual_dict[outgoing_caller].add(incoming_caller)
    mutual_dict = dict(mutual_dict)
    
    for each_person in mutual_dict:
        for each_called in mutual_dict[each_person]:
            if each_called in mutual_dict and each_person in mutual_dict[each_called]:
                x = tuple(sorted((each_person,each_called),key = lambda x: x[0]))
                mutual_set.add(x)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for outgoing_caller, incoming_dict in db.items():
        outgoing_count = 0
        for minutes in incoming_dict.values():
            for time in minutes:
                outgoing_count+=1
        temp_dict[outgoing_caller].update({"out":outgoing_count})
        
        for each_person in db:
            for each_called in db[each_person].items():
                if each_called[0] == outgoing_caller:
                    incoming_count = 0
                    for each_minute in each_called[1]:

                        incoming_count +=1
                    temp_dict[outgoing_caller].update({'in':incoming_count})     
            
    return sorted(temp_dict,key = lambda x:x[1][0])
