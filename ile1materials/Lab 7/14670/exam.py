...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        infor=line.strip('\n').split(':')
        if infor[0] in db_dict.keys():
            if infor[1] in db_dict[infor[0]]:
                for i in range(2,len(infor)):
                    db_dict[infor[0]][infor[1]].append(int(infor[i]))
                
            else:
                db_dict[infor[0]][infor[1]]=[int(infor[i]) for i in range(2,len(infor))]
        else:        
            db_dict[infor[0]][infor[1]]=[int(infor[i]) for i in range(2,len(infor))]
    return db_dict
            ...
            if x==caller:
                ...
                for i in y.keys():
                    if i==caller:
                        for number in y[i]:
                            frequency[c]+=1
    return sorted([(c,n) for c,n in frequency.items()],key=(lambda x:(-x[1],x[0])))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c,v in db.items():
        for r in v.keys():
            if c in db[r].keys():
                call_set=tuple(sorted((c,r)))
                if call_set not in mutual_call:
                    mutual_call.add(call_set)
    return mutual_call



def initiators (db : {str:{str:[int]}}) -> [str]:
    initiators_dict=defaultdict()
    for c,v in db.items():
        caller_number=0
        receiver_number=0
        for r in v.keys():
            caller_number+=len(v[r])
        for x,y in db.items():
            if x!=c and c in y.keys():
                receiver_number+=len(y[c])
        initiators_dict[c]=(caller_number,receiver_number)
    initiators_list=sorted([(x,y)for x,y in initiators_dict.items()],key=(lambda x:(-x[1][0],x[1][1])))
    return [i[0] for i in initiators_list]
