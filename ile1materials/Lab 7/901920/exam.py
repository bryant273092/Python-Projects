...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    result = set()
    people_list = []
    for caller, value_1 in db.items():
        if caller not in result:
            result.add(caller)

        for receiver, time in value_1.items():
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in lines:
        ...
        if name not in name_list:
            ...
            result[name] = {receiver:time_list}
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    frequency_dict = {}
    name_list = []
    for caller, value in db.items():
        frequency_dict[caller] = 0
        for receiver, times in value.items():
            if caller not in name_list:
                frequency_dict[caller] += len(times)

                name_list.append(caller)
                
                if receiver not in name_list:
                    name_list.append(receiver)
                    frequency_dict[receiver] = len(times)
                elif receiver in name_list:
                    frequency_dict[receiver] += len(times)

            elif caller in name_list:
                frequency_dict[caller] += len(times)
