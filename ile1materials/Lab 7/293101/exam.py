...
def all_parties(db : {str:{str:[int]}}) -> {str}:
    ...
    for caller in db:
        result.add(caller)
        for receiver_info in db.get(caller):
            result.add(receiver_info)
    return result
...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if info[0] not in result:
            ...
            integer_list1 = []
            for i in info[2:]:
                integer_list1.append(int(i))
            sub_dict1[info[1]] = integer_list1
            result[info[0]] = sub_dict1
        else:
            sub_dict2 = result.get(info[0])
            integer_list2 = []
            for j in info[2:]:
                integer_list2.append(int(j))
            if info[1] in sub_dict2:
                total_list = sub_dict2.get(info[1])
                total_list.extend(integer_list2)
                sub_dict2[info[1]] = total_list
            else:
                sub_dict2[info[1]] = integer_list2
            result[info[0]] = sub_dict2
    return result
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for caller in db:
        count = 0
        receiver_info = db.get(caller)
        for num_list in receiver_info.values():
            for Num in num_list:
                Num = Num
                count += 1
        result[caller] = count
    for key in db:
        ...
        for sub_key in value_dict:
            if sub_key not in result:
                count1 = 0
                for i in value_dict.get(sub_key):
                    i = i
                    count1 += 1
                result[sub_key] = count1
            else:
                count2 = result.get(sub_key)
                for j in db.get(key).get(sub_key):
                    j = j
                    count2 += 1
                result[sub_key] = count2
    return list(sorted(result.items(), key=(lambda item : (-item[1], item[0]))))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    result = []
    for caller in db:
        for receiver in db.get(caller):
            if receiver in db:
                if caller in db.get(receiver):
                    if tuple(sorted((caller, receiver))) not in result:
                        result.append(tuple(sorted((caller, receiver))))
    return set(result)
...
def initiators (db : {str:{str:[int]}}) -> [str]:
    ...
    for caller in db:
        count = 0
        for num_list in db.get(caller).values():
            for num in num_list:
                num = num
                count += 1
        result[caller] = [count]
    for key in result:
        ...
        for value in db.values():
            if key in value:
                for Num in value.get(key):
                    Num = Num
                    count1 += 1
            else:
                pass
        key_info = result.get(key)
        key_info.append(count1)
        result[key] = key_info
    for k in sorted(result.items(), key = (lambda item : (-item[1][0], item[1][1], item[0]))):
...
def matrix (db : {str:{str:[int]}}) -> [[int]]:
    ...
    for caller in db:
        ...
        for key in db:
            sub_dict[key] = 0
        result[caller] = sub_dict
    for Caller in db:
        for receiver in db.get(Caller):
            count = 0
            for num in db.get(Caller).get(receiver):
                count += num
            subdict = result.get(Caller)
            call_length = subdict.get(receiver)
            call_length += count
            subdict[receiver] = call_length
            result[Caller] = subdict
            subdict1 = result.get(receiver)
            call_length1 = subdict1.get(Caller)
            call_length1 += count
            subdict1[Caller] = call_length1
            result[receiver] = subdict1
    for k in result:
        call_list = []
        for i in result.get(k).values():
            call_list.append(int(i))
        matrix.append(call_list)
    return matrix
