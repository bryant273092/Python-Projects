...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file.readlines():
        ...
        int_list = []
        for num in item_list[2:]:
            int_list.append(int(num))
        inner_dict = {}
        inner_dict[item_list[1]]= list(int_list)
        if item_list[0] in d.keys():
            d[first_item].update(inner_dict)
        else:
            d[first_item] = inner_dict
    return d   
    ...
    for person in sorted(person_list):
        ...
        for k, vs in db.items():
            ...
            for m, n in vs.items():
                if m == person:
                    count += len(n)
        count_list.append((person, count))
    return sorted(count_list, key = lambda item: item[1], reverse = True)
        ...
        for k, vs in db.items():
            if k == person:
                ...
                for m, n in vs.items():
                    if m == person:
                        r_count += len(n)
                count_list.append((person, c_count, r_count))
    
    return [p for p, c, r in sorted(count_list, key = (lambda x:x[1]), reverse=True)]
