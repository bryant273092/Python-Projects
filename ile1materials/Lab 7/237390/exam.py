...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        #inner_dict[ listed_line[1]]=[int(x) for x in listed_line[2:]]
        #main_dict[listed_line[0]].update(inner_dict)
        if listed_line[1] not in main_dict[listed_line[0]]:
            main_dict[listed_line[0]].update({listed_line[1]:[int(x) for x in listed_line[2:]]})
        else:
            #print('this guy was already called',listed_line[1])
            #print('already saved minutes:',main_dict[listed_line[0]][listed_line[1]])
            #print('new minutes:',[int(x) for x in listed_line[2:]])
            new_list = main_dict[listed_line[0]][listed_line[1]]+[int(x) for x in listed_line[2:]]
            #print('new_list',new_list)
            #print([1,2,3,4] +[7,8])
            main_dict[listed_line[0]][listed_line[1]] = new_list

    #print('print9 main-ditc',main_dict)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for c in db:
        #print(c)
        for r in db[c]:
            #print('caller',c,'called ',r, 'this many times',len(db[c][r]))
            temp_dict[c] += len(db[c][r])
            temp_dict[r] += len(db[c][r])
    #print(temp_dict)
    main_list = [(i,temp_dict[i]) for i in temp_dict ]
    #print('fINNNAALLA ANSWER MAINLIST', sorted(main_list, key = lambda x: -x[1]))
    return sorted(main_list, key = lambda x:( -x[1],x[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c in db:
        for r in db[c]:
            if c in db[r] :
                #print('this caller',c,'is inside dict of reciver ',r)
                if c[0] > r[0]:
                    main_set.add((r,c))
                else:
                    main_set.add((c,r))
