...
def read_db(file : open) -> {str:{str:[int]}}:
    db_list = [db.rstrip() for db in file]
    db_dict = defaultdict(dict)
    for a in db_list:
        b= a.split(":")
        db_dict[b[0]].update({b[1]:[int(a) for a in b[2:]]})
    return dict(db_dict)
        ...
        for c,v in db.items():
            if c == caller:
                ...
                for a,b in v.items():
                    if a == caller:
                        fre = fre +len(b)
        frequency_list.append((caller,fre))
    return sorted(frequency_list,key = lambda t:(-t[1],t[0]))
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for c,v in db.items():
        for a,b in v.items():
            db_new_dict[(c,a)] = sum(int(i) for i in b)
    db_dict = {}
    
    for c,v in db_new_dict.items():
        temp=c
        if all(temp[0] != e[1] and temp[0] != e[0] for e in db_dict.keys()):
            db_dict[temp]=v
            
            for a,b in db_new_dict.items():
                if temp[0] == a[1] and temp[1]==a[0]:
                    db_dict[c] = db_dict[c] + b
    print(db_dict)
    return(sorted(list(db_dict),key=lambda t:-db_dict[t]))
