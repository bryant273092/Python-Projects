...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for i in file:
        ...
        if(line[0] not in d.keys()):
            d[line[0]] = {line[1]:[int(h.replace("\n", "")) for h in line[2:]]}
        else:
            if(line[1] not in d[line[0]]):
                d[line[0]].update({line[1]:[int(h.replace("\n", "")) for h in line[2:]]})
            else:
                d[line[0]][line[1]].extend([int(h.replace("\n", "")) for h in line[2:]])
    return dict(d)
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k in db.keys():
        caller=0
        for c in db[k].values():
            caller += len(c)
        reciever=0
        for r in db.values():
            if(k in r):
                reciever += len(r[k])   
        result.append((k,caller+reciever))
    ...
    for k in db.keys():
        ...
        for r in db.values():
            if(k in r):
                reciever += len(r[k])   
        result.append((k,caller,reciever))
    return [i[0] for i in sorted(sorted(sorted(result, key=lambda x:x[0]), key=lambda x:x[2]), key=lambda x:x[1], reverse=True)]
