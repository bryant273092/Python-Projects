...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for line in file:
        ...
        if entry[0] in db.keys():
            if entry[1] not in db[entry[0]].keys():
                db[entry[0]].update({entry[1]:[int(i) for i in entry[2:]]})
            elif entry[1] in db[entry[0]].keys():
                db[entry[0]][entry[1]].extend(int(i) for i in entry[2:])
        elif entry[0] not in db.keys():
            db[entry[0]] = {entry[1]:[int(i) for i in entry[2:]]}
...
def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    ...
    for k in db.keys():
        frequency = 0
        for c,ls in db.items():
            for r,t in ls.items():
                if c == k:
                    frequency += len(t)
                elif r == k:
                    frequency += len(t)
        talkers.append((k,frequency))
    return sorted(sorted(talkers,key = (lambda x: x[0])),key = (lambda x: x[1]),reverse=True)
        ...
        for c,ls in db.items():
            for r,t in ls.items():
                if c == k:
                    ...
                    received += len(t)
        talkers[k]=((called,received))
    return sorted(sorted(talkers,key = lambda x: talkers[x][1]),key = lambda x: talkers[x][0],reverse=True)
