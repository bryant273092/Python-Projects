...
def read_db(file : open) -> {str:{str:[int]}}:
    ...
    for i in file:
        c,r,*c_l = i.rstrip().split(':')
        if r in main_dict[c]:
            main_dict[c][r]=main_dict[c][r]+[int(b) for b in c_l]
        else:
            main_dict[c][r]=[int(b) for b in c_l]
    return dict(main_dict)


def talker_frequency (db : {str:{str:[int]}}) -> [(str,int)]:
    call_set = set()
    for caller, log in db.items():
        call_count = 0
        for reciever, calls in log.items():
            call_count += len(calls)
        for caller2, log2 in db.items():
            for reciever2, calls2 in log2.items():
                if caller == reciever2:
                    call_count +=len(calls2)
        call_set.add((caller, call_count))        
    return [(i[0],i[1]) for i in sorted(call_set, key = lambda i: (-i[1],i[0]))]
...
def mutual_callers (db : {str:{str:[int]}}) -> {(str,str)}:
    ...
    for caller, log in db.items():
        for reciever, calls in log.items():
            if reciever in db and caller in db[reciever].keys():
                call_set.add(tuple(sorted((caller, reciever))))
    return call_set
...
#     for caller, log in db.items():
#         for reciever, calls in log.items():
#             if caller in db[reciever].keys() and (reciever, caller) not in call_set:
#                 call_set.add(tuple(sorted((caller,reciever))))
#     return call_set
